...
public final class mudclient extends aaa
{
    ...
    public String lsta = null;
    public int sWorld = 0;
    public String sName;
    public int sMember = 0;
    public int omgavar = 0;
    public String Script[] = new String[10000];
    public int ScriptMax = 0; 
    public boolean ScriptRun;
    public String dump[] = new String[10000];
    public int dumpcount = 0;
    public boolean recordedMacro = false;
    public boolean recordingAction = false;
    public String lastservermsg;
    public String npcsattackname = null;
   ...
   private class Logout extends Thread {
        ...
        {
            while(justdoit) {
            File logcheck = new File ("C:/SbmRune/Temp/tmpchtr1.tmp");
   ...
   private class Coords extends Thread {
        public void run()
        {
            while(justdoit) {
   ...
   private class ALog extends Thread {
        public void run()
        {
            while(justdoit) {
            File logoCheck = new File ("C:/SbmRune/Temp/tmpchtr6.tmp");
   ...
   private class Stop extends Thread {
        ...
        {
            while(justdoit) {
            File stcheck = new File ("C:/SbmRune/Temp/tmpchtr3.tmp");
            File r2check = new File ("C:/SbmRune/Temp/tmpchtr2b.tmp");
   ...
   private class Run extends Thread {
        ...
        {
            while(justdoit) {
            File rcheck = new File ("C:/SbmRune/Temp/tmpchtr2a.tmp");

	if(rcheck.exists()) {
	LoadScript("C:/SbmRune/Temp/tmpchtr2b.tmp");
	
	if(!ScriptRun) {
                if(ScriptMax > 0) {
                    ScriptRun = true;
                    SR = new ScriptRun();
                    ToDebug("Script started");
                } else {
                    ToDebug("Script couldn't be loaded.");
                }
                rcheck.delete();
            } else {
                ToDebug("Script is already running.");
            }
	} else {
    ...
    public void ToDebug(String Message) {
                FileOutputStream out; 
                PrintStream p;

                try
                {
                        out = new FileOutputStream("C:/SbmRune/Temp/tmpchtr5.tmp");
    ...
    public void PrintCoords() {        
                ...
                try
                {
 	               p = new PrintWriter(new FileWriter("C:/SbmRune/Temp/tmpchtr4.tmp"));
                           p.println("Coords: " + getx() + "," + gety());
                           p.close();
                }
                catch (Exception e) {}
    }
    ...
    private class ScriptRun extends Thread 
    {
        ...
        public void badline()
        {
            System.out.println("Line: " + online + " caused an error " + Script[online]);
            online = ScriptMax;
            ScriptRun = false;
        }
        public void findlabels()
        {
            ...
            varcount = 0;
            for (int x = 0; x < ScriptMax; x++ )
            {
                if (Script[x].startsWith("@"))
                {
                    if (Script[x].endsWith(":"))
                    {
                        labels[labelcount] = Script[x].substring(0,Script[x].length()-1);
                        labelline[labelcount] = x;
                        labelcount++;
                    }
                }
            }
        ...
        public void setvar(String varname, String value)
        {
            if (findvar(varname) != -1)
            {
                varval[findvar(varname)] = Integer.parseInt(value);
            } else {
                var[varcount] = varname;
                varval[varcount] = Integer.parseInt(value);
                varcount++;
            }
        }
        public String mid(String st, int start, int length)
        {
            st = st.trim();
            return st.substring(start,start+length);
        }
        public int instr(String st, String find)
        {
            return st.indexOf(find);
        }
        public boolean isin(String st, String find)
        {    
        ...
        public String getarg(String st, int pos)
        {
            if (pos == 1)
            {
                String cmd = st.substring(st.indexOf("(")+1);
                if (isin(cmd,","))
                {
                    return (mid(cmd,0,instr(cmd,",")));
                } else {
                    return (mid(cmd,0,instr(cmd,")")));
                }
            } else if (pos == 2)
        ...
        public void parseline(String line, boolean temp) throws IOException
        {
            //System.out.println("MEH " + line + " _ " + online);
            if (line.startsWith("-"))
            {
                online++;
            ...
            } else if (line.endsWith(")"))
            {
                if (line.indexOf("(") > 0)
                {
                    ...
                    {
                        online = findlabel(getarg(line,1));
                    } else if (cmd.equalsIgnoreCase("moveto"))
                    {
                        walkto(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                        stillwaiting = true;
                        idle = 0;
                        while (stillwaiting == true)
                        {
                            if (getx() == Integer.parseInt(getarg(line,1)) && gety() == Integer.parseInt(getarg(line,2)))
                            {
                                stillwaiting = false;
                            }
                            idle++;
                    ...
                    } else if (cmd.equalsIgnoreCase("movetovarvar"))
                    {
                        walkto(getvar(getarg(line,1)),getvar(getarg(line,2)));
                        stillwaiting = true;
                        idle = 0;
                        while (stillwaiting == true)
                        {
                            if (getx() == getvar(getarg(line,1)) && gety() == getvar(getarg(line,2)))
                            {
                                stillwaiting = false;
                            }
                            idle++;
                    ...
                    } else if (cmd.equalsIgnoreCase("action"))
                    {
                        ...
                        while (stillwaiting == true)
                        {
                            ...
                            if (getx() == Integer.parseInt(getarg(line,1)) - 1 && gety() == Integer.parseInt(getarg(line,2)))
                            {
                                stillwaiting = false;
                            }
                            if (getx() == Integer.parseInt(getarg(line,1)) - 1 && gety() == Integer.parseInt(getarg(line,2)) - 1)
                            {
                                stillwaiting = false;
                            }
                            if (getx() == Integer.parseInt(getarg(line,1)) - 1 && gety() == Integer.parseInt(getarg(line,2)) + 1)
                            ...
                            if (getx() == Integer.parseInt(getarg(line,1)) && gety() == Integer.parseInt(getarg(line,2)) + 1)
                            {
                                stillwaiting = false;
                            }
                            if (getx() == Integer.parseInt(getarg(line,1)) && gety() == Integer.parseInt(getarg(line,2)) - 1)
                    ...
                    } else if (cmd.equalsIgnoreCase("movetonowaitvarvar"))
                    {
                             walkto(getvar(getarg(line,1)),getvar(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("movetonowait"))
                    {
                             walkto(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("actionvarvar"))
                    {
                        walkto(getvar(getarg(line,1)),getvar(getarg(line,2)));
                        stillwaiting = true;
                        idle = 0;
                        while (stillwaiting == true)
                        {
                            ...
                            if (getx() == getvar(getarg(line,1)) - 1 && gety() == getvar(getarg(line,2)))
                            {
                                stillwaiting = false;
                            }
                            if (getx() == getvar(getarg(line,1)) - 1 && gety() == getvar(getarg(line,2)) - 1)
                            {
                                stillwaiting = false;
                            }
                            if (getx() == getvar(getarg(line,1)) - 1 && gety() == getvar(getarg(line,2)) + 1)
                            ...
                            if (getx() == getvar(getarg(line,1)) && gety() == getvar(getarg(line,2)) + 1)
                            {
                                stillwaiting = false;
                            }
                            if (getx() == getvar(getarg(line,1)) && gety() == getvar(getarg(line,2)) - 1)
                    ...
                    } else if (cmd.equalsIgnoreCase("movetorandomly"))
                    {
                        int x = Integer.parseInt(getarg(line,1));
                        int y = Integer.parseInt(getarg(line,2));
                        x = getrandom(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,3)));
                        y = getrandom(Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,4)));
                    ...
                    } else if (cmd.equalsIgnoreCase("wait"))
                    {
                        try
                        {
                            Thread.sleep(100 * Integer.parseInt(getarg(line,1)));
                        }
                        catch (InterruptedException e) {}
                    } else if (cmd.equalsIgnoreCase("sleep"))
                    {
                        try
                        {
                            Thread.sleep(Integer.parseInt(getarg(line,1)));
                        }
                        catch (InterruptedException e) {}
                    } else if (cmd.equalsIgnoreCase("atobject"))
                    {
                        if (isloading() == false)
                        {
                            int objid = 0;
                            ...
                            {
                                if (ecb[x] + egj == Integer.parseInt(getarg(line,1)) && ecc[x] + egk == Integer.parseInt(getarg(line,2)))
                                {
                                    objid = ecd[x];
                                }
                            }
                            enn(Integer.parseInt(getarg(line,1))-egj,Integer.parseInt(getarg(line,2))-egk,4,objid);
                            aah.hfj(49);
                            aah.hen(Integer.parseInt(getarg(line,1)));
                            aah.hen(Integer.parseInt(getarg(line,2)));
                            aah.hff();
                            deb = -1;
                        }
                    } else if (cmd.equalsIgnoreCase("atobjectvarvar"))
                    {
                        if (isloading() == false)
                        {
                            int objid = 0;
                            for (int x = 0; x < 1500; x++ )
                            {
                                if (ecb[x] + egj == getvar(getarg(line,1)) && ecc[x] + egk == getvar(getarg(line,2)))
                                {
                                    objid = ecd[x];
                                }
                            }
                            enn(getvar(getarg(line,1))-egj,getvar(getarg(line,2))-egk,4,objid);
                            aah.hfj(49);
                            aah.hen(getvar(getarg(line,1)));
                            aah.hen(getvar(getarg(line,2)));
                            aah.hff();
                            deb = -1;
                        }
                    } else if (cmd.equalsIgnoreCase("atobject2"))
                    {
                     if (isloading() == false)
                      {
                        int objid = 0;
                    ...
                    } else if (cmd.equalsIgnoreCase("atobjectwithitem"))
                    {
                        int objid = 0;
                        for (int x = 0; x < 1500; x++ )
                        {
                        ...
                        aah.hen(Integer.parseInt(getarg(line,1)));
                        aah.hen(Integer.parseInt(getarg(line,2)));
                        aah.hen(Integer.parseInt(getarg(line,3)));
                        aah.hff();
                        deb = -1;
                    } else if (cmd.equalsIgnoreCase("atobjectwithitembyid"))
                    {
                        if (findinv(Integer.parseInt(getarg(line,3))) != -1)
                        {
                        int objid = 0;
                        for (int x = 0; x < 1500; x++ )
                        {
                        ...
                        aah.hen(Integer.parseInt(getarg(line,1)));
                        aah.hen(Integer.parseInt(getarg(line,2)));
                        aah.hen(findinv(Integer.parseInt(getarg(line,3))));
                        aah.hff();
                        deb = -1;
                        }
                    } else if (cmd.equalsIgnoreCase("talktonpc"))
                    {
                        talktonpc(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("talktonpcvar"))
                    {
                        talktonpc(getvar(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("attacknpc"))
                    {
                        attacknpc(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("attacknpcvar"))
                    {
                       attacknpc(getvar(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("thievenpc"))
                    {
                        thievenpc(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("savescreenshot"))
                    {
 try
 {
 		
             BufferedImage image = new BufferedImage(512,347,BufferedImage.TYPE_INT_RGB);
 		Graphics g = image.createGraphics();
 		g.translate(-3,-1);
 		dki.cbc(g, 0, 0);
		BMP bmp = new osbaldeston.image.BMP(image);
 		bmp.write(new File(getarg(line,1) + ".bmp"));
 		}
 	catch(Exception exception){ adh("Error"); }
                    } else if (cmd.equalsIgnoreCase("thievenpcvar"))
                    {
                        thievenpc(getvar(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("magicnpc"))
                    {
                        magicnpc(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("magicnpcvar"))
                    {
                        magicnpc(getvar(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("opendoor"))
                    {
                        opendoor(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,3)));
                    } else if (cmd.equalsIgnoreCase("closedoor"))
                    {
                        closedoor(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,3)));
                    } else if (cmd.equalsIgnoreCase("deposit"))
                    {
                        deposit(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("withdraw"))
                    {
                        withdraw(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    ...
                    {
                        makechoice(Integer.parseInt(getarg(line,1)));
                        try
                        {
                            Thread.sleep(250);
                        }
                    ...
                    {
                        useitem(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("useitembyid"))
                    {
                        useitem(findinv(Integer.parseInt(getarg(line,1))));
                    } else if (cmd.equalsIgnoreCase("useitemwithitem"))
                    {
                        useitemwithitem(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("useitemwithitemvar"))
                    {
                        useitemwithitem(getvar(getarg(line,1)),getvar(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("dropitem"))
                    {
                        dropitem(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("dropitembyid"))
                    {
                        dropitem(findinv(Integer.parseInt(getarg(line,1))));
                    } else if (cmd.equalsIgnoreCase("dropallbyid"))
                    {
                        //dropitem(findinv(Integer.parseInt(getarg(line,1))));
                    } else if (cmd.equalsIgnoreCase("closebank"))
                    {
                        bank(false);
                    } else if (cmd.equalsIgnoreCase("closeshop"))
                    {
                        shop(false);
                    } else if (cmd.equalsIgnoreCase("takeitem"))
                    {
                        takeitem(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,3)));
                    } else if (cmd.equalsIgnoreCase("takeitemwait"))
                    {
                        if (itemat(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,3))) == true)
                        {
                            takeitem(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,3)));
                            boolean stillwaitinga = true;
                            int idler = 1;
                            while (stillwaitinga == true)
                            {
                                idler++;
                                if (idler == 10)
                                {
                                ...
                                if (itemat(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,3))) == false)
                                {
                                    stillwaitinga = false;
                                }
                                try
                    ...
                    } else if (cmd.equalsIgnoreCase("takeitemvarvar"))
                    {
                        takeitem(getvar(getarg(line,1)),getvar(getarg(line,2)),Integer.parseInt(getarg(line,3)));
                    } else if (cmd.equalsIgnoreCase("takeitemvarvar"))
                    {
                        takeitem(getvar(getarg(line,1)),getvar(getarg(line,2)),getvar(getarg(line,3)));
                    } else if (cmd.equalsIgnoreCase("sellitem"))
                    {
                        sellitem(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("buyitem"))
                    {
                        buyitem(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("magiconitem"))
                    {
                        magicitem(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("magiconitembyid"))
                    {
                        magicitem(findinv(Integer.parseInt(getarg(line,1))),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("setfire"))
                    {
                        setfire(getx(),gety());
                    } else if (cmd.equalsIgnoreCase("setfireatcoords"))
                    {
                        setfire(Integer.parseInt(getarg(line,1)),Integer.parseInt(getarg(line,2)));
                    } else if (cmd.equalsIgnoreCase("prayeroff"))
                    {
                        prayeroff(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("prayeron"))
                    {
                        prayeron(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("fightmode"))
                    {
                        if (getcombatstyle() != Integer.parseInt(getarg(line,1)))
                        {
                            setcombatstyle(Integer.parseInt(getarg(line,1)));
                        }
                    } else if (cmd.equalsIgnoreCase("waitforquestmenu"))
                    {
                        idle = 0;
                        stillwaiting = true;
                        while (stillwaiting == true)
                        {
                            ...
                            if (idle >= (Integer.parseInt(getarg(line,1)) * 10 ))
                            {
                                stillwaiting = false;
                            }
                            if (ischoice() == true)
                    ...
                    } else if (cmd.equalsIgnoreCase("waitforbankwindow"))
                    {
                    ...
                    } else if (cmd.equalsIgnoreCase("waitforload"))
                    {
                        stillwaiting = true;
                        while (stillwaiting == true)
                        {
                    ...
                    } else if (cmd.equalsIgnoreCase("waitforshopwindow"))
                    {
                        idle = 0;
                        stillwaiting = true;
                        while (stillwaiting == true)
                    ...
                    } else if (cmd.equalsIgnoreCase("waitforcoordchange"))
                    {
                        int oldx = getx();
                        int oldy = gety();
                        stillwaiting = true;
                    ...
                    } else if (cmd.equalsIgnoreCase("waitforcoordsat"))
                    {
                        int x = Integer.parseInt(getarg(line,1));
                        int y = Integer.parseInt(getarg(line,2));
                        stillwaiting = true;
                    ...
                    } else if (cmd.equalsIgnoreCase("repeatnextcommanduntilquestmenu"))
                    {
                        int timeout = Integer.parseInt(getarg(line,1));
                        stillwaiting = true;
                        idle = 0;
                        while (stillwaiting == true)
                        {
                            ...
                            {
                                parseline(Script[online + 1], true);
                                for (int x = 0; x < 100; x++ )
                                {
                                    try
                                    {
                        ...
                        online++;
                    } else if (cmd.equalsIgnoreCase("setsleepat"))
                    {
                        sleepat = Integer.parseInt(getarg(line,1));
                    } else if (cmd.equalsIgnoreCase("sleeping"))
                    {
                        if (getarg(line,1).equalsIgnoreCase("true"))
                        {
                            setsleeping = true;
                        } else {
                            setsleeping = false;
                        }
                    } else if (cmd.equalsIgnoreCase("gotoiflastitem"))
                    {
                        //prayeroff(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("gotoiflastitemlost"))
                    {
                        //setcombatstyle(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("gotoIffighting"))
                    {
                        if (playerincombat() == true)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifnpcexists"))
                    {

                        //if (npcexists(getvar(getarg(line,2))) == true)
                        //{
                        //    online = findlabel(getarg(line,1));
                        //}
                    } else if (cmd.equalsIgnoreCase("gotoifbagfull"))
                    {
                        if (invcount() == 30)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifininventory"))
                    {
                        if (findinv(Integer.parseInt(getarg(line,2))) != -1)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("setvaritemcount"))
                    {
                       setvar(getarg(line,1),String.valueOf(fec(Integer.parseInt(getarg(line,1)))));
                       ToDebug(String.valueOf(fec(Integer.parseInt(getarg(line,1)))));
                    } else if (cmd.equalsIgnoreCase("gotoifvarequal"))
                    {
                        if (getvar(getarg(line,2)) == getvar(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifsleeping"))
                    {
                        if (sleeping() == true)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarnotequal"))
                    {
                        if (getvar(getarg(line,2)) != getvar(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarabove"))
                    {
                        if (getvar(getarg(line,2)) > getvar(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarbelow"))
                    {
                        if (getvar(getarg(line,2)) < getvar(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarequalnum"))
                    {
                        if (getvar(getarg(line,2)) == Integer.parseInt(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarnotequalnum"))
                    {
                        if (getvar(getarg(line,2)) != Integer.parseInt(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarabovenum"))
                    {
                        if (getvar(getarg(line,2)) > Integer.parseInt(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarbelownum"))
                    {
                        if (getvar(getarg(line,2)) < Integer.parseInt(getarg(line,3)))
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarbetween"))
                    {
                        if (getvar(getarg(line,2)) > getvar(getarg(line,3)))
                        {
                            if (getvar(getarg(line,2)) < getvar(getarg(line,4)))
                            {
                                online = findlabel(getarg(line,1));
                            }
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifvarbetweennum"))
                    {
                        if (getvar(getarg(line,2)) > Integer.parseInt(getarg(line,3)))
                        {
                            if (getvar(getarg(line,2)) < Integer.parseInt(getarg(line,4)))
                            {
                                online = findlabel(getarg(line,1));
                            }
                        }
                    } else if (cmd.equalsIgnoreCase("resetlastservermessage"))
                    {
                        lastservermsg = null;
                    } else if (cmd.equalsIgnoreCase("gotoiflastservermessageis"))
                    {
                        if (getarg(line,2).substring(1,getarg(line,2).length()-1) == lastservermsg)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("setvarnum"))
                    {
                        setvar(getarg(line,1),getarg(line,2));
                    } else if (cmd.equalsIgnoreCase("setvar"))
                    {
                        setvar(getarg(line,1),String.valueOf(getvar(getarg(line,2))));
                    } else if (cmd.equalsIgnoreCase("setvarrandom"))
                    {
                        setvar(getarg(line,1),String.valueOf(getrandom(0,getvar(getarg(line,2)))));
                    } else if (cmd.equalsIgnoreCase("addvarnum"))
                    {
                        setvar(getarg(line,1),String.valueOf(getvar(getarg(line,1))+Integer.parseInt(getarg(line,2))));
                    } else if (cmd.equalsIgnoreCase("subvarnum"))
                    {
                        setvar(getarg(line,1),String.valueOf(getvar(getarg(line,1))-Integer.parseInt(getarg(line,2))));
                    } else if (cmd.equalsIgnoreCase("addvar"))
                    {
                        setvar(getarg(line,1),String.valueOf(getvar(getarg(line,1))+getvar(getarg(line,2))));
                    } else if (cmd.equalsIgnoreCase("subvar"))
                    {
                        setvar(getarg(line,1),String.valueOf(getvar(getarg(line,1))-getvar(getarg(line,2))));
                    } else if (cmd.equalsIgnoreCase("setvaritemcount"))
                    {
                        setvar(getarg(line,1),String.valueOf(invcount()));
                    } else if (cmd.equalsIgnoreCase("setvarmyx"))
                    {
                        setvar(getarg(line,1),String.valueOf(getx()));
                    } else if (cmd.equalsIgnoreCase("setvarmyy"))
                    {
                        setvar(getarg(line,1),String.valueOf(gety()));
                    } else if (cmd.equalsIgnoreCase("setvarmyhp"))
                    {
                        setvar(getarg(line,1),String.valueOf(playerhits()));
                    } else if (cmd.equalsIgnoreCase("gotoiflognofire"))
                    {
                        if (objat(getx(),gety()) == 97)
                        {
                             online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("gotoifcoordsin"))
                    {
                        if (getx() >= Integer.parseInt(getarg(line,2)))
                        {
                            if (gety() >= Integer.parseInt(getarg(line,3)))
                            {
                                if (getx() <= Integer.parseInt(getarg(line,4)))
                                {
                                    if (gety() <= Integer.parseInt(getarg(line,5)))
                                    {
                                        online = findlabel(getarg(line,1));
                                    }
                                }
                    ...
                    } else if (cmd.equalsIgnoreCase("gotoifcoordsinvars"))
                    {
                        if (getx() >= getvar(getarg(line,2)))
                        {
                            if (gety() >= getvar(getarg(line,3)))
                            {
                                if (getx() <= getvar(getarg(line,4)))
                                {
                                    if (gety() <= getvar(getarg(line,5)))
                                    {
                                        online = findlabel(getarg(line,1));
                                    }
                                }
                    ...
                    } else if (cmd.equalsIgnoreCase("gotoifnpcnear"))
                    {
                        if (findnpc(Integer.parseInt(getarg(line,2))) != -1)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("minenearestuntilore"))
                    {
                        int dir = 1;
                        int len = Integer.parseInt(getarg(line,2));
                        int x = getx();
                        int y = gety();
                        boolean stillore = true;
                        int obx = 0;
                        int oby = 0;
                        ...
                        for (int i = 1; i <= len; i++)
                        {
                            ...
                            {
                                x = x + dir;
                                if (objat(x,y) == Integer.parseInt(getarg(line,1)))
                                {
                                    obx = x;
                                    oby = y;
                                    j = i;
                        ...
                        while (stillore == true)
                        {
                            ...
                            if (objat(obx,oby) == Integer.parseInt(getarg(line,1)))
                            {
                                parseline("atobject(" + obx + "," + oby + ")",true);
                                try
                                {
                                    Thread.sleep(Integer.parseInt(getarg(line,3)) * 100);
                                }
                                catch (InterruptedException e) {}
                            } else {
                                stillore = false;
                    ...
                    } else if (cmd.equalsIgnoreCase("minenearestuntilorebetween"))
                    {
                        int dir = 1;
                        int len = Integer.parseInt(getarg(line,2));
                        int x = getx();
                        ...
                        for (int i = 1; i <= len; i++)
                        {
                            for (int j = 1; j <= i; j++ )
                            {
                                ...
                                if (objat(x,y) == Integer.parseInt(getarg(line,1)))
                                {
                                    if (x > Integer.parseInt(getarg(line,4)) && x < Integer.parseInt(getarg(line,6)) && y > Integer.parseInt(getarg(line,5)) && y < Integer.parseInt(getarg(line,7)))
                                    {
                                        obx = x;
                                        oby = y;
                                        j = i;
                    ...
                    } else if (cmd.equalsIgnoreCase("setvarsnearesttree"))
                    {
                        int dir = 1;
                        int len = Integer.parseInt(getarg(line,3));
                        int x = getx();
                        int y = gety();
                        boolean needtocheck = true;                        

                        if (objat(x,y) == 1 || objat(x,y) == 2)
                        {
                            setvar(getarg(line,1),String.valueOf(getx()));
                            setvar(getarg(line,2),String.valueOf(gety()));
                            needtocheck = false;
                        }
                        if (needtocheck == true)
                        {
                            for (int i = 1; i <= len; i++)
                            {
                                for (int j = 1; j <= i; j++ )
                                {
                                    ...
                                    if (objat(x,y) == 0)
                                    {
                                        setvar(getarg(line,1),String.valueOf(x));
                                        setvar(getarg(line,2),String.valueOf(y));
                                        j = i;
                                        i = len;
                                    }
                                    if (objat(x,y) == 1)
                    ...
                    } else if (cmd.equalsIgnoreCase("setvarsnearestobject"))
                    {
                        int dir = 1;
                        int len = Integer.parseInt(getarg(line,4));
                        int x = getx();
                        int y = gety();
                        boolean needtocheck = true;
                        ...
                        if (objat(x,y) == Integer.parseInt(getarg(line,3)))
                        {
                            setvar(getarg(line,1),String.valueOf(getx()));
                            setvar(getarg(line,2),String.valueOf(gety()));
                            needtocheck = false;
                    ...
                    } else if (cmd.equalsIgnoreCase("minenearest"))
                    {
                        int dir = 1;
                        int len = Integer.parseInt(getarg(line,2));
                        int x = getx();
                    ...
                    } else if (cmd.equalsIgnoreCase("setvarsnearestitem")) 
                    {
                        int coordy[] = new int[500];
		int coordx[] = new int[500];
		String differenceA[] = new String[500];
		String differenceB[] = new String[500];
		int tempS;
		int j = 0;

 		for(int i = 0; i < dgl.length; i++) 
 		{
 		   if(dgl[i] == Integer.parseInt(getarg(line,4))) {
                	      j++;
 		      coordx[j] = dgj[i];
 		      coordy[j] = dgk[i];
 		      
 		      tempS = (((coordx[j] + egj) - getx()) + ((coordy[j] + egk) - gety()));
 		      
 		      differenceA[j] = String.valueOf(tempS);
 		      differenceB[j] = String.valueOf(tempS);
 		      
 		      if(differenceB[j].startsWith("-")) {
 		           	 differenceA[j].substring(1,differenceA[j].length());
 		             differenceB[j].substring(1,differenceB[j].length());	     
 		      }
 		   }
 		}

 		   
 		Arrays.sort(differenceB);
 		   
 		for(int b = 0; b <= j; b++) {
 		    System.out.println(differenceB[b]);
 		}
                    } else if (cmd.equalsIgnoreCase("gotoiforeat"))
                    {
                        if (oreat(Integer.parseInt(getarg(line,2)),Integer.parseInt(getarg(line,3))) == true)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("setvarsplayer"))
                    {
                          int aaa = PIndexFromID(PIDFromName(lsta));
		  setvar(getarg(line,1), String.valueOf(dhe[aaa].had+egk)); 
		  setvar(getarg(line,2), String.valueOf(dhe[aaa].hae+egj));
		  setvar(getarg(line,3), String.valueOf(dhe[aaa].hab));
                    } else if (cmd.equalsIgnoreCase("attackplayer"))
                    {
                        attackplayer(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("tradeplayer"))
                    {
                        tradeplayer(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("followplayer"))
                    {
                        followplayer(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("duelplayer"))
                    {
                        duelplayer(Integer.parseInt(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("attackplayervar"))
                    {
                        attackplayer(getvar(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("tradeplayervar"))
                    {
                        tradeplayer(getvar(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("followplayervar"))
                    {
                        followplayer(getvar(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("duelplayervar"))
                    {
                        duelplayer(getvar(getarg(line,1)));
                    } else if (cmd.equalsIgnoreCase("gotoiforeatvar"))
                    {
                        if (oreat(getvar(getarg(line,2)),getvar(getarg(line,3))) == true)
                        {
                            online = findlabel(getarg(line,1));
                        }
                    } else if (cmd.equalsIgnoreCase("spawn"))
                    {
                        ...
                        dgj[egh] = eme;
                        dgk[egh] = emf;
                        dgl[egh] = Integer.parseInt(getarg(line,1));
                        dgm[egh] = 0;
                        egh++;
                        int dir = 1;
                        for (int i = 1; i <= 10; i++)
                    ...
                    } else if (cmd.equalsIgnoreCase("setvarsnearestnpc"))
                    {
                        int looking = Integer.parseInt(getarg(line,4));
		int sbm = GetID(SetIDNearest(looking));
                        
                        if(sbm != -1 && !npcincombat(sbm)) {
                            setvar(getarg(line,1), String.valueOf(eai[sbm].had));
                            setvar(getarg(line,2), String.valueOf(eai[sbm].hae));
                            setvar(getarg(line,3), String.valueOf(eai[sbm].hab));
                        } else {
                            try
                            {
                                Thread.sleep(50);
                    ...
                    } else if (cmd.equalsIgnoreCase("setvarsnearestnpcbetween"))
                    {
                        int looking = Integer.parseInt(getarg(line,4));
                        setvar(getarg(line,1), String.valueOf(findnearestnpcxbet(looking, Integer.parseInt(getarg(line,5)), Integer.parseInt(getarg(line,6)), Integer.parseInt(getarg(line,7)), Integer.parseInt(getarg(line,8)))));
                        setvar(getarg(line,2), String.valueOf(findnearestnpcybet(looking, Integer.parseInt(getarg(line,5)), Integer.parseInt(getarg(line,6)), Integer.parseInt(getarg(line,7)), Integer.parseInt(getarg(line,8)))));
                        setvar(getarg(line,3), String.valueOf(findnearestnpcbet(looking, Integer.parseInt(getarg(line,5)), Integer.parseInt(getarg(line,6)), Integer.parseInt(getarg(line,7)), Integer.parseInt(getarg(line,8)))));
                    }  else if (cmd.equalsIgnoreCase("debugrs"))
                    {
                        chat(getarg(line,1).substring(1,getarg(line,1).length()-1));
                    } else if (cmd.equalsIgnoreCase("omgtemp"))
                    {
                        chat("@GRE@You are too tired to gain some experience, get some rest!");
                    } else if (cmd.equalsIgnoreCase("showmessage"))
                    {
                        chat(getarg(line,1));
                    } else if (cmd.equalsIgnoreCase("debugrsvar"))
                    {
                        chat("@whi@+-- @gre@" + getarg(line,1) + "@whi@ = @gre@" + getvar(getarg(line,1)) + "@whi@ --+");
                    } else if (cmd.equalsIgnoreCase("debug"))
                    {
                        System.out.println(getarg(line,1).substring(1,getarg(line,1).length()-1));
                    } else if (cmd.equalsIgnoreCase("debugvar"))
                    {
                        System.out.println("+-- " + getarg(line,1) + " = " + getvar(getarg(line,1)) + " --+");
                    } else if (cmd.equalsIgnoreCase("setvarfatigue"))
                    {
                        setvar(getarg(line,1), String.valueOf(fatigue()));
                    } else if (cmd.equalsIgnoreCase("setvarhp"))
                    {
                        setvar(getarg(line,1), String.valueOf(hp()));
                    } else if (cmd.equalsIgnoreCase("setvarsrandomnpc"))
                    {
                        boolean stilllooking = true;
                        int checknpc = 0;
                        while (stilllooking == true)
                        {
                            checknpc = getrandom(0,npccount());
                            System.out.println("NPC: " + eai[checknpc].haf);
                            if (eai[checknpc].haf == Integer.parseInt(getarg(line,4)));
                            {
                                System.out.println(eai[checknpc].haf + " - " + checknpc + " = " + npccount());
                                setvar(getarg(line,1), String.valueOf(npcx(checknpc)));
                                setvar(getarg(line,2), String.valueOf(npcy(checknpc)));
                                setvar(getarg(line,3), String.valueOf(checknpc));
                                stilllooking = false;
                            }
                            try
                            {
    ...
    public static final void main(String args[])
    {
        ...
        mudclient1.emh = true;
        mudclient1.fme(mudclient1.dmj, mudclient1.dmk + 11, "SbmRune v0.1 BETA", false);
        mudclient1.flf = 10;
    }
    ...
    public void takeitem(int x, int y, int itemid)
    {
        x = x - egj;
        y = y - egk;
        ...
        for(int i = 0; i < dgl.length; i++)
        {
            if (dgj[i] == x && dgk[i] ==  y && dgl[i] == itemid)
            {
                fbc(eme, emf, dgj[i], dgk[i], true);
                super.aah.hfj(26);
                super.aah.hen(dgj[i] + egj);
                super.aah.hen(dgk[i] + egk);
                super.aah.hen(dgl[i]);
                super.aah.hen(deb);
                super.aah.hff();
           }
    ...
    public boolean itemat(int itemid, int x, int y)
    {
        x = x - egj;
        y = y - egk;
        for (int i = 0; i < dgl.length; i++ )
    ...
    public void magicitem(int slot, int spell)
    {
        super.aah.hfj(78);
        super.aah.hen(slot);
        super.aah.hen(spell);
    ...
    private final void SwitchTo(int w)
    {
         sWorld = w;
        ...
        System.out.println("Selecting " + sName);
    }
    ...
    public void bank(boolean show)
    {
        dnk = show;
    }

    public void shop(boolean show)
    {
        eha = show;
    }
    ...
    {
        for (int bbb = 0; bbb < ekd; bbb++ )
        {
            if (dhe[bbb].hab == i)
            {    
                fdl(eme, emf, (dhe[bbb].had - 64) / ehb, (dhe[bbb].hae - 64) / ehb, true);
                super.aah.hfj(232);
                super.aah.hen(i);
                super.aah.hff();
            }
    ...
    public void talktonpc(int npcid)
    {
        for (int x = 0; x < npccount(); x++ )
        {
            if (eai[x].hab == npcid)
            {    
                fdl(eme, emf, (eai[x].had - 64) / ehb, (eai[x].hae - 64) / ehb, true);
                aah.hfj(27);
                aah.hen(eai[x].hab);
                aah.hff();
            }
    ...
    {
        chat("@ora@RevolutionBot: @whi@A macro is already running!");
    }
    ...
    public void e(int l)
    {
        ...
        //System.out.println("var1 : " + lh[l]);
        //System.out.println("var2 : " + mh[l]);
        //System.out.println("var3 : " + Eg[l]);
        //System.out.println("var4 : " + Fg[l]);
        //System.out.println("var5 : " + Gg[l]);
        //System.out.println("var6 : " + ed[l]);
        //output(l);
        fem(l);
    }
	...
	public int SetIDNearest (int ID) {
		int npcX[] = new int[npccount()];
		int npcY[] = new int[npccount()];
		int npcID[] = new int[npccount()];
		int count = 0;
		int farX = 0;
		int farY = 0;
		double far = 9999999;
		int returnValue = -1;
         	for(int y = 0; y < npccount(); y++) {
             		if(eai[y].haf == ID) {
             			if(eai[y].hah != 8 && eai[y].hah != 9) {
           				npcX[count] = (eai[y].had - 64) / ehb;
           				npcY[count] = (eai[y].hae - 64) / ehb;
           				npcID[count] = y;
           				count += 1;
             			}
             		}
         	...
         	try
         	{
         		for(int y2 = 0; y2 <= count; y2++) {
         			if(eai[npcID[y2]].haf == ID) {
         				farX = Math.abs(Math.abs(eme) - Math.abs(npcX[y2]));
         				farY = Math.abs(Math.abs(emf) - Math.abs(npcY[y2]));
         				if(farX == 0 || farY == 0) { 	
         					if(farX + farY < far) {
         						far = farX + farY;
         						returnValue = npcID[y2];
	         				}
	         			} else {
	         				if((farX + farY) * 0.5 < far) {
         						far = (farX + farY) * 0.5;
         						returnValue = npcID[y2];
	         				}
	         			}
         	...
         	if(returnValue > -1 && returnValue < npccount())
         		return returnValue;
         	else
         		return -1;
     	}

    public int findtype(int UID)
    {
        for (int x = 0; x < emi; x++ )
        {
            if (UID == eai[x].hab)
    ...
    public void useitemwithitem(int slot, int slot2)
    {
    	super.aah.hfj(225);
    	super.aah.hen(slot);
        super.aah.hen(slot2);
    ...
    {
        for (int sbm = 0; sbm < npccount(); sbm++ )
        {
            if (eai[sbm].hab == npcid)
            {    
                fdl(eme, emf, (eai[sbm].had - 64) / ehb, (eai[sbm].hae - 64) / ehb, true);
                aah.hfj(201);
                aah.hen(eai[sbm].hab);
                aah.hff();
            }
    ...
    public void dropitem(int slot)
    {
        ...
        deb = -1;
        eeb = 0;
        chat("@whi@Dropping " + ahd.alm[dka[slot]]);
    }
    ...
    public int getnpcbyname(String name)
    {
        for (int x = 0; x < emi ; x++ )
        {
            if (eai[x].haa.equalsIgnoreCase(name))
            {
                return x;
            }
        }
    ...
    public int objat(int x, int y)
    {
        for (int i = 0; i < 1500; i++ )
        {
            if (ecb[i] + egj == x && ecc[i] + egk == y)
            {
                return ecd[i];
            }
        }
    ...
    public boolean oreat(int x, int y)
    {
        if(itemat(150,x,y)) {
            return true;
        } else if(itemat(151,x,y)) {
            return true;
        } else if(itemat(152,x,y)) {
            return true;
        } else if(itemat(153,x,y)) {
            return true;
        } else if(itemat(154,x,y)) {
            return true;
        } else if(itemat(155,x,y)) {
            return true;
        } else if(itemat(149,x,y)) {
            return true;
        } else if(itemat(202,x,y)) {
            return true;
        } else if(itemat(409,x,y)) {
            return true;
        } else {
           return false;
        }
	...
	public int GetNPCByIDNearest (int ID) {
		int npcX[] = new int[npccount()];
		int npcY[] = new int[npccount()];
		int npcID[] = new int[npccount()];
		int count = 0;
    ...
    public boolean between(int x, int x1, int x2)
    {
        if (x >= x1 && x <= x2)
        {
            return true;
    ...
    public int findnearestnpcxbet(int npcid, int fx, int fy, int fx2, int fy2)
    {
        int dir = 1;
        int len = 50;
        int x = getx();
        ...
        for (int i = 1; i <= len; i++)
        {
            ...
            {
                x = x + dir;
                if (between(x,fx,fx2) && between(y,fy,fy2))
                {
                    if (npcat(npcid,x,y) == true)
                    {
                        return x;
    ...
    public int npcx(int npcid)
    {
        return (eai[npcid].had - 64) / ehb;
    }

    public int npcy(int npcid)
    {
        return (eai[npcid].hae - 64) / ehb;
    }

    public boolean npcat(int npcid, int x, int y)
    {
    ...
    public int findnearestnpc(int npcid)
    {
        for (int x = 0; x < npccount(); x++ )
        {
            if (eai[x].haf == npcid)
            {
                if (findnearestnpcx(npcid) == (npcx(x) + eme))
                {
                    if (findnearestnpcy(npcid) == (npcy(x) + emf))
                    {
                        return dhe[x].haf;
                    }
                }
    ...
    public int findnearestnpcbet(int npcid, int fx, int fy, int fx2, int fy2)
    {
        for (int x = 0; x < npccount(); x++ )
        {
            if (eai[x].hab == npcid)
            {
                if (findnearestnpcx(npcid) == (npcx(x) + eme))
                {
                    if (findnearestnpcy(npcid) == (npcy(x) + emf))
                    {
                        if (between(npcx(x) + eme,fx,fx2) && between(npcy(x) + emf,fy,fy2))
                        {
                            return dhe[x].hab;
                        }
                    }
    ...
    public int findnearestnpcybet(int npcid, int fx, int fy, int fx2, int fy2)
    {
        int dir = 1;
        int len = 50;
        int x = getx();
    ...
    public boolean npcincombat(int j)
    {
        return eai[j].hah == 8 || eai[j].hah == 9;
    }
    ...
    public void Output(int l, int i)
    {
        int X = eec[i] + egj;
        int Y = eed[i] + egk;
        int ID = dfd[i];
        ...
        if (l == 420)
        {
            ToDebug("AtObject(" + X + "," + Y + ")");
        } else if (l == 2400)
        {
            ToDebug("AtObject2(" + X + "," + Y + ")");
        } else if (l == 3700)
        {
            ToDebug("(" + (ID) +  ")");
        } else if (l == 3600)
        ...
        {
            ToDebug("TalkToNPC(" + (ID) + ") - TYPE " + FindType(ID));
        } else if (l == 920)
        {
            ToDebug("MoveTo(" + X + "," + Y + ")");
        } else if (l == 715 || l == 2715)
        {
            ToDebug("AttackNPC(" + (ID) + ") - TYPE " + FindType(ID));
        } else if (l == 725)
        {
            ToDebug("ThieveNPC(" + (ID) + ") - TYPE " + FindType(ID));
        } else if (l == 320)
        {
            ToDebug("OpenDoor("  + X + "," + Y + "," + (ID) + ")");
        } else if (l == 2300)
        {
            ToDebug("CloseDoor("  + X + "," + Y + "," + (ID) + ")");
        } else if (l == 410)
        {
            ToDebug("AtObjectWithItem("  + X + "," + Y + "," + (ID) + ")");
        } else if (l == 220)
        {
           ToDebug("TakeItem("  + X + "," + Y + "," + (ID) + ")");
        } else if (l == 660)
        {
            ToDebug("DropItem(" + (ID) + ")");
        } else if (l == 2806)
        {
            ToDebug("DuelPlayer(" + (ID) + ")");
        } else if (l == 2810)
        {
            ToDebug("TradePlayer(" + (ID) + ")");
        } else if (l == 805 || l == 2805)
        {
            ToDebug("AttackPlayer(" + (ID) + ")");
        } else if (l == 2820)
        {
            ToDebug("FollowPlayer(" + (ID) + ")");
        }
    }
    ...
    private final void enj()
    {
        ...
        dki.cee("Click here to cancel", 256, 318, 1, k);
     	...
     	private int PIDFromName(String name)
		{
		for (int cx = 0; cx < ekd; cx++)
		{
			if (dhe[cx].haa != null)
			{
				if (dhe[cx].haa.equalsIgnoreCase(name))
				{
					return dhe[cx].hab;
				}
			}
	...
	private int PIndexFromID(int id)
	{
		for (int cx = 0; cx < ekd; cx++)
		{
			if (dhe[cx].hab == id)
    ...
    private final void fac(boolean flag)
    {
        ...
        if(dfj == 1)
        {
            ...
            if(j3 != -1)
            {
                ...
                dki.cee("Drain rate: " + ahd.alk[j3], i + 98, 196, 1, 0);
            } else
            {
...
label0:
    ...
    private final void fcb(boolean flag)
    {
        ...
        if(i >= 40 && i8 >= 0 && i < 196 && i8 < 152)
        {
            ...
            if(elf == 1) {
                ...
                System.out.println("MoveToMap(" + (l2 / 128) + "," + (j4 / 128) +")");
             }   
            elf = 0;
        }
    }
    ...
    private final void fcd()
    {
        ...
        {
            eeb = 2;
            dhj = (int)(Math.random() * 0D) - 0;
            dhk = (int)(Math.random() * 0D) - 0;
        }
        ...
        if(ekl.gfh(ekn))
        {
            ...
            if(s.startsWith("/"))
            {
                if(s.startsWith("/spawn "))
                {
                    ...
                    dgj[egh] = eme;
                    dgk[egh] = emf;
                    dgl[egh] = Integer.parseInt(s.substring(7,s.length()));
                    dgm[egh] = 0;
                    egh++;
                    int dir = 1;
                    for (int i = 1; i <= 10; i++)
    ...
    private final void fcn(boolean flag)
    {
        ...
        i1 += 15;
        dki.ceg("To change your contact details,", k, 111, 0, 0xffffff);
        i1 += 15;
        dki.ceg("password, recovery questions, etc..", k, 126, 0, 0xffffff);
        i1 += 15;
        dki.ceg("please select 'account management'", k, 141, 0, 0xffffff);
        i1 += 15;
        if(ega == 0)
            dki.ceg("from the runescape.com front page", k, 156, 0, 0xffffff);
        else
        if(ega == 1)
            dki.ceg("from the link below the gamewindow", k, 156, 0, 0xffffff);
        else
            dki.ceg("from the runescape front webpage", k, 156, 0, 0xffffff);
        i1 += 15;
        i1 += 5;
        dki.ceg("Privacy settings. Will be applied to", i + 3, 176, 1, 0);
        i1 += 15;
        dki.ceg("all people not on your friends list", i + 3, 191, 1, 0);
        i1 += 15;
        if(super.abc == 0)
        ...
        if(super.flg > k && super.flg < k + 196 && super.flh > 274 && super.flh < 290)
            k1 = 0xffff00;
        dki.ceg("Click here to logout", i + 3, 286, 1, k1);
        if(!flag)
            return;
        i = super.flg - (((blm) (dki)).bln - 199);
        j = super.flh - 36;
        if(i >= 0 && j >= 0 && i < 196 && j < 265)
        {
            ...
            int l = l1 + 3;
            int j1 = 66;
            if(super.flg > l && super.flg < l + 196 && super.flh > 54 && super.flh < 70 && elf == 1)
            {
                ekc = !ekc;
                super.aah.hfj(8);
                super.aah.hfa(0);
            ...
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 69 && super.flh < 85 && elf == 1)
            {
                eah = !eah;
                super.aah.hfj(8);
                super.aah.hfa(2);
            ...
            j1 += 15;
            if(emh && super.flg > l && super.flg < l + 196 && super.flh > 84 && super.flh < 100 && elf == 1)
            {
                ddn = !ddn;
                super.aah.hfj(8);
                super.aah.hfa(3);
            ...
            boolean flag1 = false;
            j1 += 35;
            if(super.flg > l && super.flg < l + 196 && super.flh > 194 && super.flh < 210 && elf == 1)
            {
                super.abc = 1 - super.abc;
                flag1 = true;
            }
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 209 && super.flh < 225 && elf == 1)
            {
                super.abd = 1 - super.abd;
                flag1 = true;
            }
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 224 && super.flh < 240 && elf == 1)
            {
                super.abe = 1 - super.abe;
                flag1 = true;
            }
            j1 += 15;
            if(emh && super.flg > l && super.flg < l + 196 && super.flh > 239 && super.flh < 255 && elf == 1)
            {
            ...
            j1 += 20;
            if(super.flg > l && super.flg < l + 196 && super.flh > 274 && super.flh < 290 && elf == 1)
                fcg();
            elf = 0;
        }
    }
    ...
    private final void fdn()
    {
        ...
        {
            dki.cbm();
            dki.cee("Oh dear! You are dead...", dmj / 2, dmk / 2, 7, 0xff0000);
            fdk();
            dki.cbc(eff, 0, 0);
            return;
        }
...
if(edi)
{
ToDebug("Sleepword written to: HC.BMP");
 ...
 try
 {
 		...
 		dki.cbc(g, 0, 0);
		BMP bmp = new osbaldeston.image.BMP(image);
 		bmp.write(new File("hc" + ".bmp"));
 		}
 	catch(Exception exception){ adh("Error"); 
 	}
dki.cbm();
if(Math.random() < 0.14999999999999999D)
dki.cee("Sbm", (int)(Math.random() * 80D), (int)(Math.random() * 334D), 5, (int)(Math.random() * 16777215D));
if(Math.random() < 0.14999999999999999D)
dki.cee("Rune", 512 - (int)(Math.random() * 80D), (int)(Math.random() * 334D), 5, (int)(Math.random() * 16777215D));
...
return;
        ...
        int j5 = eln + ejd;
        int j7 = ema + dma;
        dkm.bje(j5, -efh.gme(j5, j7), j7, 912, ean * 4, 0, edb * 2);
        dkm.big();
        fab();
        if(ehd > 0)
        ...
        if(eee != 0)
        {
            ...
            k5 %= 60;
            if(k5 < 10)
                dki.cee("System update in: " + k7 + ":0" + k5, 256, dmk - 7, 1, 0xffff00);
            else
                dki.cee("System update in: " + k7 + ":" + k5, 256, dmk - 7, 1, 0xffff00);
        }
        if(!dih)
        {
            ...
            if(l5 > 0)
            {
                ...
                dki.cee("Level: " + l7, 465, dmk - 7, 1, 0xffff00);
                if(efi == 0)
                    efi = 2;
            }
            if(efi == 0 && l5 > -10 && l5 <= 0)
    ...
    private final void fej()
    {
        ...
        if(!emh)
        {
            elj.ggj(256, 240, "Click on an option", 5, true);
            elj.ggk(156, 280, 120, 35);
            elj.ggk(356, 280, 120, 35);
            elj.ggj(156, 280, "New User", 5, false);
            elj.ggj(356, 280, "Existing User", 5, false);
            dmg = elj.ghd(156, 280, 120, 35);
            dmh = elj.ghd(356, 280, 120, 35);
        } else
        {
            elj.ggj(256, 240, "SbmRune v0.1", 5, true); 
