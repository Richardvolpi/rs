public class uncertironbar extends Methods
{
    public uncertironbar(mudclient mc){super(mc);}
    private long l;

    public void MainBody(String as[])
    {
	AutoLogin(true);
	  Display("@mag@Draynor Iron Bar Uncerter script -- @gre@Nattawat");
	  Display("@whi@Make sure you already have certs in your bag and no bars!");
        while(Running())
        {
	 uncert();
	 Wait(200);
	...
	while (!InBank() && CountInv(170) > 0)
	  banking();
	while (InBank() && CountInv(170) > 0)
	{
	    while (CountInv(170) > 0 && InBank())
	    {
	      Deposit(170,1);
              Wait(Rand(200,500));
            }
            while (CountInv(528) < 20 && InBank())
            {
              Withdraw(528,5);
              Wait(Rand(1500,2000));
            }
