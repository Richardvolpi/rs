...
public class GameModel {
    ...
    public GameModel(int numVertices, int numFaces) {
        ...
        anIntArrayArray279 = new int[numFaces][1];
        for (int v = 0; v < numFaces; v++)
            anIntArrayArray279[v][0] = v;

    }

    public GameModel(int numVertices, int numFaces, boolean autocommit, boolean isolated, boolean unlit, boolean pickable, boolean projected) {
        transformState = 1;
        visible = true;
        unknownSceneBoolean = false;
        transparent = false;
    ...
    private void allocate(int numV, int numF) {
        ...
        faceCameraNormalScale = new int[numF];
        faceCameraNormalMagnitude = new int[numF];
        if (!projected) {
            vertexCameraX = new int[numV];
            vertexCameraY = new int[numV];
            vertexCameraZ = new int[numV];
            vertexViewX = new int[numV];
            vertexViewY = new int[numV];
        }
        if (!pickable) {
            faceSpriteType = new byte[numF];
    ...
    public void projectionPrepare() {
        vertexCameraX = new int[numVertices];
        vertexCameraY = new int[numVertices];
        vertexCameraZ = new int[numVertices];
    ...
    public void merge(GameModel pieces[], int count, boolean transState) {
        ...
        allocate(numV, numF);
        if (transState)
            anIntArrayArray279 = new int[numF][];
        for (int i = 0; i < count; i++) {
            ...
            for (int srcF = 0; srcF < source.numFaces; srcF++) {
                ...
                faceCameraNormalScale[dstF] = source.faceCameraNormalScale[srcF];
                faceCameraNormalMagnitude[dstF] = source.faceCameraNormalMagnitude[srcF];
                if (transState)
                    if (count > 1) {
                        anIntArrayArray279[dstF] = new int[source.anIntArrayArray279[srcF].length + 1];
                        anIntArrayArray279[dstF][0] = i;
                        for (int i2 = 0; i2 < source.anIntArrayArray279[srcF].length; i2++)
                            anIntArrayArray279[dstF][i2 + 1] = source.anIntArrayArray279[srcF][i2];

                    } else {
                        anIntArrayArray279[dstF] = new int[source.anIntArrayArray279[srcF].length];
                        for (int j2 = 0; j2 < source.anIntArrayArray279[srcF].length; j2++)
                            anIntArrayArray279[dstF][j2] = source.anIntArrayArray279[srcF][j2];
    ...
    public void copyLighting(GameModel model, int srcVs[], int nV, int inF) {
        ...
        model.faceCameraNormalScale[outF] = faceCameraNormalScale[inF];
        model.faceCameraNormalMagnitude[outF] = faceCameraNormalMagnitude[inF];
    }
    ...
    public void project(int cameraX, int cameraY, int cameraZ, int cameraPitch, int cameraRoll, int cameraYaw, int viewDist, int clipNear) {
        apply();
        if (z1 > Scene.frustumFar || z2 < Scene.frustumNear || x1 > Scene.frustumRight || x2 < Scene.frustumLeft || y1 > Scene.frustumTop || y2 < Scene.frustumBottom) {
            visible = false;
            return;
        }
        visible = true;
    ...
    public int z1;
    public int z2;
    public boolean unknownSceneBoolean;
