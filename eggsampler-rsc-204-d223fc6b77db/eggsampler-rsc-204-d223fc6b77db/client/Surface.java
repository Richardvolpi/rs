...
public class Surface
    ...
    public void drawCircle(int x, int y, int size, int colour, int alpha) {
        ...
        for (int k3 = i3; k3 <= j3; k3 += vertInc) {
            int l3 = k3 - y;
    ...
    public void clear() {
        for (int i = 0; i < surfaceSetPixels.length; i++) {
            surfaceSetPixels[i] = null;
            spriteWidth[i] = 0;
            spriteHeight[i] = 0;
            spriteColoursUsed[i] = null;
            spriteColours[i] = null;
        }

    }

    public void loadSprite(int spriteId, byte spriteData[], byte indexData[], int frameCount) {
    ...
    public void readSleepWord(int spriteId, byte spriteData[]) {
        int pixels[] = surfaceSetPixels[spriteId] = new int[10200];
    ...
    public void preloadSprite(int spriteId) {// ???
        ...
        surfaceSetPixels[spriteId] = ai1;
        spriteColoursUsed[spriteId] = null;
        spriteColours[spriteId] = null;
    }

    public void drawSprite(int sprite, int x, int y, int w, int h) {// same as the one below and wat is this
        spriteWidth[sprite] = w;
        spriteHeight[sprite] = h;
        spriteTranslate[sprite] = false;
        spriteTranslateX[sprite] = 0;
        spriteTranslateY[sprite] = 0;
        spriteWidthFull[sprite] = w;
        spriteHeightFull[sprite] = h;
        int area = w * h;
        int pixel = 0;
        surfaceSetPixels[sprite] = new int[area];
        for (int xx = x; xx < x + w; xx++) {
            for (int yy = y; yy < y + h; yy++)
                surfaceSetPixels[sprite][pixel++] = pixels[xx + yy * width2];

        }

    }

    public void drawSprite2(int sprite, int x, int y, int w, int h) {
        spriteWidth[sprite] = w;
        spriteHeight[sprite] = h;
        spriteTranslate[sprite] = false;
        spriteTranslateX[sprite] = 0;
    ...
    public void drawSprite(int x, int y, int id) {
        ...
        if (surfaceSetPixels[id] == null) {
            drawSprite(pixels, spriteColoursUsed[id], spriteColours[id], rX, rY, width, height, w2, h2, inc);
            return;
        } else {
            drawSprite(pixels, surfaceSetPixels[id], 0, rX, rY, width, height, w2, h2, inc);
            return;
        }
    }
    ...
    public void spriteClipping(int i, int j, int k, int l, int i1) {
        try {
            int j1 = spriteWidth[i1];
            int k1 = spriteHeight[i1];
            int l1 = 0;
            int i2 = 0;
            int j2 = (j1 << 16) / k;
            int k2 = (k1 << 16) / l;
            if (spriteTranslate[i1]) {
                int l2 = spriteWidthFull[i1];
                int j3 = spriteHeightFull[i1];
                j2 = (l2 << 16) / k;
                k2 = (j3 << 16) / l;
                i += ((spriteTranslateX[i1] * k + l2) - 1) / l2;
                j += ((spriteTranslateY[i1] * l + j3) - 1) / j3;
                if ((spriteTranslateX[i1] * k) % l2 != 0)
                    l1 = (l2 - (spriteTranslateX[i1] * k) % l2 << 16) / k;
                if ((spriteTranslateY[i1] * l) % j3 != 0)
                    i2 = (j3 - (spriteTranslateY[i1] * l) % j3 << 16) / l;
                k = (k * (spriteWidth[i1] - (l1 >> 16))) / l2;
                l = (l * (spriteHeight[i1] - (i2 >> 16))) / j3;
            }
            int i3 = i + j * width2;
            int k3 = width2 - k;
            if (j < boundsTopY) {
            ...
            plotScale(pixels, surfaceSetPixels[i1], 0, l1, i2, i3, k3, k, l, j2, k2, j1, byte0);
            return;
        } catch (Exception Ex) {
    ...
    public void method232(int x, int y, int idx, int some) {
        if (spriteTranslate[idx]) {
            x += spriteTranslateX[idx];
            y += spriteTranslateY[idx];
        }
        int size = x + y * width2;
        int j1 = 0;
        int height = spriteHeight[idx];
        int width = spriteWidth[idx];
        ...
        if (surfaceSetPixels[idx] == null) {
            method239(pixels, spriteColoursUsed[idx], spriteColours[idx], j1, size, width, height, extraXSpace, j2, byte0, some);
            return;
        } else {
            method238(pixels, surfaceSetPixels[idx], 0, j1, size, width, height, extraXSpace, j2, byte0, some);
            return;
        }
    }
    ...
    public void drawActionBubble(int x, int y, int scaleX, int scaleY, int sprite, int alpha) {
        try {
            int k1 = spriteWidth[sprite];
            int l1 = spriteHeight[sprite];
            int i2 = 0;
            int j2 = 0;
            int k2 = (k1 << 16) / scaleX;
            int l2 = (l1 << 16) / scaleY;
            if (spriteTranslate[sprite]) {
                ...
                scaleX = (scaleX * (spriteWidth[sprite] - (i2 >> 16))) / i3;
                scaleY = (scaleY * (spriteHeight[sprite] - (j2 >> 16))) / k3;
            }
            int j3 = x + y * width2;
            ...
            tranScale(pixels, surfaceSetPixels[sprite], 0, i2, j2, j3, l3, scaleX, scaleY, k2, l2, k1, byte0, alpha);
            return;
        } catch (Exception Ex) {
            System.out.println("error in sprite clipping routine");
        }
    ...
    public void spriteClipping(int i, int j, int k, int l, int i1, int j1) {
        try {
            ...
            int l2 = (l1 << 16) / l;
            if (spriteTranslate[i1]) {
                int i3 = spriteWidthFull[i1];
                int k3 = spriteHeightFull[i1];
                k2 = (i3 << 16) / k;
                l2 = (k3 << 16) / l;
                i += ((spriteTranslateX[i1] * k + i3) - 1) / i3;
                j += ((spriteTranslateY[i1] * l + k3) - 1) / k3;
                if ((spriteTranslateX[i1] * k) % i3 != 0)
                    i2 = (i3 - (spriteTranslateX[i1] * k) % i3 << 16) / k;
                if ((spriteTranslateY[i1] * l) % k3 != 0)
                    j2 = (k3 - (spriteTranslateY[i1] * l) % k3 << 16) / l;
                k = (k * (spriteWidth[i1] - (i2 >> 16))) / i3;
                l = (l * (spriteHeight[i1] - (j2 >> 16))) / k3;
            }
            int j3 = i + j * width2;
            int l3 = width2 - k;
            if (j < boundsTopY) {
            ...
            plotScale(pixels, surfaceSetPixels[i1], 0, i2, j2, j3, l3, k, l, k2, l2, k1, byte0, j1);
            return;
        } catch (Exception Ex) {
            System.out.println("error in sprite clipping routine");
        }
    ...
    private void drawSprite(int ai[], int ai1[], int i, int j, int k, int l, int i1,
    ...
    private void drawSprite(int pix[], byte colourIdx[], int colours[], int rX, int rY, int width, int height,
                             int w2, int h2, int rowInc) {
        int l1 = -(width >> 2);
        width = -(width & 3);
        for (int i2 = -height; i2 < 0; i2 += rowInc) {
            for (int j2 = l1; j2 < 0; j2++) {
                byte byte0 = colourIdx[rX++];
                if (byte0 != 0)
                    pix[rY++] = colours[byte0 & 0xff];
                else
                    rY++;
                byte0 = colourIdx[rX++];
                if (byte0 != 0)
            ...
            for (int k2 = width; k2 < 0; k2++) {
                byte byte1 = colourIdx[rX++];
                if (byte1 != 0)
                    pix[rY++] = colours[byte1 & 0xff];
                else
                    rY++;
            }
    ...
    private void plotScale(int ai[], int ai1[], int i, int j, int k, int l, int i1,
    ...
    public void drawMinimapSprite(int x, int y, int sprite, int rotation, int scale) {// "scale" is not actually scaling when it comes to the landscape
        ...
        int l10 = k6 * j1;
        int ai[] = surfaceSetPixels[sprite];
        for (int i11 = k6; i11 < l6; i11++) {
            ...
            if (k11 - j11 <= 0) {
                ...
                if (!interlace || (i11 & 1) == 0)
                    if (!spriteTranslate[sprite])
                        drawMinimapSomething(pixels, ai, 0, l10 + j11, l11, j12, i12, k12, j11 - k11, j9);
                    else
                        drawMinimapSomethingTranslate(pixels, ai, 0, l10 + j11, l11, j12, i12, k12, j11 - k11, j9);
                l10 += j1;
            }
        }
    ...
    private void drawMinimapSomething(int ai[], int ai1[], int i, int j, int k, int l, int i1,
                                        int j1, int k1, int l1) {
        for (i = k1; i < 0; i++) {
    ...
    private void drawMinimapSomethingTranslate(int ai[], int ai1[], int i, int j, int k, int l, int i1,
                                                  int j1, int k1, int l1) {
    ...
    public void spriteClipping(int i, int j, int k, int l, int sprite, int j1, int k1,
                                int l1, boolean flag) {
        try {
            if (j1 == 0)
                j1 = 0xffffff;
            ...
            if (spriteTranslate[sprite]) {
                ...
                i += ((j5 * k + fullWidth) - 1) / fullWidth;
                int l5 = ((k5 * l + fullHeight) - 1) / fullHeight;
                j += l5;
                i3 += l5 * l3;
                if ((j5 * k) % fullWidth != 0)
                    k2 = (fullWidth - (j5 * k) % fullWidth << 16) / k;
                if ((k5 * l) % fullHeight != 0)
                    l2 = (fullHeight - (k5 * l) % fullHeight << 16) / l;
                k = ((((spriteWidth[sprite] << 16) - k2) + j3) - 1) / j3;
                l = ((((spriteHeight[sprite] << 16) - l2) + k3) - 1) / k3;
            }
            int j4 = j * width2;
            i3 += i << 16;
            if (j < boundsTopY) {
            ...
            if (!interlace)
                i5 = 2;
            if (k1 == 0xffffff) {
                if (surfaceSetPixels[sprite] != null)
                    if (!flag) {
                        transparentSpritePlot(pixels, surfaceSetPixels[sprite], 0, k2, l2, j4, k, l, j3, k3, width, j1, i3, l3, i5);
                        return;
                    } else {
                        transparentSpritePlot(pixels, surfaceSetPixels[sprite], 0, (spriteWidth[sprite] << 16) - k2 - 1, l2, j4, k, l, -j3, k3, width, j1, i3, l3, i5);
                        return;
                    }
                if (!flag) {
                    transparentSpritePlot(pixels, spriteColoursUsed[sprite], spriteColours[sprite], 0, k2, l2, j4, k, l, j3, k3, width, j1, i3, l3, i5);
                    return;
                } else {
                    transparentSpritePlot(pixels, spriteColoursUsed[sprite], spriteColours[sprite], 0, (spriteWidth[sprite] << 16) - k2 - 1, l2, j4, k, l, -j3, k3, width, j1, i3, l3, i5);
                    return;
                }
            }
            if (surfaceSetPixels[sprite] != null)
                if (!flag) {
                    transparentSpritePlot(pixels, surfaceSetPixels[sprite], 0, k2, l2, j4, k, l, j3, k3, width, j1, k1, i3, l3, i5);
                    return;
                } else {
                    transparentSpritePlot(pixels, surfaceSetPixels[sprite], 0, (spriteWidth[sprite] << 16) - k2 - 1, l2, j4, k, l, -j3, k3, width, j1, k1, i3, l3, i5);
                    return;
                }
            if (!flag) {
                transparentSpritePlot(pixels, spriteColoursUsed[sprite], spriteColours[sprite], 0, k2, l2, j4, k, l, j3, k3, width, j1, k1, i3, l3, i5);
                return;
            } else {
                transparentSpritePlot(pixels, spriteColoursUsed[sprite], spriteColours[sprite], 0, (spriteWidth[sprite] << 16) - k2 - 1, l2, j4, k, l, -j3, k3, width, j1, k1, i3, l3, i5);
                return;
            }
        } catch (Exception Ex) {
            System.out.println("error in sprite clipping routine");
    ...
    private void transparentSpritePlot(int pix[], int ai1[], int i, int j, int k, int l, int i1,
                                         int j1, int k1, int l1, int i2, int j2, int k2, int l2,
                                         int i3) {
        int i4 = j2 >> 16 & 0xff;
        int j4 = j2 >> 8 & 0xff;
                ...
                if (i3 != 0) {
                    for (int k6 = k5; k6 < k5 + l5; k6++) {
                        ...
                        if (i != 0) {
                            ...
                            int l3 = i & 0xff;
                            if (j3 == k3 && k3 == l3)
                                pix[k6 + l] = ((j3 * i4 >> 8) << 16) + ((k3 * j4 >> 8) << 8) + (l3 * k4 >> 8);
                            else
                                pix[k6 + l] = i;
                        }
                        j += k1;
    ...
    private void transparentSpritePlot(int ai[], int ai1[], int i, int j, int k, int l, int i1,
                                         int j1, int k1, int l1, int i2, int j2, int k2, int l2,
                                         int i3, int j3) {
        int j4 = j2 >> 16 & 0xff;
        int k4 = j2 >> 8 & 0xff;
    ...
    private void transparentSpritePlot(int pix[], byte colourIdx[], int colours[], int i, int j, int k, int l,
    ...
    private void transparentSpritePlot(int ai[], byte abyte0[], int ai1[], int i, int j, int k, int l,
                                         int i1, int j1, int k1, int l1, int i2, int j2, int k2,
        ...
        try {
            ...
            for (int i2 = 0; i2 < s.length(); i2++) {
                if (s.charAt(i2) == '@' && i2 + 4 < s.length() && s.charAt(i2 + 4) == '@')
                    ...
                    j1 += abyte0[characterWidth[s.charAt(i2)] + 7];
                if (s.charAt(i2) == ' ')
                    l1 = i2;
                if (s.charAt(i2) == '%') {
                    l1 = i2;
    ...
    public Image image;
    public int surfaceSetPixels[][];
    public byte spriteColoursUsed[][];
    public int spriteColours[][];
