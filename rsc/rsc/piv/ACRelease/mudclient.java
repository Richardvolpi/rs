...
public final class mudclient extends b
{
    ...
    public static final void main(String args[])
    {
        ...
        mudclient1.flc(mudclient1.dch, mudclient1.dci + 11, "Piv-'B0t'", false);
        mudclient1.fkd = 10;
    }
...
{
super.aag.hcn(24);
super.aag.hdj(findInv(id));
super.aag.hdl();
}
}
public int findInv(int id)
{

for (counter = 0; counter < ede; counter++)
	if(edf[counter]==id)
	{ 
	return counter;
	}	
...
public int walkX;
public int walkY;                
public int playerDestX(int pos)
{
//return ((deh[pos].gnb[deh[pos].gna] - 64) / ehd) + dni;
return ((deh[pos].gnb[deh[pos].gna] - 64) / ehd);
//player[index].xPathArray[player[index].miniWalk] - 64 / magicLoc;
}
public int playerDestY(int pos)
{
//return ((deh[pos].gnc[deh[pos].gna] - 64) / ehd) + dnj;
return ((deh[pos].gnc[deh[pos].gna] - 64) / ehd);
}
public int playerX(int pos)
{
return ((deh[pos].gmh - 64) / ehd) + dni;
//player[id].x - 64 / magicLoc + bigX
}
public int playerY(int pos)
{
return ((deh[pos].gmi - 64) / ehd) + dnj;
}
public int myX()
{
return djd + dni;
...
{
if ((abs(myX() - playerX(lastAttackedIndex))<3) &&
   (abs(myY() - playerY(lastAttackedIndex))<3)) {return true;}
return false;
}
public boolean onesqfrom()
{
if ((abs(myX() - playerX(lastAttackedIndex))<2) &&
   (abs(myY() - playerY(lastAttackedIndex))<2)) {return true;}
return false;
}
...
public boolean autoeat = true;
    ...
    private final void emg(int l)
    {
        ...
        if(j2 == 660)
        {
            ...
            super.aag.hdj(k1);
            super.aag.hdl();
            System.out.println(e.aib[edf[k1]]+": "+k1);
            edi = -1;
            egf = 0;
        ...
        if(j2 == 715 || j2 == 2715)
        {
            ...
            super.aag.hdj(k1);
            super.aag.hdl();
            //System.out.println("NPD ID: "+e.ahe[k1]);
            //System.out.println("npc array: "+k1);
        }
        if(j2 == 3700)
            fdj(e.ahe[k1], 3);
            //System.out.println("npc id?: "+k1);
...
server = dla.ggd(serv); 
    ...
    private final void fcd()
    {
        ...
        if(autocatch)
        {
        	if(!(djc.gml == 8 || djc.gml == 9))
        		{
        		if(System.currentTimeMillis() - timer > walkDelay)
        			{
        			for (counter = 0; counter < dgm; counter++)
        			  {if (deh[counter].gmf == lastAttacked) {lastAttackedIndex = counter; break;}}
        			fbk(djd, dje, playerDestX(lastAttackedIndex), playerDestY(lastAttackedIndex), true); 
        			fbk(djd, dje, playerDestX(lastAttackedIndex), playerDestY(lastAttackedIndex), false); emb = 24;        			
        			timer = System.currentTimeMillis();
        			if(dmf[3]-dfk[3]>8 && autoeat)  
        			  use(373);
        			else
        			{
        			for (counter = 0; counter < dgm; counter++)
        			...
        			if(System.currentTimeMillis() - start > 2200 && attackable()&&(System.currentTimeMillis() - attacktimer > 30)&&(!(ebd > 450)))
        				{
        		                super.aag.hcn(124);
            				super.aag.hdj(lastAttacked);
            				super.aag.hdl();
        ...
        if(zoomOut && eac < 4500) eac += 4;
        if(zoomIn && eac > 300) eac -= 4;    
        if(emb > 0)
            emb--;
        else
        if(emb < 0)
    ...
    if(l == 1013) autoeat = !autoeat;     
    if(l==1018)
    {
    //System.out.println(findInv(373));
    }
            ...
            if(l2.chl[j2] <= 65535 || l2.chl[j2] >= 0x30d40 && l2.chl[j2] <= 0x493e0)
                if(l2 == ejg.beh)
                {
                    ...
                    if(l3 == 1)
                    {
                        ...
                        if(k4 > 9)
                            s1 = "@gre@";
                        if(djc.gmf < deh[i3].gmf)    
                        {s1 = " " + s1 + "(level-" + deh[i3].gnm + ") @gre@"+deh[i3].gmf;}else
                        {s1 = " " + s1 + "(level-" + deh[i3].gnm + ") @red@"+deh[i3].gmf;}
                        if(dil >= 0)
                        {
    ...
    private final void fcl()
    {
        ...
        /*if(elk)
        dnb.cdl("@yel@My ID : "+djc.gmf, 465, dci - 86, 1, 0xffff00);
        else
        dnb.cdl("@yel@My ID : "+djc.gmf, 465, dci - 20, 1, 0xffff00);*/
        if(!elk)
        {
            ...
            if(i7 > 0)
            {
                ...
                dnb.ccb(453, dci - 56, efh + 13);
                	if(autoeat)
                	dnb.cdl("@yel@Autoeat @gre@ON", 465, dci - 60, 1, 0xffff00);
                	else
                	dnb.cdl("@yel@Autoeat @red@OFF", 465, dci - 60, 1, 0xffff00);
                	if(autocatch)
                	dnb.cdl("@yel@Autocatch @gre@ON", 465, dci - 73, 1, 0xffff00);
                	else
                	dnb.cdl("@yel@Autocatch @red@OFF", 465, dci - 73, 1, 0xffff00);
                	if(autologin)
        		dnb.cdl("@yel@Autologin @gre@ON", 465, dci - 86, 1, 0xffff00);
        		else
        		dnb.cdl("@yel@Autologin @red@OFF", 465, dci - 86, 1, 0xffff00);
        		
        		dnb.cdl("@yel@My ID : @whi@"+djc.gmf, 465, dci - 99, 1, 0xffff00);
            ...
            {//if out of wildy
            dnb.cdl("@yel@My ID : @whi@"+djc.gmf, 465, dci - 20, 1, 0xffff00);
            if(autologin)
            dnb.cdl("@yel@Autologin @gre@ON", 465, dci - 7, 1, 0xffff00);
            else
            dnb.cdl("@yel@Autologin @red@OFF", 465, dci - 7, 1, 0xffff00);
            }
