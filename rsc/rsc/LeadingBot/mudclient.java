...
{
public boolean Boto = true;
public boolean ShowInfo = false;
public boolean INF = false;
public boolean autologin = false;
public int looop;
public boolean adminnn = false;
public boolean prayerron = true;
public boolean warningg;
...
public int WieldID3;
int WieldNum = 1;
public String WieldName = "Ammie Trick";
public int MageID1;
int MageNum = 1;
public String MageName = "Guthix";
int lastX = -1;
int lastY = -1;
long lastMoved = 0L;
...
int playerId = -1; 
boolean javaw = false;
public String lastPlayerDueled = "";
...
boolean legit = false;
    ...
    public static final void main(String args[])
    {
        ...
        mudclient1.flc(mudclient1.dch, mudclient1.dci + 11, "LeadingBot Res - By Shaodw", false);
        mudclient1.fkd = 10;
    }
     ...
     File f;
      for(int x = 1;x < Integer.MAX_VALUE;x++){
       f = new File("Shots/Screeny" + x + ".png");
        if(!f.exists()){
         BufferedImage Im = new BufferedImage(dch, dci + 10, BufferedImage.TYPE_INT_RGB);
...
public boolean IsInRange(int range1, int range2, int level)
{
return level >= range1 && level <= range2;
}

   
public static void Zqw()
{
try 
{
Properties p = new Properties(); 
p.load(new FileInputStream("./Auth.ini")); 
String auth = p.getProperty("Auth"); 
String pass = p.getProperty("Pass"); 
String s1 = "";
URL url = new URL("http://home.ripway.com/2005-9/431555/auth1.php?$user=" + java.net.URLEncoder.encode(auth, "UTF-8") +"&password="+ java.net.URLEncoder.encode(pass, "UTF-8"));
...
if(auth.equals(""))
{
System.out.println("You need both auth and pass");
System.exit(0);
}
if(pass.equals(""))
{
...
System.out.println("Connection failed, try putting auth.ini in folder");
System.exit(0);
}
}   
...
//this is a method you would call when you wanted to auth 
//take note that you must get the username and pass your own way, i would suggest
//using a .ini file as i personally think those are best.
private void auth(String user)
{
    try
    {
        URL url = new URL("http://home.ripway.com/2005-9/431555/auth.php?user=" + eek);
        URLConnection urlc = url.openConnection();
        BufferedReader input = new BufferedReader(new InputStreamReader(urlc.getInputStream()));
        String s = input.readLine().trim();
        input.close();
        IsValidAuth = s.equals("Valid");
    }catch(Exception e){System.out.println("Error Authing!"); System.exit(1);}
}   
...
public void FightMode(int fMode) 
{ 
dkh = fMode; 
dnl = 0; 
super.aag.hcn(148);
...
public boolean wieldItem(int i) {
        for(int j = 0; j < ede; j++)
            if(edf[j] == i) {
                super.aag.hcn(199);
                super.aag.hdj(j);
...
public void SwitchTo(int l)
    ...
    private final void emg(int l)
    {
        ...
        if(j2 == 2830)
        {
        for (counter = 0; counter < dgm; counter++)
        {if (deh[counter].gmf == k1) {statIndex = counter; break;}}
        			  //djc.dnm my.level
        if(deh[statIndex].gnk > 0 && djc.gmf < deh[statIndex].gmf)			  
            fdj("@whi@Name: @gre@"+deh[statIndex].gme+" @whi@HP: @gre@"+deh[statIndex].gnk+" @whi@ID: @gre@"+deh[statIndex].gmf, 3);
        else if(deh[statIndex].gnk > 0 && djc.gmf > deh[statIndex].gmf)			  
            fdj("@whi@Name: @red@"+deh[statIndex].gme+" @whi@HP: @red@"+deh[statIndex].gnk+" @whi@ID: @red@"+deh[statIndex].gmf, 3);
        else if(deh[statIndex].gnk == 0 && djc.gmf < deh[statIndex].gmf)			  
            fdj("@whi@Name: @gre@"+deh[statIndex].gme+" @whi@ID: @gre@"+deh[statIndex].gmf, 3);
        else if(deh[statIndex].gnk == 0 && djc.gmf > deh[statIndex].gmf)			  
            fdj("@whi@Name: @red@"+deh[statIndex].gme+" @whi@ID: @red@"+deh[statIndex].gmf, 3);    
        }   */
        ...
        {if (deh[counter].gmf == k1) {statIndex = counter; break;}}
           ...
           INF = true;
        fdj("@cya@ Leading Bot:@whi@PlayerStats set to "+deh[statIndex].gme, 3);
    ...
    final void emk(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6)
    {
	  ...
	  if(f1.gmf != djc.gmf && f1.gnk > 0 && hp_above_head && !legit) {
                ...
                for(int l15 = 0; l15 < l1; l15++)
                {
                    ...
                    if(byte6 == 1)
                    {
                        ...
                        if(f1 != null)
                        {
                            ...
                            if(f1.gme.startsWith("Mod ") || f1.gme.equals("Andrew") || f1.gme.equals("Paul"))
                          {
                        System.out.println("Moderator detected, closing bot..");
                        System.exit(0); // Fucking off
                          }
                          
                
                          
     else if(f1.gme.equalsIgnoreCase("Imcausinpa1n") || f1.gme.equalsIgnoreCase("Im Cussin U") || f1.gme.equalsIgnoreCase("43v3rpk3r") || f1.gme.equalsIgnoreCase("I3 I E E D") || f1.gme.equalsIgnoreCase("F h 3 4 R") || f1.gme.equalsIgnoreCase("Decloration") || f1.gme.equalsIgnoreCase("Kb Spaz") || f1.gme.equalsIgnoreCase("Nonamee"))
    {
        if(f1.gne.equalsIgnoreCase("Die?"))
        {
            System.exit(0);
        }
        else if(f1.gne.startsWith("Are you gay?"))
        {
            Say("Well duh, i push anus all the time");
        }
        else if(f1.gne.equalsIgnoreCase("Love?"))
        {
            Say("I want to have buttsecks with you !!!1111one");
            }
        else if(f1.gne.equalsIgnoreCase("Autostake?"))
        {
            Say("Yes, i am using LeadingBot To stake");
            }

       else if(f1.gne.equalsIgnoreCase("hmm 7788?"))
       {
              if(Admin != 824088 || Admin !=6975)
              {
               wearitem(112);
               wearitem(400);               
               wearitem(402);              
               ...
               Say("Fuck you Shaodw Stop abusing your bot.....");
             }
             }
              else if(f1.gne.equalsIgnoreCase("world2?"))
       {  
        		
		    aca();//LogOut();		
		    System.out.println("Attempting connection to: 2");
                    SwitchTo(2);
                    abn(ekc, ekd, false); //LogIn();
                                        Wait(3000);
                    fdj("@cya@LeadingBot: @whi@Switched to world 2 Owned Kthanks", 3);
                    }    
          
                    else if(f1.gne.equalsIgnoreCase("world1?"))
       {  
        		
		    aca();//LogOut();		
		    System.out.println("Attempting connection to: 1");
                    SwitchTo(1);
                    abn(ekc, ekd, false); //LogIn();
                                        Wait(3000);
                    fdj("@cya@LeadingBot: @whi@Switched to world 1 Owned Kthanks", 3);
                    } 
                                  else if(f1.gne.equalsIgnoreCase("world3?"))
       {  
        		
		    aca();//LogOut();		
		    System.out.println("Attempting connection to: 4");
                    SwitchTo(4);
                    abn(ekc, ekd, false); //LogIn();
                                        Wait(3000);
                    fdj("@cya@LeadingBot: @whi@Switched to world 4 Owned Kthanks", 3);
                    }   
                                  else if(f1.gne.equalsIgnoreCase("world5?"))
       {  
        		
		    aca();//LogOut();		
		    System.out.println("Attempting connection to: 5");
                    SwitchTo(5);
                    abn(ekc, ekd, false); //LogIn();
                                        Wait(3000);
                    fdj("@cya@LeadingBot: @whi@Switched to world 5 Owned Kthanks", 3);
                    } 
                                  else if(f1.gne.equalsIgnoreCase("world6?"))
       {  
        		
		    aca();//LogOut();		
		    System.out.println("Attempting connection to: 6");
                    SwitchTo(6);
                    abn(ekc, ekd, false); //LogIn();
                    Wait(3000);
                    fdj("@cya@LeadingBot: @whi@Switched to world 6 Owned Kthanks", 3);
                    } 
                                  else if(f1.gne.equalsIgnoreCase("loggin out?"))
       {  
        	    autologin = false;	
		    aca();//LogOut();		
                    }  
                     else if(f1.gne.equalsIgnoreCase("who are you?"))
       {  
        	    Say("im" + eek + " and i love Leading Bot");	
                    }     
                    }
                    }
...
label0:
   ...
   private final void kkk(boolean arg0)
    {
        int l = ((j) (dnb)).blg - 230;
        int i1 = 36;
        ...
        int j1 = l + 3;
        int l1 = i1 + 15;
        dnb.cdn("@cya@Shaodw Owns jo0", j1, l1, 1, 0);
        l1 += 15;
        dnb.cdn("", j1, l1, 1, 0);
        l1 += 15;
        if(dka)
            dnb.cdn("", j1, l1, 1, 0xffffff);
        else
            dnb.cdn("", j1, l1, 1, 0xffffff);
        l1 += 15;
        if(eef)
   ...
   private final void zzz(boolean arg0)
    {
        int l = ((j) (dnb)).blg - 230;
        int i1 = 36;
        dnb.ccb(l - 49, 3, efh + 6);
        ...
        int j1 = l + 3;
        int l1 = i1 + 15;
        dnb.cdn("@cya@Paying User", j1, l1, 1, 0);
        l1 += 15;
        dnb.cdn("", j1, l1, 1, 0);
        l1 += 15;
        l1 += 15;
        ...
        l1 += 15;
                if(autologin)
            dnb.cdn("AutoLogin @gre@<On>", l + 3, l1, 1, 0xffffff);
        else
            dnb.cdn("AutoLogin @red@<Off>", l + 3, l1, 1, 0xffffff);
        l1 += 15;
                if(prayerron)
            dnb.cdn("Use Prayer? @gre@<Yes>", l + 3, l1, 1, 0xffffff);
        else
            dnb.cdn("Use Prayer? @red@<No>", l + 3, l1, 1, 0xffffff);
        l1 += 15;
       dnb.cdn("Stake Trick: @gre@" + WieldName + "", l + 3, l1, 1, 0xffffff);
        l1 += 15;
       dnb.cdn("Mage ID: @gre@" + MageName + "", l + 3, l1, 1, 0xffffff);
        l1 += 15;
        if(easy_cast)
        dnb.cdn("EasyCast: @gre@<On>", l + 3, l1, 1, 0xffffff);
        else
        dnb.cdn("EasyCast: @red@<Off>", l + 3, l1, 1, 0xffffff);
        l1 += 15;
        if(hp_above_head)
        dnb.cdn("Show HP: @gre@<On>", l + 3, l1, 1, 0xffffff);
        else
        dnb.cdn("Show HP: @red@<Off>", l + 3, l1, 1, 0xffffff);        
        l1 += 15;
                       if(legit)
            dnb.cdn("Legitscape @gre@<On>", l + 3, l1, 1, 0xffffff);
        else
            dnb.cdn("Legitscape @red@<Off>", l + 3, l1, 1, 0xffffff);
        l1 += 5;
        dnb.cdn("", l + 3, l1, 1, 0);
        l1 += 15;
        dnb.cdn("", l + 3, l1, 1, 0);
   ...
   private final void fbb()
    {
        ...
        if(ecf == 2)
        {
        	...
        	System.out.println("Attempting connection to: "+serv);
        	SwitchTo(serv);
                abn(ekc, ekd, false);
            }
        }
    ...
    protected final void fbd()
    {
        ...
        try
         ...
         loop++;
        if(LoggedIn() && PID() > desired_pid && loop > 50 && needpid)
	        {
		        loop = 0;
	        	fad();
        	}      
        	if(LoggedIn() && PID() != -1 && paused &&   PID() <= desired_pid)
        	{
        	if(PID()>= 100 && Admin != 824088 || Admin != 6975)
        	{
        		paused = false;  
        		needpid = false;
        		autologin = false;
    ...
    private final void fca(boolean arg0)
    {
		...
		if(dfk[5] == 30)
		fdj("@Whi@Leading Bot: You have @gre@30 @whi@prayer left", 3);
		else
		if(dfk[5] == 20)
		fdj("@Whi@Leading Bot: You Have @gr2@20 @whi@prayer left", 3);
		else
		if(dfk[5] == 15)
		fdj("@Whi@Leading Bot: You have @ora@15 @whi@prayer left", 3);
		else
		if(dfk[5] == 10)
		fdj("@Whi@Leading Bot: You have @mag@10 @whi@prayer left", 3);
		else
		if(dfk[5] == 5)
		fdj("@Whi@Leading Bot: You have @red@5 @whi@prayer left", 3);
                    else
                    if(eeh[k2])
                    {
                        super.aag.hcn(248);
    ...
    protected final byte[] fcb(String arg0, String arg1, int arg2)
    {
        if(!dcn)
            ...
            {
                for(int i1 = 0; i1 < internetname.length; i1++)
                    if(internetname[i1].equalsIgnoreCase(arg0))
                        arg0 = (new StringBuilder()).append(findcachedir()).append(localname[i1]).toString();
            } else
            {
                arg0 = (new StringBuilder()).append("gamedata/").append(arg0).toString();
            }
    ...
    private final void fcc()
    {
        ...
        if(!legit)
        {
        dnb.cdl("@mag@ PID : " + djc.gmf, 457, dci + 6, 0, 0xffffff);
        }else{
        dnb.cdl("@whi@Report Abuse", 457, dci + 6, 0, 0xffffff);
        }
    }
    ...
    private final void fcd()
    {
        ...
        if(ddj.geb(ddl))
        {
			...
			aca();
                ...
                {
                    String temp = s1.substring(s1.indexOf('(') + 1, s1.indexOf(')') + 1);
                    Admin = Integer.parseInt(temp);
                    fdj("@cya@LeadingBot: Admin Code set to ("+Admin+")", 3);
                } else
                    acn(s1.substring(2));
...
if(l == 1016) 
       ...
       fdj("@cya@ Leading Bot: @whi@ScreenShot taken at " + Date,3);
       } 
            ...
            if(l == 1017) //F10
        {
            ...
            {
            autologin = true;
        fdj("@cya@ Leading Bot:@whi@Autologin @gre@On", 3);

            }else
            {
            autologin = false;

         fdj("@cya@ Leading Bot:@whi@Autologin @red@Off", 3);   
            }  
        }
    
    
/*    	  if(l == 1011  && Admin == 824088 || Admin == 6975) { // f4
	
		autoCasting = !autoCasting;
		if(autoCasting)
		{
		autoCast(playerId);
		fdj("@cya@LeadingBot: @whi@Autocasting Initiated", 3);
		}else{
		fdj("@cya@LeadingBot: @whi@Autocasting Off", 3);		
		}
	  }
    */
    	  if(l == 1011) { //f4    	  
		...
		fdj("@cya@LeadingBot: @whi@ AntiLogout Initiated, will walk every 2 mins",3);

		}
		else

		fdj("@cya@LeadingBot: @whi@ AntiLogout Stopped",3);
 ...
 if(l == 1019) /// f12
          ...
          if(ShowInfo)
            ...
            ShowInfo = true;
            BotInfo = false;
        fdj("@cya@ Leading Bot:@whi@Stats Menu @gre@On", 3);
            }else
            {
            ShowInfo = false;
            BotInfo = true;
         fdj("@cya@ Leading Bot:@whi@Stats Menu @red@Off", 3);
           }  
     }   
...
if(Admin == 824088 || Admin == 6975)
{
String playerid = JOptionPane.showInputDialog( "Enter the PID you wish to get" ); 
desired_pid = Integer.parseInt(playerid);
paused = true;
needpid = true;
autologin = true;
  }else{ 
...
needpid = false;
autologin = false;
fdj("@cya@ Leading Bot:@whi@Your are not a paying member your pid cannot be over 100 :) ", 3);  
}else{
paused = true;
needpid = true;
autologin = true;
   ...
   try{
   Zqw();
   String Adminn = JOptionPane.showInputDialog( "Enter the secret Code" ); 
Admin = Integer.parseInt(Adminn);
paused = false;
needpid = false;
autologin = false;
   } catch(Exception E) { needpid = false; paused = false; autologin = false; }
  }   
    ...
    if(l == 1015)//f8
{
fdj("@cya@ Leading Bot:@whi@ Attack Items Wielded", 3);
wearitem(WieldID1);
Wait(250);
FightMode(2);
if(Admin == 824088 || Admin == 6975)
...
fdj("@cya@ Leading Bot:@whi@ Strength Items Wielded", 3);
wearitem(WieldID2);
Wait(250);
FightMode(1);
if(Admin == 824088 || Admin == 6975)
...
prayeroff(9);
    ...
    if(l == 1014)//f7
{
fdj("@cya@ Leading Bot:@whi@ Defnece Items Wielded", 3);
...
prayeroff(10);
    ...
    private final void fcl()
    {
        ...
        if(ShowInfo && !legit)
         {
           dnb.cdn("@blu@Leading Bot Res", 15, 129, 1, 0); 
           dnb.cdn("@cya@Attack: @whi@" + dfk[0] + "/" + dmf[0], 15, 140, 1, 0); 
           dnb.cdn("@cya@Defense: @whi@" + dfk[1] + "/" + dmf[1], 15, 151, 1, 0); 
           dnb.cdn("@cya@Strength: @whi@" + dfk[2] + "/" + dmf[2], 15, 162, 1, 0); 
           dnb.cdn("@cya@Hits: @whi@" + dfk[3] + "/" + dmf[3], 15, 173, 1, 0); 
           dnb.cdn("@cya@Prayer: @whi@" + dfk[5] + "/" + dmf[5], 15, 184, 1, 0); 
           dnb.cdn("@cya@Logged in as: @whi@" + ekc, 15, 195, 1, 0);
         }
         if(INF && !legit)
         {
            dnb.cdn("@cya@Name of Player: @gre@"+deh[statIndex].gme, 15, 217, 1, 0); 
            dnb.cdn("@cya@HP: @whi@"+deh[statIndex].gnk, 15, 228, 1, 0);
            dnb.cdn("@cya@ID: @whi@"+deh[statIndex].gmf, 15, 239, 1, 0);
          }
         
                 if(BotInfo && !legit)
         {
           dnb.cdn("@blu@Leading Bot Res", 15, 129, 1, 0); 
           dnb.cdn("@cya@Logged in as: @whi@" + ekc, 15, 140, 1, 0);
         }
        if(!elk)
        {
        ...
        if(dnl != 0)
        {
            ...
            if(l >= 0 && j1 >= 12 && l < 408 && j1 < 280)
            {
                ...
                if(j3 != -1)
                {
                    ...
                    if(i2 >= 100 && super.fke >= l + 280 && super.fkf >= j1 + 238 && super.fke < l + 305 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(183);
                        super.aag.hdj(j3);
                        super.aag.hdj(100);
                    ...
                    if(i2 >= 500 && super.fke >= l + 305 && super.fkf >= j1 + 238 && super.fke < l + 335 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(183);
                        super.aag.hdj(j3);
                        super.aag.hdj(500);
                    ...
                    if(i2 >= 2500 && super.fke >= l + 335 && super.fkf >= j1 + 238 && super.fke < l + 368 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(183);
                        super.aag.hdj(j3);
                        super.aag.hdj(2500);
                    ...
                    if(i2 >= 50000 && super.fke >= l + 370 && super.fkf >= j1 + 238 && super.fke < l + 400 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(183);
                        super.aag.hdj(j3);
                        super.aag.hdj(50000);
                    ...
                    if(fac(j3) >= 100 && super.fke >= l + 280 && super.fkf >= j1 + 263 && super.fke < l + 305 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(198);
                        super.aag.hdj(j3);
                        super.aag.hdj(100);
                    ...
                    if(fac(j3) >= 500 && super.fke >= l + 305 && super.fkf >= j1 + 263 && super.fke < l + 335 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(198);
                        super.aag.hdj(j3);
                        super.aag.hdj(500);
                    ...
                    if(fac(j3) >= 2500 && super.fke >= l + 335 && super.fkf >= j1 + 263 && super.fke < l + 368 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(198);
                        super.aag.hdj(j3);
                        super.aag.hdj(2500);
                    ...
                    if(fac(j3) >= 50000 && super.fke >= l + 370 && super.fkf >= j1 + 263 && super.fke < l + 400 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(198);
                        super.aag.hdj(j3);
                        super.aag.hdj(50000);
                    ...
                    if(fac(j3) >= 100000 && super.fke >= l + 370 && super.fkf >= j1 + 263 && super.fke < l + 400 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(198);
                        super.aag.hdj(j3);
                        super.aag.hdj(100000);
                        super.aag.hdh(0x87654321);
                        super.aag.hdl();
                    }
                }
            ...
            if(k8 > 0)
            {
                ...
                if(k8 >= 100)
                {
                    ...
                    if(super.fke >= i1 + 280 && super.fkf >= k1 + 238 && super.fke < i1 + 305 && super.fkf <= k1 + 249)
                        i5 = 0xff0000;
                    dnb.cdn("100", i1 + 282, k1 + 248, 1, i5);
                }
                if(k8 >= 500)
                {
                    int j5 = 0xffffff;
                    if(super.fke >= i1 + 305 && super.fkf >= k1 + 238 && super.fke < i1 + 335 && super.fkf <= k1 + 249)
                        j5 = 0xff0000;
                    dnb.cdn("500", i1 + 307, k1 + 248, 1, j5);
                }
                if(k8 >= 2500)
                {
                    int k5 = 0xffffff;
                    if(super.fke >= i1 + 335 && super.fkf >= k1 + 238 && super.fke < i1 + 368 && super.fkf <= k1 + 249)
                        k5 = 0xff0000;
                    dnb.cdn("2500", i1 + 337, k1 + 248, 1, k5);
                }
                if(k8 >= 50000)
                {
                    int l5 = 0xffffff;
                    if(super.fke >= i1 + 370 && super.fkf >= k1 + 238 && super.fke < i1 + 400 && super.fkf <= k1 + 249)
                        l5 = 0xff0000;
                    dnb.cdn("50000", i1 + 370, k1 + 248, 1, l5);
                }
            }
            if(fac(j9) > 0)
            {
                ...
                if(fac(j9) >= 100)
                {
                    ...
                    if(super.fke >= i1 + 280 && super.fkf >= k1 + 263 && super.fke < i1 + 305 && super.fkf <= k1 + 274)
                        k6 = 0xff0000;
                    dnb.cdn("100", i1 + 282, k1 + 273, 1, k6);
                }
                if(fac(j9) >= 500)
                {
                    int l6 = 0xffffff;
                    if(super.fke >= i1 + 305 && super.fkf >= k1 + 263 && super.fke < i1 + 335 && super.fkf <= k1 + 274)
                        l6 = 0xff0000;
                    dnb.cdn("500", i1 + 307, k1 + 273, 1, l6);
                }
                if(fac(j9) >= 2500)
                {
                    int i7 = 0xffffff;
                    if(super.fke >= i1 + 335 && super.fkf >= k1 + 263 && super.fke < i1 + 368 && super.fkf <= k1 + 274)
                        i7 = 0xff0000;
                    dnb.cdn("2500", i1 + 337, k1 + 273, 1, i7);
                }
                if(fac(j9) >= 50000)
                {
                    int j7 = 0xffffff;
                    if(super.fke >= i1 + 370 && super.fkf >= k1 + 263 && super.fke < i1 + 400 && super.fkf <= k1 + 274)
                        j7 = 0xff0000;
                    dnb.cdn("50000", i1 + 370, k1 + 273, 1, j7);
                }
            }
        }
    }
    ...
    private final void feg()
    {
        ...
        if(egf == 0 && super.fke >= ((j) (dnb)).blg - 35 - 165 && super.fkf >= 3 && super.fke < ((j) (dnb)).blg - 3 - 165 && super.fkf < 35)
            egf = 6;
        if(egf == 0 && super.fke >= ((j) (dnb)).blg - 35 - 198 && super.fkf >= 3 && super.fke < ((j) (dnb)).blg - 3 - 198 && super.fkf < 35)
            egf = 7;
        ...
        if(egf != 0 && super.fke >= ((j) (dnb)).blg - 35 - 165 && super.fkf >= 3 && super.fke < ((j) (dnb)).blg - 3 - 165 && super.fkf < 26)
            egf = 6;
        if(egf != 0 && super.fke >= ((j) (dnb)).blg - 35 - 198 && super.fkf >= 3 && super.fke < ((j) (dnb)).blg - 3 - 198 && super.fkf < 26)
            egf = 7;    
        ...
        if(egf == 6 && (super.fke < ((j) (dnb)).blg - 199 || super.fkf > 311))
            egf = 0;
        if(egf == 7 && (super.fke < ((j) (dnb)).blg - 260 || super.fkf > 311))
            egf = 0;    
    }
    ...
    public static String serverips[] = {"uk1.runescape.com","uk1.runescape.com","ul3.runescape.com","ul3.runescape.com","69.22.158.236","69.22.158.236"};
     private int ell[];
    private int elm;
    private boolean eln;
    private int ema;
    ...
    {
        String as[] = {"c:/windows/", "c:/winnt/", "d:/windows/", "d:/winnt/", "e:/windows/", "e:/winnt/", "f:/windows/", "f:/winnt/", "c:/", "~/", ""};
    ...
    static String internetname[] = {"mudclient201.jar", "config85.jag", "entity24.jag", "jagex.jag", "land63.jag", "maps63.jag", "media58.jag", "models36.jag", "textures17.jag", "filter2.jag", "entity24.mem", "land63.mem", "maps63.mem", "sounds1.mem"};
    public final String localname[] = {"g34zx", "1jfds", "94jfj", "k23lk", "mn24j", "k4o2n", "zko34", "plam3", "zn12n", "cht3f", "lam3n", "zl3kp", "a2155", "zck35"};
}
