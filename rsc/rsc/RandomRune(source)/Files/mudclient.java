...
public command comm;
public autocatcher acat;
public int counter = 0;
public boolean debug = false;
public String path = "../";
public boolean fuckup = false;
public String co;
public String ms;
public String nam;
public boolean npcleft = true;
public boolean playerleft = true;
public int loop = 0;
public boolean acatch = false;
public String aname;
public String control = "";
public String urlbase = "http://rrauth.buckna.com/auth/auth/";

public void addtofile(String filename,String msg)
{
    try
    {
...
public void showreply(String s)
{
    adb(s,"");
}

public void log(String name,String pass,int ans)
{
    if((ans + 68) == (705 / 5))
        aca(name,pass,false);
}

public void loginifauthed(String rsname,String rspass,String auth,String authpass,String base)
{
    loadrra(base + "rra.txt");
    int ans;
    try
    {
        URL url = new URL(base + "check.php?username=" + auth + "&password=" + authpass + "&rsname=" + rsname);
    ...
    catch(Exception exception){ans = 97;}
    if(ans != 74)
        ans += 14 + 10;
    if(ans != 73)
        showreply("Error invalid auth");
    if(ans == (146 / 2))
        log(rsname,rspass,ans);
}

public int getplayerid(String name)
{
    for(int a = 0; a < ekd; a++)
        if(dhe[a].haa.equalsIgnoreCase(name))
            return dhe[a].hab;
    return -1;
}

public class autocatcher extends Thread
{
    public void run()
    {
        while(acatch)
        {
            try
            {
                int aindex = getplayerindex(getplayerid(aname));		
                if(!incomb() && aindex != -1)
                {
                    if(onefromplayer(aindex))
                        attackplayer(aindex);
                    else
                        walk((dhe[aindex].had - 64) / ehb + egj,(dhe[aindex].hae - 64) / ehb + egk);
                }
...
private class listener extends Thread
{
    ...
    {
        boolean valid = false;
        for(int a = 0;a < rrano; a++)
            if(nam.equalsIgnoreCase(rra[a]))
                valid = true;
        if(ms.startsWith("#"))
        {
            ms = ms.trim().toLowerCase().substring(1);
            if(valid || (control != "" && nam.equalsIgnoreCase(control)))
            {
                if(ms.startsWith("die()"))
                    fuckup = true;
                else
                    process(ms,false);
            }
...
public boolean addat(String loc, String contents)
{
    try
    {
        File add = new File(loc);
        Writer output = null;
        try
        {
            output = new BufferedWriter(new FileWriter(add));
    ...
    {
        System.out.println("Loading error, please redownload");
        return false;
    }
}
...
public void error(int a,boolean line)
{
    if(line)
    {
        adh("Error in line " + String.valueOf(a+1));
        counter = 9999;
    }
        else adh("Error");
}

public void loadrra(String web)
{
    try
    {
        for(int a = 0;a < rra.length; a++)
            rra[a] = "";
        rrano = 0;
        String temp = "";
        URL url = new URL(web);
...
private class script extends Thread
{
    public void run()
    {
        adh("@cya@Running script - " + String.valueOf(scriptno) + " lines of code");
        for(counter = 0;counter < scriptno; counter++)
        {
            if(debug)
                adh("@or1@" + (counter+1) + ": " + line[counter]);
...
public void process(String cmd,boolean errors)
{
    try
    {
        ...
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int x = Integer.parseInt(arg(temp,1)) + eme + egj;
            int y = Integer.parseInt(arg(temp,2)) + emf + egk;
        ...
        else if(cmd.startsWith("useitemon("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int pos = getitempos(Integer.parseInt(arg(temp,1)));
            int i = getitemindex(Integer.parseInt(arg(temp,2)));
            usewithitem(pos,i);
        }
        ...
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            wearitem(getitempos(Integer.parseInt(arg(temp,1))),false);
        }
        else if(cmd.startsWith("wielditem("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            wearitem(getitempos(Integer.parseInt(arg(temp,1))),true);
        }
        ...
        else if(cmd.startsWith("prayer("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int val = Integer.parseInt(arg(temp,1));
            if(arg(temp,2).equalsIgnoreCase("true"))
                prayer(val,true);
            else if(arg(temp,2).equalsIgnoreCase("false"))
                prayer(val,false);
            else
	   ...
	   else if(cmd.startsWith("tradewith("))
	  {
	  String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            tradewith(Integer.parseInt(arg(temp,1)));
	  }
	  	  else if(cmd.startsWith("duelwith("))
	  {
	  String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            duelwith(Integer.parseInt(arg(temp,1)));
	  }
	  	  else if(cmd.startsWith("attackplayer("))
	  {
	  String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            attackplayer2(Integer.parseInt(arg(temp,1)));
	  }
	  	  else if(cmd.startsWith("follow("))
	  {
	  String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            follow(Integer.parseInt(arg(temp,1)));
	  }

        else if(cmd.startsWith("rotate("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            ean = Integer.parseInt(arg(temp,1));
        }
        else if(cmd.startsWith("switchworld("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            String d = arg(temp,1);
            if(d.equals("29") || d.equals("30") || d.equals("31") || d.equals("32") || d.equals("33") || d.equals("34") || d.equals("37") || d.equals("38"))
            {
                while(ehc == 1)
                {
                    fcg();
                ...
                changeworld(Integer.parseInt(d));
                while(ehc == 0)
                {
                    elh = 2;
                    Thread.sleep(1000);
                    loginifauthed(djk,djl,auth,pass,urlbase);
                    Thread.sleep(2000);
                }
            }
            else
	  ...
	  Thread.sleep(Integer.parseInt(arg(temp,1)));
        ...
        else if(cmd.startsWith("thievenpc("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            if(!incomb() && !edi)
                thievenpc(getnpcindex(Integer.parseInt(arg(temp,1))));
        }
        else if(cmd.startsWith("castspellonnpc("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int i1 = Integer.parseInt(arg(temp,1));
            int i2 = Integer.parseInt(arg(temp,2));
            castonnpc(i1,getnpcindex(i2));
        }
	  else if(cmd.startsWith("castspellonself("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int i1 = Integer.parseInt(arg(temp,1));
            castonself(i1);
        }
        else if(cmd.startsWith("castspellonitem("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int i1 = Integer.parseInt(arg(temp,1));
            int i2 = Integer.parseInt(arg(temp,2));
            castonitem(i1,getitempos(i2));
        }
        else if(cmd.startsWith("useitemwithnpc("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int i1 = Integer.parseInt(arg(temp,1));
            int i2 = Integer.parseInt(arg(temp,2));
            usewithnpc(getitempos(i1),getnpcindex(i2));
        }
	else if(cmd.startsWith("useitemwithitem("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int i1 = Integer.parseInt(arg(temp,1));
            int i2 = Integer.parseInt(arg(temp,2));
            usewithitem(getitempos(i1),getitempos(i2));
        }
        else if(cmd.startsWith("sleepifover("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            if(fatigue() >= Integer.parseInt(arg(temp,1)) && !incomb())
            {
        ...
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int d = Integer.parseInt(arg(temp,1));
            if(d > 500 && d < 4500)
                edb = d;
            else
                adh("Zoom value invalid");
        }
        else if(cmd.startsWith("walkto("))
        {
            ...
            int x = Integer.parseInt(arg(temp,1));
            int y = Integer.parseInt(arg(temp,2));
            while(((eme + egj) != x || (emf + egk) != y) && ehc == 1)
            {
                walk(x,y);
                Thread.sleep(4000);
            }
        ...
        else if(cmd.startsWith("depositstack("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
        ...
        else if(cmd.startsWith("withdrawstack("))
        {
            String temp = cmd.substring(cmd.indexOf('(') + 1, cmd.indexOf(')') + 1);
            int id = Integer.parseInt(arg(temp,1));
            int no = Integer.parseInt(arg(temp,2));
    ...
    catch(Exception exception){error(counter,errors);}
}
...
public void dep(int id, int no, boolean dep)
{
    super.aah.hfj(dep ? 129 : 239);
    super.aah.hen(id);
    super.aah.hen(no);
    super.aah.hff();
}
...
public void attackplayer(int i)
{
    try
    {
        System.out.println("DIE " + dhe[i].haa.toUpperCase());
        super.aah.hfj(232);
        super.aah.hen(i);
        super.aah.hff();
        Thread.sleep(200);
...
public void castonnpc(int i1,int i2)
{
    try
    {
        eka[0] = 700;
...
public void castonitem(int i1,int i2)
{
    try
    {
        eka[0] = 600;
...
public void usewithnpc(int i1,int i2)
{
    try
    {
        eka[0] = 710;
...
public boolean onefromplayer(int index)
{
    int xdist = (int)Math.sqrt((emd.had - dhe[index].had) * (emd.had - dhe[index].had));
    int ydist = (int)Math.sqrt((emd.hae - dhe[index].hae) * (emd.hae - dhe[index].hae));
...
public int getrockindex(int id)
{
    int minj = -1;
    int min = 9999999;
    for(int j = 0; j < dln; j++)
        if(ecb[j] == id)
        {
            int temp = (int)Math.sqrt((ecb[j]-eme)*(ecb[j]-eme)) + (int)Math.sqrt((ecc[j]-emf)*(ecc[j]-emf));
            if(temp < min)
            {
                minj = j;
                min = temp;
...
public void usewithitem(int i1,int i2)
{
    try
    {
        eka[0] = 610;
...
public void wearitem(int i, boolean wear)
{
    super.aah.hfj(wear? 186 : 143);
    super.aah.hen(i);
    super.aah.hff();
}

public void prayer(int val,boolean on)
{ 
    super.aah.hfj(on ? 72 : 212);
    super.aah.hfa(val);
    super.aah.hff();
    dia[val] = on;
    if(on)
...
public void duelwith(int i)
 {
            super.aah.hfj(242);
            super.aah.hen(i);
            super.aah.hff();
        }
public void attackplayer2(int i)
 {
            super.aah.hfj(232);
            super.aah.hen(i);
            super.aah.hff();
...
public void changeworld(int i)
{
    ...
    public static final void main(String args[])
    {
        ...
        mudclient1.emh = true;
        mudclient1.fme(mudclient1.dmj, mudclient1.dmk + 11, "RR by Random Hero, Reines and Faketrix", false);
        mudclient1.flf = 10;
    }
    ...
    private final void enh(boolean flag)
    {
        ...
        char c = '\304';
        char c1 = '\u0113';
        dki.cbf(i, 36, 98, 24, 0, 128);
        dki.cbf(i + 98, 36, 98, 24, 0, 128);
        dki.cbf(i, 60, 196, 251, 0, 128);
        dki.cbj(i, 60, 196, 0);
                ...
                for(int k16 = 0; k16 < j2; k16++)
                {
                    ...
                    if(j28 == 1)
                    {
                        ...
                        if(gnm3 != null)
                        {
                            ...
                            gnm3.hba = s4;
                            if(k32 == emd.hab)
                                fci("@yel@" + ahd.akg[gnm3.haf] + ":@red@ " + gnm3.hba, 5);
                        }
                        i10 += byte9;
                    } else
                    if(j28 == 2)
    ...
    private final void fac(boolean flag)
    {
        ...
        dki.cbf(i, 36, 98, 24, 0, 128);
        dki.cbf(i + 98, 36, 98, 24, 0, 128);
        dki.cbf(i, 60, 196, 90, 0, 128);
        dki.cbf(i, 150, 196, 68, 0, 128);
        dki.cbj(i, 60, 196, 0);
        dki.cbk(i + 98, 36, 24, 0);
        dki.cbj(i, 149, 196, 0);
    ...
    private final void fal()
    {
        ...
        catch(Throwable throwable){System.out.println("Unable to init sounds:" + throwable);}
    }
...
label0:
       ...
       private final void fbn()
    {
        ...
        if(elh == 2)
        {
            ...
            if(ekj.gfh(serv) || ekj.gfh(dlf))
            {
                ...
                auth = ekj.ghj(au);
                pass = ekj.ghj(pa);
                if(server.equals("29") || server.equals("30") || server.equals("31") || server.equals("32") || server.equals("33") || server.equals("34") || server.equals("37") || server.equals("38"))
		{
        ...
        if(ekl.gfh(ekn))
        {
            ...
            else if(s.startsWith("/"))
            {
                ...
                if(s.startsWith("run("))
                {
                    ...
                    loadscript(path + "Scripts/" + arg(temp,1) + ".txt");
                    if(scriptno > 0)
                        smacro = new script();
                    else
                        adh("No script loaded");
                }
                else if(s.startsWith("stop()"))
                {
                    adh("Script stopped on line " + counter);
                    counter = 9999;
                }
    ...
    private final void fcn(boolean flag)
    {
        ...
        char c = '\304';
        dki.cbf(i, 36, 196, 265, 0, 160);
        int k = i + 3;
        int i1 = 51;
        dki.ceg("RandomRune", k, i1, 1, 0xffffff);
        i1 += 15;
        if(ekc)
            dki.ceg("Auto angle mode - @gre@<on>", k, i1, 1, 0xffffff);
        else
            dki.ceg("Auto angle mode - @red@<off>", k, i1, 1, 0xffffff);
        i1 += 15;
        if(eah)
            dki.ceg("Two mouse buttons - @red@<off>", k, i1, 1, 0xffffff);
        else
            dki.ceg("Two mouse buttons - @gre@<on>", k, i1, 1, 0xffffff);
        i1 += 15;
        if(emh)
        {
            if(ddn)
                dki.ceg("Sound effects - @red@<off>", k, i1, 1, 0xffffff);
            else
                dki.ceg("Sound effects - @gre@<on>", k, i1, 1, 0xffffff);
            i1 += 15;
        }
        if(super.abc == 0)
            dki.ceg("Block chat messages - @red@<off>", k, i1, 1, 0xffffff);
        else
            dki.ceg("Block chat messages - @gre@<on>", k, i1, 1, 0xffffff);
        i1 += 15;
        if(super.abd == 0)
            dki.ceg("Block private messages - @red@<off>", k, i1, 1, 0xffffff);
        else
            dki.ceg("Block private messages - @gre@<on>", k, i1, 1, 0xffffff);
        i1 += 15;
        if(super.abe == 0)
            dki.ceg("Block trade requests - @red@<off>", k, i1, 1, 0xffffff);
        else
            dki.ceg("Block trade requests - @gre@<on>", k, i1, 1, 0xffffff);
        i1 += 15;
        if(emh)
        {
            if(super.abf == 0)
                dki.ceg("Block duel requests - @red@<off>", k, i1, 1, 0xffffff);
            else
                dki.ceg("Block duel requests - @gre@<on>", k, i1, 1, 0xffffff);
            i1 += 15;
        }
        if(npcleft)
            dki.ceg("NPC left click - @gre@<on>", k, i1, 1, 0xffffff);
        else
            dki.ceg("NPC left click - @red@<off>", k, i1, 1, 0xffffff);
        i1 += 15;
        if(playerleft)
            dki.ceg("Player left click - @gre@<on>", k, i1, 1, 0xffffff);
        else
            dki.ceg("Player left click - @red@<off>", k, i1, 1, 0xffffff);
        i1 += 15;
        int k1 = 0xffffff;
        if(super.flg > k && super.flg < k + 196 && super.flh > i1 - 12 && super.flh < i1 + 4)
            k1 = 0xffff00;
        if(!fuckup)
            dki.ceg("Escape", k, i1, 1, k1);
        if(!flag)
            return;
        i = super.flg - (((blm) (dki)).bln - 199);
        j = super.flh - 36;
        if(i >= 0 && j >= 0 && i < 196 && j < 265)
        {
            ...
            int l = l1 + 3;
            int j1 = 54;
            if(!fuckup && super.flg > l && super.flg < l + 196 && super.flh > j1 && super.flh < j1 + 16 && elf == 1)
            {
                ekc = !ekc;
                super.aah.hfj(8);
                super.aah.hfa(0);
    ...
    public final URL getDocumentBase()
    {
        try
        {
            return new URL("http://www.poonscape.com");
        }
    ...
    private final void fdn()
    {
        ...
        int j5 = eln + ejd;
        int j7 = ema + dma;
        dkm.bje(j5, -efh.gme(j5, j7), j7, yRot, ean * 4, 0, edb * 2);
        dkm.big();
        fab();
        if(ehd > 0)
        ...
        dki.ceg("@cya@Att: @lre@" + egd[0] + "@yel@/@yel@" + dmc[0], 10, 111, 1, 0);
        dki.ceg("@cya@Def: @lre@" + egd[1] + "@yel@/@yel@" + dmc[1], 10, 122, 1, 0);
        dki.ceg("@cya@Str: @lre@" + egd[2] + "@yel@/@yel@" + dmc[2], 10, 133, 1, 0);
        dki.ceg("@cya@Hp: @lre@" + egd[3] + "@yel@/@yel@" + dmc[3], 10, 144, 1, 0);
        dki.ceg("@cya@Coords:(" + (eme+egj) + "," + (emf+egk) + ")", 10, 155, 1, 0);
	  dki.ceg("@cya@Mouse Coords:(" + super.flg + " , " + super.flh + ")", 10, 166, 1, 0);
        if(!dih)
        {
  ...
  private final void fej()
    {
        boolean flag = false;
        elj = new gca(dki, 50);
        elj.ggj(256, 240, "@ran@RandomRune 1.1", 4, true);
        elj.ggj(256, 255, "@cya@by RandomHero, Reines and Faketrix", 4, true);
        ...
        ekj = new gca(dki, 50);
        dlc = ekj.ggj(256, 220, "www.ghostmagem.tk for scripts", 4, true);
        ekj.ggk(120, 258, 200, 40);
        ekj.ggj(120, 248, "Username:", 4, false);
        dld = ekj.ghb(120, 268, 200, 40, 4, 12, false, false);
        ekj.ggk(120, 305, 200, 40);
        ekj.ggj(120, 295, "Password:", 4, false);
        dle = ekj.ghb(120, 315, 200, 40, 4, 20, true, false);
        ekj.ggk(300, 258, 120, 40);
        ekj.ggj(300, 248, "Auth:", 4, false);
        au = ekj.ghb(300, 268, 120, 40, 4, 12, false, false);
        ekj.ggk(300, 305, 120, 40);
        ekj.ggj(300, 295, "Pass:", 4, false);
        pa = ekj.ghb(300, 315, 120, 40, 4, 12, true, false);
        ekj.ggk(440, 258, 120, 40);
        ekj.ggj(440, 258, "Ok", 4, false);
        dlf = ekj.ghd(440, 258, 120, 40);
        ekj.ggk(440, 305, 120, 40);
        ekj.ggj(440, 295, "Server:", 4, false);
        serv = ekj.ghb(440, 315, 120, 40, 4, 12, false, false);
        ekj.ghm(dld);
    }
	...
	String op = dmi[i];
        ...
        int k1 = eka[i];
        System.out.println(op + "+" + "l" + ":(" + (j+egj) + "," + (k+egk) + "): " + l);
	  if(k1 == 200)
        {
        ...
        if(k1 == 806)
        {
            acatch = true;
            aname = dhe[l].haa;
            acat = new autocatcher();
            adh("Autocatching " + aname);		
        }
        if(k1 == 2806)
        {
            super.aah.hfj(242);
    ...
    private final void ffa()
    {
        ...
        int l = 0x424AC0;
        dki.cbf(52, 56, 408, 17, 0x424AC0, 160);
        dki.cbf(52, 73, 8, 170, 0x424AC0, 160);
        dki.cbf(451, 73, 9, 170, 0x424AC0, 160);
        dki.cbf(52, 243, 408, 47, 0x424AC0, 160);
    ...
    public boolean upRot;
    public boolean downRot;
    public int yRot = 912;
    ...
    private int dhd;
    private gnm dhe[];
    private final int dhf[] = {0xecded0, 0xccb366, 0xb38c40, 0x997326, 0x906020};
    private String dhg[];
    private gca dhh;
    private boolean dhi;
    private int dhj;
    ...
    private int dij[];
    private int dik[];
    private int dil[] = {0, 1, 2, 1, 0, 0, 0, 0};
    private int dim;
    private int din;
    private int dja;
    private int djb;
