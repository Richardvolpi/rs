public class certironbar extends Methods
{
    public certironbar(mudclient mc){super(mc);}
    private long l;

    public void MainBody(String as[])
    {
	AutoLogin(true);
	  Display("@mag@Draynor Iron Bar Certer script -- @gre@Nattawat");
	  Display("@whi@Make sure you bars in your bag!");
        while(Running())
        {
		WalkToWait (224,633);
        	Wait(10);
            ...
            while(!InBank() && Running() && GetMillis() - l <= 10000)
                Wait(1000);
	    while(CountInv(528) > 0 && Running() && InBank())
	    {
		Deposit(528,1);
		Wait(500);
	    }
            while(CountInv(170) < 25 && Running() && InBank())
            {
                Withdraw(170,1);
                Wait(300);
            }
