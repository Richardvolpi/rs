...
public final class mudclient extends aaa
{
   ...
   private AutoEat AE;
    ...
    private class AutoEat extends Thread {
        public void run()
        {
                           ...
                           if(fec(foodID) >= 1)
		   {
                               ...
                               fci("@ora@InfiniTBoT: @whi@AutoEat: @red@Couldn't find any " + foodName, 3);
                               autoEat = false;
                               Wait(50);
                           }
                        } else {
    ...
    private class AutoThieve extends Thread {
		...
		if(AutoThieve(GetNPCByIDNearest(MacroNPCID))) {
			Wait(MacroTime);
		} else {
			Wait(50);
		}			
    ...
    private class AutoFight extends Thread {
		...
		if(AutoAttack(GetNPCByIDNearest(MacroNPCID)))
           	{
			Wait(MacroTime);
		} else {
			Wait(50);
    ...
    public static final void main(String args[])
    {
        ...
        mudclient1.emh = true;
        mudclient1.fme(mudclient1.dmj, mudclient1.dmk + 11, "InfiniTBoT By Sbm, Br0ken and Swizz- updated by Rich Lil Nigg", false);
        mudclient1.flf = 10;
    }
    ...
    private final void enh(boolean flag)
    {
        ...
        dki.cbf(i, 60, 196, 251, 0x40FFF, 128);
        dki.cbj(i, 60, 196, 0);
    ...
    private final void fac(boolean flag)
    {
        ...
        dki.cbf(i, 36, 98, 24, k, 128);
        dki.cbf(i + 98, 36, 98, 24, l, 128);
        dki.cbf(i, 60, 196, 90, 0x40FFF, 128);
        dki.cbf(i, 150, 196, 68, 0x40FFF, 128);
        dki.cbj(i, 60, 196, 0);
        dki.cbk(i + 98, 36, 24, 0);
        dki.cbj(i, 149, 196, 0);
    ...
    private final void fae()
    {
    	...
    	private boolean OneSQFrom(int x1, int y1, int x2, int y2)
	{
		if (Between(x1, x2-1, x2+1) && Between(y1, y2-1, y2+1) || Between(x1, x2-2, x2+2) && Between(y1, y2-2, y2+2) )
		{
			return true;
		}
		return false;
    	...
    	private void AutoCatch()
	{
		//try
		//{
			if (PIDFromName(catchName) > 0)
			{
				if (System.currentTimeMillis() - catchState >= walkTime)
				{
					int catchArray = PIndexFromID(PIDFromName(catchName));
					
					if(!SelfIsFighting()) {
					if (OneSQFrom(eme, emf, AbsX(dhe[catchArray].had), AbsY(dhe[catchArray].hae)) == true && attackState >= 120)
					{
						catchState = System.currentTimeMillis();
						 fdl(eme, emf, AbsX(dhe[catchArray].had), AbsY(dhe[catchArray].hae), true);
						AttackUser(dhe[catchArray].hab);
					} else {
						fdl(eme, emf, AbsX(dhe[catchArray].had), AbsY(dhe[catchArray].hae), false);
						attackState += 1;
						catchState = System.currentTimeMillis();            
						if(ehd == -24)
							ehd = 24;
    ...
    private final void fam(boolean flag)
    {
        ...
        dki.cbf(i, 36, 98, 24, k, 128);
        dki.cbf(i + 98, 36, 98, 24, l, 128);
        dki.cbf(i, 60, 196, 158, 0x40FFF, 128);
        dki.cbj(i, 60, 196, 0);
        dki.cbk(i + 98, 36, 24, 0);
        dki.cbj(i, 202, 196, 0);
...
label0:
        ...
        System.out.println("InfiniTBot by Sbm007, Swizz and Br0ken, updated by Rich Lil Nigg");
    }
    ...
    private final void fbn()
    {
        ...
        if(elh == 2)
        {
            ...
            if(ekj.gfh(dle))
                ekj.ghm(sbm);
            if(ekj.gfh(dle) || ekj.gfh(dlf) || ekj.gfh(sbm))
            {
                djk = ekj.ghj(dld);
                djl = ekj.ghj(dle);
                djs = ekj.ghj(sbm);
  ...
  private final void fcf()
    ...
    private final void fcn(boolean flag)
    {
        ...
        char c = '\304';
        dki.cbf(i, 36, 196, 65, 0x40FFF, 160);
        dki.cbf(i, 101, 196, 65, 0x40FFF, 160);
        dki.cbf(i, 166, 196, 95, 0x40FFF, 160);
        dki.cbf(i, 261, 196, 40, 0x40FFF, 160);
        int k = i + 3;
        int i1 = 51;
        if(ekc)
            dki.ceg("Camera: @gre@Auto", k, 51, 1, 0xffffff);
        else
            dki.ceg("Camera: @red@Manual", k, 51, 1, 0xffffff);
        i1 += 15;
        if(eah)
            dki.ceg("Mouse Buttons: @red@One", k, 66, 1, 0xffffff);
        else
            dki.ceg("Mouse Buttons: @gre@Two", k, 66, 1, 0xffffff);
        i1 += 15;
        if(emh)
            if(ddn)
                dki.ceg("Sound Effects: @red@Off", k, 81, 1, 0xffffff);
            else
                dki.ceg("Sound Effects: @gre@On", k, 81, 1, 0xffffff);
        i1 += 15;
        if(super.abc == 0)
            dki.ceg("Block Chat Messages: @red@Off", i + 3, 101, 1, 0xffffff);
        else
            dki.ceg("Block Chat Messages: @gre@On", i + 3, 101, 1, 0xffffff);
        i1 += 15;
        if(super.abd == 0)
            dki.ceg("Block Private Messages: @red@Off", i + 3, 116, 1, 0xffffff);
        else
            dki.ceg("Block Private Messages: @gre@On", i + 3, 116, 1, 0xffffff);
        i1 += 15;
        if(super.abe == 0)
            dki.ceg("Block Trade Requests: @red@Off", i + 3, 131, 1, 0xffffff);
        else
            dki.ceg("Block Trade Requests: @gre@On", i + 3, 131, 1, 0xffffff);
        i1 += 15;
        if(emh)
            if(super.abf == 0)
                dki.ceg("Block Duel Requests: @red@Off", i + 3, 146, 1, 0xffffff);
            else
                dki.ceg("Block Duel Requests: @gre@On", i + 3, 146, 1, 0xffffff);
        i1 += 15;      
                dki.ceg("Hop To: @gre@" + choseWorld + " @whi@(Current: @gre@" + sWorld + "@whi@)", k, 171, 1, 0xffffff);
        i1 += 15;
        if(!alwaysLefty)
                dki.ceg("Always Left Click: @red@Off", k, 186, 1, 0xffffff);
        else
                dki.ceg("Always Left Click: @gre@On", k, 186, 1, 0xffffff);
        i1 += 15;
        if(!fogOfwar)
                dki.ceg("Fog of War: @red@Disabled", k, 201, 1, 0xffffff);
        else
                dki.ceg("Fog of War: @gre@Enabled", k, 201, 1, 0xffffff);
        i1 += 15;
                dki.ceg("Fix Privacy Settings", k, 216, 1, 0xffffff);
        i1 += 15;
        if(foodType == 0) {
                dki.ceg("Food Type: @gre@Cooked Meat", k, 231, 1, 0xffffff);
                foodName = "Cooked Meat";
                foodID = 132;
        } else {
        if(foodType == 1) {
                dki.ceg("Food Type: @gre@Shrimps", k, 231, 1, 0xffffff);
                foodName = "Shrimps";
                foodID = 350;
        } else {
        if(foodType == 2) {
                dki.ceg("Food Type: @gre@Anchovies", k, 231, 1, 0xffffff);
                foodName = "Anchovies";
                foodID = 352;
        } else {
        if(foodType == 3) {
                dki.ceg("Food Type: @gre@Sardine", k, 231, 1, 0xffffff);
                foodName = "Sardine";
                foodID = 355;
        } else {
        if(foodType == 4) {
                dki.ceg("Food Type: @gre@Salmon", k, 231, 1, 0xffffff);
                foodName = "Salmon";
                foodID = 357;
        } else {
        if(foodType == 5) {
                dki.ceg("Food Type: @gre@Trout", k, 231, 1, 0xffffff);
                foodName = "Trout";
                foodID = 359;
        } else {
        if(foodType == 6) {
                dki.ceg("Food Type: @gre@Herring", k, 231, 1, 0xffffff);
                foodName = "Herring";
                foodID = 362;
        } else {
        if(foodType == 7) {
                dki.ceg("Food Type: @gre@Pike", k, 231, 1, 0xffffff);
                foodName = "Pike";
                foodID = 364;
        } else {
        if(foodType == 8) {
                dki.ceg("Food Type: @gre@Tuna", k, 231, 1, 0xffffff);
                foodName = "Tuna";
                foodID = 367;
        } else {
        if(foodType == 9) {
                dki.ceg("Food Type: @gre@Swordfish", k, 231, 1, 0xffffff);
                foodName = "Swordfish";
                foodID = 370;
        } else {
        if(foodType == 10) {
                dki.ceg("Food Type: @gre@Lobster", k, 231, 1, 0xffffff);
                foodName = "Lobster";
                foodID = 373;                
        } else {
        if(foodType == 11) {
                dki.ceg("Food Type: @gre@Shark", k, 231, 1, 0xffffff);
                foodName = "Shark";
                foodID = 546;
        } else {
        if(foodType == 12) {
                dki.ceg("Food Type: @gre@Mackerel", k, 231, 1, 0xffffff);
                foodName = "Mackerel";
                foodID = 553;
        } else {
        if(foodType == 13) {
                dki.ceg("Food Type: @gre@Bass", k, 231, 1, 0xffffff);
                foodName = "Bass";
                foodID = 555;
        } else {
        if(foodType == 14) {
                dki.ceg("Food Type: @gre@Kebab", k, 231, 1, 0xffffff);
                foodName = "Kebab";
                foodID = 210;
        } else {
        if(foodType == 15) {
                dki.ceg("Food Type: @gre@Cod", k, 231, 1, 0xffffff);
                foodName = "Cod";
                foodID = 551;
        }
        }
		...
		if(miningMode == 0) {
			dki.ceg("Set to mine/fish to your: @gre@Up", k, 246, 1, 0xffffff);
			miningMode = 0;
		} else {
		if(miningMode == 1) {
			dki.ceg("Set to mine/fish to your: @gre@Down", k, 246, 1, 0xffffff);
			miningMode = 1;
		} else {
		if(miningMode == 2) {
			dki.ceg("Set to mine/fish to your: @gre@Right", k, 246, 1, 0xffffff);
			miningMode = 2;
		} else {
		if(miningMode == 3) {
			dki.ceg("Set to mine/fish to your: @gre@Left", k, 246, 1, 0xffffff);
			miningMode = 3;
		}
		}
		}
        ...
        if(i >= 0 && j >= 0 && i < 196 && j < 265)
        {
            ...
            int l = l1 + 3;
            int j1 = 66;
            if(super.flg > l && super.flg < l + 196 && super.flh > 39 && super.flh < 55 && elf == 1)
            {
                ekc = !ekc;
            ...
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 51 && super.flh < 67 && elf == 1)
            {
                eah = !eah;
                super.aah.hfj(165);
                super.aah.hfa(2);
            ...
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 159 && super.flh < 175 && elf == 1)
            {
                LogOut();
                SwitchTo(choseWorld);
                LogIn();
            ...
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 174 && super.flh < 190 && elf == 1)
            {
                alwaysLefty = !alwaysLefty;
            }

            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 189 && super.flh < 205 && elf == 1)
            {
                fogOfwar = !fogOfwar;
            }
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 204 && super.flh < 220 && elf == 1)
            {
                fci("@ora@InfiniTBoT: @whi@Private settings fixed.",3);
                super.abc = 0;
                super.abd = 0;
            ...
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 219 && super.flh < 235 && elf == 1)
            {
                if(foodType == 15) {
                    foodType = 0;
                } else {
            ...
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 237 && super.flh < 248 && elf == 1)
            {
            	if(miningMode == 3) {
            		miningMode = 0;
            	} else {
            ...
            j1 += 15;
            if(emh && super.flg > l && super.flg < l + 196 && super.flh > 63 && super.flh < 79 && elf == 1)
            {
                ddn = !ddn;
                super.aah.hfj(165);
                super.aah.hfa(3);
            ...
            boolean flag1 = false;
            j1 += 35;
            if(super.flg > l && super.flg < l + 196 && super.flh > 89 && super.flh < 105 && elf == 1)
            {
                super.abc = 1 - super.abc;
                flag1 = true;
            }
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 104 && super.flh < 120 && elf == 1)
            {
                super.abd = 1 - super.abd;
                flag1 = true;
            }
            j1 += 15;
            if(super.flg > l && super.flg < l + 196 && super.flh > 119 && super.flh < 135 && elf == 1)
            {
                super.abe = 1 - super.abe;
                flag1 = true;
            }
            j1 += 15;
            if(emh && super.flg > l && super.flg < l + 196 && super.flh > 134 && super.flh < 150 && elf == 1)
            {
    ...
    private final void fdk()
    {
        ...
        dki.cee("InfiniTBoT", 457, dmk + 6, 0, 0xffffff);
    }
    ...
    private final void fdn()
    {
        ...
        if(edi)
        {
            ...
            dki.cbh(dmj / 2 - 100, 160-24, 200, 40, 0);
            dki.cee("You are sleeping", dmj / 2, 50-24, 7, 0xffff00);
            dki.cee("Fatigue: " + (eji * 100) / 750 + "%", dmj / 2, 90-24, 7, 0xffff00);
            dki.cee("When you want to wake up just use your", dmj / 2, 140-24, 5, 0xffffff);
            dki.cee("keyboard to type the word in the box below", dmj / 2, 160-24, 5, 0xffffff);
            dki.cee(super.fln + "*", dmj / 2, 180-24, 5, 65535);
            if(ejk == null)
                dki.cci(dmj / 2 - 127, 230-24, eii + 1);
            else
                dki.cee(ejk, dmj / 2, 260-24, 5, 0xff0000);
            dki.cbi(dmj / 2 - 128, 229-24, 257, 42, 0xffffff);
            fdk();
            dki.cee("If you can't read the word", dmj / 2, 290-24, 1, 0xffffff);
            dki.cee("@yel@click here@whi@ to get a different one", dmj / 2, 305-24, 1, 0xffffff);
            dki.cbc(eff, 0, 0);
            return;
        }
        if(!efh.gjj)
        ...
        if(!dih)
        {
            if((ejh * 100) / 750 >= SleepFat && autoSleep && !SelfIsFighting())
			{
				if(fec(1263) >= 1)
				{
		 			fci("@ora@InfiniTBoT: @whi@AutoSleeping...", 3);
               	 	UseItem(1263);
		...
		if((ejh * 100) / 750 < SleepFat && isSleeping)
		{
            ...
            if(showInfo){
            	dki.cee("@ora@Username: @whi@" + djk, 68, 170, 1, 0xffff00);
            	dki.cee("@ora@Str: @whi@" + dmc[2] + " @ora@(" + (emk[2] / 4) + " exp)", 70, 180, 1, 0xffff00);
            	dki.cee("@ora@HP: @whi@" + dmc[3] + " @ora@(" + (emk[3] / 4) + " exp)", 70, 190, 1, 0xffff00);
            	dki.cee("@ora@Coords: @whi@(" + (eme+egj) + " , " + (emf+egk) + ")", 450, 260, 1, 0xffff00);
            	dki.cee("@ora@MCoords: @whi@(" + super.flg + " , " + super.flh + ")", 450, 270, 1, 0xffff00);
            	dki.cee("@ora@InfinitBot@whi@ vA", 450, 300, 1, 0xffff00);
            }	            
            if(eme + dha + egj >= 2640)
                l5 = -50;
            if(l5 > 0)
    ...
    private final void fej()
    {
        ...
        if(!emh)
        {
            elj.ggj(256, 240, "@ora@Infinit@whi@Bot - @gre@Made by Br0ken, Swizz and Sbm", 5, true);
        ...
        {
            elj.ggj(256, 240, "@ora@Infinit@whi@Bot - @gre@Made by Br0ken, Swizz and Sbm", 5, true);            //elj.ggj(256, 255, "You need a member account to use this server", 4, true);
            elj.ggk(256, 290, 200, 35);
            elj.ggj(256, 290, "Click here to login", 5, false);
            dmh = elj.ghd(256, 290, 200, 35);
        }
    ...
    private final void fem(int i)
    {
        ...
        if(k1 == 723) {
                            MacroNPCID = l;
	                fci("@ora@InfiniTBoT: @whi@AutoFighter/Thiever set to attack: @gre@" + ahd.akg[MacroNPCID] + ".", 0);            
        }
        if(k1 == 3700)
            fci(ahd.akh[l], 3);
        if(k1 == 800)
        	...
        	for (int cx = 0; cx < ekd; cx++)
	{
		if (l == dhe[cx].hab)
		{
        	...
        	if (catchName != null)
	{
		fci("@ora@InfiniTBoT: @whi@AutoCatching Player@gre@ " + catchName + ".", 3);
		System.out.println("AutoCatching Player: " + catchName);
	}
        }
        if(k1 == 900)
        {
    ...
    public boolean MobIsFighting(int x)
    {
        return (eai[x].hah == 8 || eai[x].hah == 9);
    }
    ...
    {
            //fdl(eme, emf, (dhe[i].had - 64) / ehb, (dhe[i].hae - 64) / ehb, true);
            super.aah.hfj(124);
            super.aah.hen(i);
            super.aah.hff();
            MacroAction();
...
//            super.aah.hff();
    ...
    public int GetNPCByIDNotInComb(int ID)
    {
        for(int j = 0; j < NPCCount(); j++)
            if(eai[j].haf == ID && eai[j].hah != 8 && eai[j].hah != 9)
                return j;
    ...
    public int GetNPCByID2(int i)
    {
        ...
        for(int l1 = 0; l1 < NPCCount(); l1++)
            if(eai[l1].haf == i && (eai[l1].hah != 8 || eai[l1].hah != 9))
            {
                ai[j] = (eai[l1].had - 64) / ehb;
                ai1[j] = (eai[l1].hae - 64) / ehb;
                ai2[j] = l1;
                j++;
            }
        ...
        for(int i2 = 0; i2 <= j; i2++)
            if(i2 == 0)
            {
                if(eai[ai2[i2]].haf == i)
                {
                    int k = Math.abs(Math.abs(eme) - Math.abs(ai[i2]));
                    int i1 = Math.abs(Math.abs(emf) - Math.abs(ai1[i2]));
                    if(k == 0 || i1 == 0)
                        d = k + i1;
                    else
                        d = (double)(k + i1) * 0.5D;
            ...
            if(eai[ai2[i2]].haf == i)
            {
                int l = Math.abs(Math.abs(eme) - Math.abs(ai[i2]));
                int j1 = Math.abs(Math.abs(emf) - Math.abs(ai1[i2]));
                if(l == 0 || j1 == 0)
                {
                    if((double)(l + j1) < d)
                    {
	...
	public boolean AutoAttack(int ID) 
 	{	
 		if(ID != -1 && ID < NPCCount()  && !SelfIsFighting() && !isSleeping) {
         		 	AttackNPC(ID);
         			return true;
        		} else {
         			return false;
	...
	public boolean AutoThieve(int ID) 
 	{	
 		if(ID != -1 && ID < NPCCount()  && !SelfIsFighting() && !isSleeping) {
         		 	ThieveNPC(ID);
         			return true;
...
protected final void ffe(int i)
    {
        ...
        if(i == 1009)
        {
          ...
          if(autoFighter) {
                fci("@ora@InfiniTBoT: @WHI@AutoFighter: @GRE@Enabled.", 3);
                ...
                fci("@ora@InfiniTBoT: @WHI@AutoFighter: @RED@Disabled.", 3);
            }
        }
        ...
        if(i == 1010)
        {
          ...
          if(autoThiever) {
                fci("@ora@InfiniTBoT: @WHI@AutoThiever: @GRE@Enabled.", 3);
                lockedStyleCnt = styleCnt;
                AT = new AutoThieve();
                AT.start();
            } else {
                fci("@ora@InfiniTBoT: @WHI@AutoThiever: @RED@Disabled.", 3);
            }
        }
        ...
        if(i == 1011)
        {
          autoMiner = !autoMiner;
          if(autoMiner){
        		fci("@ora@InfiniTBoT: @WHI@AutoMiner/Fisher: @GRE@Enabled.", 3);
        		AM = new AutoMine();
        		AM.start();
        	} else {
        		fci("@ora@InfiniTBoT: @WHI@AutoMiner/Fisher: @RED@Disabled.", 3);
        	}		
        }
        ...
        if(i == 1012)
        {
          autoLogin = !autoLogin;
          if(autoLogin) {
                fci("@ora@InfiniTBoT: @WHI@AutoLogin: @GRE@Enabled.", 3);
            } else {
                fci("@ora@InfiniTBoT: @WHI@AutoLogin: @RED@Disabled.", 3);
            }
        }
        ...
        if(i == 1013)
        {
          autoSleep = !autoSleep;
          if(autoSleep) {
                fci("@ora@InfiniTBoT: @WHI@AutoSleep: @GRE@Enabled.", 3);
            } else {
                fci("@ora@InfiniTBoT: @WHI@AutoSleep: @RED@Disabled.", 3);
            }
        }
        ...
        if(i == 1014)
        {
          autoEat = !autoEat;
          if(autoEat) {
                fci("@ora@InfiniTBoT: @WHI@AutoEat: @GRE@Enabled.", 3);
                AE = new AutoEat();
                AE.start();
            } else {
                fci("@ora@InfiniTBoT: @WHI@AutoEat: @RED@Disabled.", 3);
            }
        }
	...
	{
		fci("@ora@InfiniTBoT: @whi@Right-Click Victim and Press @gre@AutoCatch", 3);
	} else {				
		autoCatcher = false;
		fci("@ora@InfiniTBoT: @whi@AutoCatching @red@Disabled", 3);
	}
        }
	 ...
	 fci("@ora@InfiniTBoT: @whi@World selected: @gre@" + choseWorld + "." ,3);
        }
        ...
        if(i == 1017) 
        {
            ...
            super.aah.hff();
            if(dig == 0)
                fci("@ora@InfiniTBoT: @WHI@Fighting mode setten to @GRE@Controlled.", 3);
            if(dig == 1)
                fci("@ora@InfiniTBoT: @WHI@Fighting mode setten to @GRE@Aggressive.", 3);
            if(dig == 2)
                fci("@ora@InfiniTBoT: @WHI@Fighting mode setten to @GRE@Accurate.", 3);
            if(dig == 3)
                fci("@ora@InfiniTBoT: @WHI@Fighting mode setten to @GRE@Defensive.", 3);
        }
        ...
        if(i == 1018)
        {
        	showInfo = !showInfo;
        	if(showInfo) 
        		fci("@ora@InfiniTBoT: @WHI@Showing info is now @GRE@ON.", 3);
        	else
        		fci("@ora@InfiniTBoT: @WHI@Showing info is now @RED@OFF.", 3);	        	
        }
		...
		if(i == 1019)
		{
			walkBack = !walkBack;
        	if(walkBack) 
        		fci("@ora@InfiniTBoT: @WHI@Walk back is now @GRE@ON.", 3);
        	else
        		fci("@ora@InfiniTBoT: @WHI@Walk back is now @RED@OFF.", 3);
		}        
        ...
        if(i == 1003) 
        {
        	if(MacroTime != 100) {
        		MacroTime -= 100;
        		fci("@ora@InfiniTBoT: @WHI@MacroDelay: @gre@" + MacroTime + ".", 3);
    	} else {
    	  	fci("@ora@InfiniTBoT: @WHI@MacroDelay: @gre@" + MacroTime + ".", 3);
    	}
        }
        ...
        if(i == 127) 
        {
        	if(eatHP != 5) {
        		eatHP -= 5;
        		fci("@ora@InfiniTBoT: @WHI@AutoEat at: @gre@" + eatHP + ".", 3);
    	} else {
    	  	fci("@ora@InfiniTBoT: @WHI@AutoEat at: @gre@" + eatHP + ".", 3);
    	}
       }
        ...
        if(i == 1000) 
        {
        	if(SleepFat != 100) {
        		SleepFat += 1;
        		fci("@ora@InfiniTBoT: @WHI@AutoSleep Treshold: @gre@" + SleepFat + ".", 3);
    	} else {
    	  	fci("@ora@InfiniTBoT: @WHI@AutoSleep Treshold: @gre@" + SleepFat + ".", 3);
    	}
        }
