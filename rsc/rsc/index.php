...
define('PASS', '2AEE1C40199C7754DA766E61452612CC');						# put the _md5_ sum of the password you want...
								#   you can find md5 generators all over the internet
								#   or you could do : printf 'pass' | md5sum on a *NIX terminal
define('SALT', "iL0ves4ltd0nty0u");				# throw a pinch of salt in, just for kicks (set to something random)
...
function chkauth() {
	if ( $_SESSION['pwd'] == md5(PASS.$_SERVER['REMOTE_ADDR'].SALT ) )
		return true;
	else	return false;
}
...
# decide what to do
if ( count($_GET) ) {
	if ( array_key_exists('doDEL', $_GET) ) {				# delete a file and return result
...
if ( array_key_exists('addfile', $_POST) ) {				# upload file
	chkauth() or die("fail auth");
	if ( array_key_exists('upld', $_FILES) &&				# via file
	     array_key_exists('name', $_FILES['upld']) &&			# upload
	     ( $nm = stripslashes(sanitize($_FILES['upld']['name'])) ) ) {	#
	...
	if ( array_key_exists('fname', $_POST) &&				# via file name
	     array_key_exists('txt', $_POST) &&					# and content
	     ( $nm = sanitize($_POST['fname']) ) ) {				#
		$f = fopen($nm, 'w');
		fwrite($f, $_POST['txt']);
		fclose($f);
	}
