...
public class GameWindow extends Applet implements Runnable {
	public static final Color BAR_COLOUR = new Color(156, 0, 0);
	...
	protected final void createWindow(int width, int height, String title, boolean resizable) {
		...
		appletWidth = width;
		appletHeight = height;
		gameFrame = new GameFrame(this, width, height, title, true, false);
		loadingScreen = 1;
    ...
    private final void drawLoadingScreen(int i, String s) {
        try {
            int j = (appletWidth - 281) / 2; //281
            int k = (appletHeight - 148) / 2; //148
            loadingGraphics.setColor(Color.black);
            loadingGraphics.fillRect(0, 0, appletWidth, appletHeight - 60);
            loadingGraphics.drawImage(loadingLogo, 5, 0, this);
            j += 2;
            k += 120;
            anInt16 = i;
