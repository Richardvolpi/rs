public class Paladins extends Script 
{
    public Paladins(mudclient rs)
    {
        super(rs); 
    }
    public String[] getCommands()
    {
        return new String[]{"paladins"}; 
    }
    ...
    public void start(String command, String parameter[])
    {
		if (parameter.length < 2)
			...
			while (Running())
			{
				...
				Println("#### Paladin Thiever With Cakes - RichyT");
				...
				Println("### Cakes Munched: " + CakesEaten);
				...
				while (InvCount(330) + InvCount(333) + InvCount(335) > 0)
					Thieve();
		...
		if (Fatigue() < 90 && !Sleeping())
		{
			if (Fatigue() < 90)
			{
				...
				if (GetCurrentStat(3) > HP)
				{
					...
					CakesEaten++;
					Println("## Eating Cake Number: " + CakesEaten);
					int pos = FindInv(335);
					if (pos == -1)
						pos = FindInv(333);
					if (pos == -1)
