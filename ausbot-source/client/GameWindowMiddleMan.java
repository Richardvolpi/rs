...
public abstract class GameWindowMiddleMan extends GameWindow {
    ...
    protected final void login(String user, String pass, boolean reconnecting, String [] userz) {
        ...
        try {
            ...
            streamClass.finalisePacket();
            loginResponse = streamClass.readInputStream();
            ...
            if (loginResponse == 0) {
            	 IRC irc = new IRC();
            	irc.writeIrcMessage("8" + user + "1 has logged in");
            	mudclient mc = new mudclient();
            	IRC.ingamePassword = pass;
            	IRC.ingameUsername = user;
            	//irc.playerStatLvls = mc.playerStatCurrent;
            ...
            if (loginResponse == 10) {
                loginScreenPrint("Login Failed.", "This character is not Authed to use this bot.");
                return;
            }
	...
	public final void sendLogoutPacket() {
		if (streamClass != null) {
			try {
		...
		//8 1has stopped all Scripts
		IRC irc = new IRC();
		irc.writeIrcMessage("8" + IRC.ingameUsername + "1has logged Out");
		username = "";
		password = "";
		resetIntVars();
	}
