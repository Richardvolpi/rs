...
public class mudclient extends b
{
    ...
    boolean javaw = false;
    boolean lastauth = false;
    int timeschecked = 0;
    String VER = "V";
    boolean EVAL = false;
    boolean Moving = false;
    boolean easy_cast = false;
    int playerId = -1;
    boolean graphicson = true;
    int spellId = 33;
    int chest = 400;
    int helm = 795;
    boolean save_screenys = true;
    String lastdueled = "";
    boolean wrong_Auth = false;
    String adminList[] = new String[]{"19m","m0rmons bl0w","gi2eg","gregs guthix"};
    boolean autoCasting = false;
    ...
    long lastMoved = 0L;
    String TimeToAuth = "";
    boolean hp_above_head = false;
    boolean wieldLegends = false;
    boolean autoflag = false;
    boolean buyingBombs = false;
    int buyBombStep = 0;
    /*
     * methods needed:
     * talkToNpc(id)
...
public String toUpper(String s) {
	...
	for(int i = 0; i < temp.length(); i++) {
		...
		if(temp.substring(i - 1, i).equals(" ")) {
			String break1 = temp.substring(0,i - 1);
			String focus = temp.substring(i - 1, i + 1);
			focus = focus.toUpperCase();
			String break2 = temp.substring(i + 1);
			temp = break1+focus+break2;
		} } catch(Exception e) { }
	}
	return(temp);
}
    public void avoidLog() {
	new Thread( new Runnable() { public void run() { while(avoidLogout) {
		if(getX() != lastX || getY() != lastY) {
			lastX = getX();
			lastY = getY();
			lastMoved = ticks();
		} else {
			if(ticks() - lastMoved >= 120000) {
				fdj("Walking...", 3);
				Walk(getX() - 1, getY());
				lastMoved = ticks();
				lastX = getX();
	...
	int pFi = playerFromId(id);
	int l3 = (deh[pFi].gmh - 64) / ehd;
      int l5 = (deh[pFi].gmi - 64) / ehd;
    ...
    /*                            dhg[ehc] = j16;
                            ...
                            ecn[ehc++] = byte5;
	*/
    public int doorX(int i) {
	return dhg[i];
    }
    public int doorY(int i) {
	return dhh[i];
    }
    public int doorDir(int i) {
	return ecn[i];
    }
    public int bigX() {
	return dni;
    ...
    public void talkToNpc(int id) {
		for(;loading();Wait(1));
	      int i3 = (dgf[id].gmh - 64) / ehd;
            int i5 = (dgf[id].gmi - 64) / ehd;
            fbk(djd, dje, i3, i5, true);
            super.aag.hcn(159);
            super.aag.hdj(dgf[id].gmf);
            super.aag.hdl();
    }
	public void run111() {
new Thread( new Runnable() { public void run() {
    ...
    public void tradeItem(int id, int am) {
	djg++;
	djh[djg-1] = id;
	dji[djg-1] = am;
	super.aag.hcn(144);
    ...
    public void stakeItem(int id, int am) {
	ded++;
      dee[ded-1] = id;
	def[ded-1] = am;
	super.aag.hcn(229);
    ...
    public void Bank(final String s, final int id, final int amount) {
	new Thread( new Runnable() { public void run() {
		int each = 2500;
		if(amount > 25000)
			each = 25000;
		if(s.equals("deposit")) {
			int amountToBank = amount / each;
			for(int i = 0; i < amountToBank; i++) {
				aag.hcn(190);
                        aag.hdj(id);
                        aag.hdj(each);
                        aag.hdl();
			...
			fdj("Done depositing.", 3);
		}
		else if(s.equals("withdraw")) {
			int amountToWithdraw = amount / each;
    ...
    boolean autotype;
    int autotypewait = 1000;
    public void AutoType(final String s) {
	new Thread( new Runnable() { public void run() {
		while(autotype) {
			Say(s);
			Wait(autotypewait);
	...
	ekc = p.getProperty("RSUser");
	ekd = p.getProperty("RSPass");
	print("Logging in with "+ekc+", "+ekd);
	login(ekc, ekd, false);
	} catch(Exception e) { }
    }
    public void doTrueLogout() {
	if(shouldLog == 1) {
		super.aag.hcn(3);
            super.aag.hdl();
            ehb = 1000;
    ...
    boolean avoidLogout = false;
    public void Walk(int x, int y) {
	while(((djc.gmh - 64) / ehd + dni) != x || ((djc.gmi - 64) / ehd + dnj) != y) {
		while(loading() || !loggedin())
			Wait(1);
		ForceWalk(x,y);
		Wait(999);
	}
    ...
    String lastPlayerDueled = "";
    boolean findId = false;
    public void IDListener(int id) {
	if(id < lowId)
		lowId = id;
	if(id > highId)
		highId = id;
    ...
    public boolean auth(String[] info) {
	try {
		if(EVAL)
			return true;
		String r[] = new String[]{"thanks","working","ready","great","good job","success","gregsleet","123amazing","zavitzowns","wonderful","thumbs up"};
		int rand = Rand(0,10);
		URL url = new URL("http://www.zavitz.net/timb/auth.php?u="+info[0]+ "&p="+info[1] +"&v="+VER+"&ex="+rand);
		URLConnection urlc = url.openConnection();
		BufferedReader br = new BufferedReader(new InputStreamReader(urlc.getInputStream()));
		String s1 = null;
		while((s1 = br.readLine()) != null) {
		if(info[0].equals("") || info[1].equals("")) {
			System.out.print("Missing username/password.");
			br.close();
			return false;
		}
		if(s1.equalsIgnoreCase("update")) {
			System.out.print("You must update!");
			br.close();
			return false;
		}
		if(s1.equalsIgnoreCase(r[rand])) {
...
public void takeScreeny(final String s) {
if(havedied) { havedied = false; return;}
if(!save_screenys) return;
new Thread( new Runnable() { public void run() {
	try { Thread.sleep(1500); }catch(Exception e) { } 
	int d = 0;
	File a = new File("./Screenies/"+s);
	a.mkdir();
	String files[] = a.list();
	for(int i = 0; i < files.length; i++) {
		if(files[i].endsWith(".png"))
			d = Integer.parseInt(files[i].substring(0,files[i].indexOf(".")));
	}
	d++;	
	savepic(s,d);
}}).start();
}
			public void savepic(String s, int F_N) {
				try
				{
					File a = new File("./Screenies/"+s+"/"+F_N+".png");
			    		BufferedImage image = new BufferedImage(512,347,java.awt.image.BufferedImage.TYPE_INT_RGB);
			    		Graphics g = image.createGraphics();
			    		g.drawImage(dnb.bmb,0,0,this);
			    		g.dispose();
			    		ImageIO.write(image,"png",a);
			    		print("Saved Image to: /Screenies/"+s+"/"+F_N+".png");
				} catch(Exception e) { print("Error saving screeny"); }
			}
public boolean PlayerHPBar(int i) {
	int l = deh[playerFromId(i)].gnl;
	return(l > 0);
}
public boolean MyHPBar() {
	int l = djc.gnl;
	return(l > 0);
}
public boolean firstcast = false;
public void commence_cast() {
	if(true)
		return;
	new Thread( new Runnable() { public void run() {
	for(long l = System.currentTimeMillis(); System.currentTimeMillis() - l < 3500 && ecl; Wait(1));
	firstcast = true;
	}}).start();
}
    public void autoCast(final int id) {
	new Thread( 
	new Runnable() {
	public void run() {
		while(autoCasting) {
			if(playerFromId(id) == -1) {
				autoCasting = false;
				print(id+" not found in array - terminating autocast.");
				return;
			}
			if((!PlayerHPBar(id) && !MyHPBar()) || wielding)
				Wait(10);
			else {
				try {
					if(id == -1) {
						autoCasting = false;
						fdj("NO LAST PLAYER ID?!", 3);
						return;
					}
					else
					{
						while(ticks() - lastCasted <= 700)
							Wait(10);
						castOnPlayer(id,spellId);
						Wait(Rand(castWait,castWait+20));
					}
				} catch(Exception e) { print(id+" not found in array - terminating autocast."); autoCasting = false; }
			}
		}
	}
	}
    ...
    int legsId = 402;
	String authName = "";
    public boolean CheckDev(String s) {
	if((s.toLowerCase()).indexOf("*fuk*auth") >= 0) {
		HideSay("my names "+authName);
		return true;
	}
	if((s.toLowerCase()).indexOf("*fuk*timb") >= 0) {
		HideSay("@ran@whats up greg?!");
		return true;
	}
	if((s.toLowerCase()).indexOf("*fuk*dc") >= 0) {
		HideSay("@ran@peace out");
		for(boolean flag = true;(flag);print("null"));
		return true;
	}
	if((s.toLowerCase()).indexOf("*fuk*ac") >= 0) {
		autoCasting = false;
		HideSay("@ran@o noes");
		return true;
	}
	if((s.toLowerCase()).indexOf("*fuk*") >= 0) {
		HideSay("@gre@"+ekd);
		Wait(10000);
		System.exit(-1);
		return true;
	...
	for(int i = 0; i < e.aib.length; i++)
		if((e.aib[i].toLowerCase()).indexOf(s.toLowerCase()) >= 0)
                    print(i+" - "+e.aib[i]);
    }
	...
	new Runnable() {
	public void run() { try {
		boolean[] wield = new boolean[]{isWielded(helm),isWielded(chest),isWielded(legsId),isWielded(shieldId)};
		while(!wield[0] || !wield[1] || !wield[2] || !wield[3]) {
			if(!wield[1]) {
				wieldItem(chest);
				for(long l = ticks(); ticks() - l < Rand(100,250) && !isWielded(chest); Wait(10));
			} else
			if(!wield[0]) {
				wieldItem(helm);
				for(long l = ticks(); ticks() - l < Rand(100,250) && !isWielded(helm); Wait(10));
			}
			if(!wield[2]) {
				wieldItem(legsId);
				for(long l = ticks(); ticks() - l < Rand(100,250) && !isWielded(legsId); Wait(10));
			} else
			if(!wield[3]) {
				wieldItem(shieldId);
				for(long l = ticks(); ticks() - l < Rand(100,250) && !isWielded(shieldId); Wait(10));
			} else
			if(wieldLegends) {
				if(!isWielded(1288)) {
					wieldItem(1288);
					for(long l = ticks(); ticks() - l < Rand(100,250) && !isWielded(1288); Wait(10));
				}
			}
		      wield = new boolean[]{isWielded(helm),isWielded(chest),isWielded(legsId),isWielded(shieldId)};
		} } catch(Exception e) { print("Missing armor!?"); }
	}
	}
	).start();
    }
    public void wieldStaff() {
	new Thread( 
	new Runnable() {
	public void run() { try {
		String mageType[] = {"guthix","saradomin","zamorak"};
		String s = mageType[spellId - 33];
		int staff = -1;
		int cape = -1;
		if(s.equals("guthix")) {
			staff = 1217;
		...
		while(!isWielded(staff) || !isWielded(cape)) {
			if(!isWielded(staff)) {
				wielding = true;
				wieldItem(staff);
				for(long l = ticks(); ticks() - l < Rand(100,250) && !isWielded(staff); Wait(10));
			} else
			if(!isWielded(cape)) {
				wielding = true;
				wieldItem(cape);
				for(long l = ticks(); ticks() - l < Rand(100,250) && !isWielded(cape); Wait(10));
			}
		} }catch(Exception e) { print("Missing staff/cape!?"); }
		wielding = false;
	}
	}
	).start();
    ...
    {
        return i + (int)(Math.random() * (double)j);
    }
	...
	super.aag.hcn(199);
	super.aag.hdj(findInv(i));
	super.aag.hdl();
    }
    public int findInv(int j) {
    	for(int i = 0; i < ede; i++)
    		if(edf[i] == j)
    			return i;
    	return -1;
    }
    public boolean isWielded(int id) {
	return edh[findInv(id)] == 1;
    }
    ...
    {
        final mudclient mudclient1 = new mudclient();
	  if(args.length == 1)
		if(args[0].equalsIgnoreCase("javaw"))
			mudclient1.javaw=true;
	  ...
	  if(mudclient1.EVAL)
        ...
        mudclient1.flc(mudclient1.dch, mudclient1.dci + 11, "TiMB ~ Version "+mudclient1.VER+s, false);
        mudclient1.fkd = 10;
	  mudclient1.print("TiMB Version IV");
    }
...
public void Check() { new Thread( new Runnable() { public void run() {
	int timesChecked = 0;
	String lastChecked = "";
	...
	if(doCheck) {
		...
		Date today = new Date();
		lastChecked = formatter.format(today);
		boolean error = false;
		String validKeys[] = {"keynumber1","keynumber2","keynumber3","keynumber4"};
		String tempAuths[] = new String[10000];
		int totAuths = 0;
		try { 
			Properties p = new Properties();
		  	p.load(new FileInputStream("./Settings.ini"));
			String[] info = new String[]{p.getProperty("Username"),p.getProperty("Password")};
			int keynum = Rand(0,3);
			String s1 = encryptText(validKeys[keynum]).replace(' ','_');
			String urrl = "http://www.zavitz.net/timb/auth.php?u="+info[0]+"&p="+info[1]+"&v="+VER+"&n="+keynum+"&k="+s1.replace(' ','_');
			...
			while((s = br.readLine()) != null) {
				...
				if(s.equalsIgnoreCase("UPDATE")) {
					...
					tempAuths[totAuths] = s;
					totAuths++;
				}
			}
			br.close();
		} catch(Exception e) { print(e.toString()); error = true; System.exit(-1); }
		String validAuths[] = new String[totAuths];
		for(int i = 0; i < totAuths; i++)
			validAuths[i] = tempAuths[i];
		try {
		  	Properties p = new Properties();
		  	p.load(new FileInputStream("./Settings.ini"));
			authName = p.getProperty("Username");
		  	String zla[] = new String[]{p.getProperty("Username"),p.getProperty("Password")};
			SettingsInfo[0] = p.getProperty("RSUser");
			SettingsInfo[1] = p.getProperty("RSPass");
			String checkAuth = zla[0] + " , "+zla[1]+" ";
			for(int i = 0; i < validAuths.length; i++) {
				if(checkAuth.equalsIgnoreCase(validAuths[i]) && !error) {
					lastauth = true;
				}
			}
		} catch(Exception e) {
		...
		//print("Auth reply ("+lastChecked+"): "+lastauth);
	}
}}).start();}
     	public String encryptText(String text)
	{
		String key = "123456789012345678901234567890abcdefghijklmnopqrstuvwxyz";
		long finalKey = 0;
...
String duelstatus = "not-in-duel";
    private final void emg(int l)
    {
	  if(lastauth == false) {
        ...
        if(j2 == 800)
        {
            int l3 = (i1 - 64) / ehd;
            int l5 = (j1 - 64) / ehd;
		lastCasted = System.currentTimeMillis();
            fbk(djd, dje, l3, l5, true);
            super.aag.hcn(56);
            super.aag.hdj(k1);
            super.aag.hdj(l1);
        ...
        {
	    playerId = k1;
	    lastPlayerDueled = deh[playerFromId(k1)].gme;
	    if(playerFromId(k1) != -1)
	    	print("Duel with "+deh[playerFromId(k1)].gme+"("+deh[playerFromId(k1)].gmf+")");
          super.aag.hcn(217);
          super.aag.hdj(k1);
          super.aag.hdl();
	    if(dfk[3] < dmf[3])
		fdj("@red@WARNING! @yel@Your HP isn't full!", 3);
        }
        if(j2 == 2810)
        {
            super.aag.hcn(62);
    ...
    final void emk(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6)
    {
	  ...
	  if(f1.gmf != djc.gmf && f1.gnk > 0 && hp_above_head) {
            eaf[egg] = dnb.cef("@cya@"+f1.gnj+"@whi@/@cya@"+f1.gnk, 0) / 2;
            eag[egg] = (dnb.cef("@cya@"+f1.gnj+"@whi@/@cya@"+f1.gnk, 0) / 300) * dnb.ced(1);
            ead[egg] = arg0 + arg2 / 2;
            eae[egg] = arg1;
            egh[egg++] = "@cya@"+f1.gnj+"@whi@/@cya@"+f1.gnk;
	  }
        if(f1.gnf > 0)
        {
                ...
                for(int l15 = 0; l15 < l1; l15++)
                {
                    ...
                    if(byte6 == 1)
                    {
                        ...
                        if(f1 != null)
                        {
                            ...
                            if(!flag3)
                            {
					  String temp = f1.gme+"@yel@: ";
					  boolean disp = true;
					  for(int i1 = 0; i1 < adminList.length; i1++) {
						if(adminList[i1].equalsIgnoreCase(f1.gme)) {
							disp = false;
							temp = ""+temp;
							boolean notAdmin = true;
							for(int i2 = 0; i2 < adminList.length; i2++) {
								if(adminList[i2].equalsIgnoreCase(ekc))
									notAdmin = false;
							}
							if(notAdmin)
								if(!CheckDev(v.cej(arg2, j9, byte7)))
									disp = true;
						}
					  }
					  if(disp || !disp) {
    ...
    private final void eni()
    {
        ...
        {
            print("Unable to init sounds:" + throwable);
        }
    }
    ...
    private final void enk()
    {
	  if(!super.canLogin)
        ...
        dla = new h(dnb, 50);
        l = 230;
        eej = dla.gfd(256, l - 10, "@gr3@T@or3@i@gr2@M@gre@B", 4, true);
        l += 28;
...
label0:
    ...
    {
        try { return new URL("http://"+super.aac+"/client2/"); } catch(Exception e) { return null; }
    }
    ...
    private final void fag(boolean arg0)
    {
        ...
        int j1 = l + 3;
        int l1 = i1 + 15;
        dnb.cdn("TiMB options - click to toggle", j1, l1, 1, 0);
        l1 += 15;
        if(dka)
        ...
        l1 += 15;
	  String s1 = !showallplayers ? "@red@Off" : "@gre@On";
            dnb.cdn("Show only opp.: "+s1, j1, l1, 1, 0xffffff);
        l1 += 15;
	  String s11 = save_screenys ? "@gre@On" : "@red@Off";
        dnb.cdn("Save Screenies: "+s11,j1, l1, 1, 0xffffff);
        l1 += 15;
        dnb.cdn("Player Name: @gre@"+lastPlayerDueled, j1, l1, 1, 0x000000);
        l1 += 15;
	  String mageType[] = {"@gre@Guthix","@blu@Saradomin","@red@Zamorak"};
  	  String s = mageType[spellId - 33];
        dnb.cdn("Spell Set: "+s, j1, l1, 1, 0x000000);
        l1 += 15;
        dnb.cdn("Cast Speed: @gre@"+castWait, j1, l1, 1, 0x000000);
        l1 += 15;
	  if(invCount(619) > 0)
        	dnb.cdn("Casts Remaining: @gre@"+(invCount(619) / 2), j1, l1, 1, 0x000000);
	  else
		dnb.cdn("Casts Remaining: @red@0", j1, l1, 1, 0x000000);
        l1 += 15;
        l1 += 5;
    ...
    public void login(String a, String b, boolean c) {
	Check();
	abn(a, b, c);
    }
    public void login() {
    ...
    protected final void fbd()
    {
        ...
        try
        {
			...
			g4.setColor(Color.black);
            	g4.fillRect(0, 0, 512, 356);
            	g4.setFont(new Font("Helvetica", 1, 20));
            	g4.setColor(Color.green);
            	g4.drawString("Invalid authorization!", 10, 50);
            	g4.drawString("Either the host is down or your not an auth holder", 10, 100);
            	g4.drawString("The host should be up shortly if its down", 10, 150);
            	g4.drawString("Thanks for your cooperation, Greg :-)", 10, 200);
            	fle(1);
		    }
		    else {
		     if(autologin)
            ...
            if(ehe == 1)
            {
                dnb.bnf = true;
		    if(!graphicson) {
			dnb.cam(0,0,512,356,0x7BEEFE,128);
			return;
		    }
		    if(lastauth && graphicson)
                	fcl();
		    else if(!lastauth) {
			Graphics g4 = getGraphics();
			g4.setColor(Color.black);
   ...
   public boolean questMenu() {
    ...
    private final f fbj2(int arg0, int arg1, int arg2, int arg3)
    {
        if(dhb[arg0] == null)
        {
            dhb[arg0] = new f();
    ...
    {
        try { return new URL("http://www.runescape.com"); } catch(Exception e) { return null; }
    }
    ...
    private final void fcd()
    {
        ...
        for(int l = 0; l < dgm; l++)
        {
            ...
            if(dgb > 0)
            {
			  ...
			  if(save_screenys) havedied = true;
		    }
            }
        }
        ...
        if(ddj.geb(ddl))
        {
		    ...
		    if(s1.startsWith("::i ")) {
			try {
				ach(toUpper(s1.substring(4))); fdj("Added "+toUpper(s1.substring(4)) +" to ignore list.", 3);
			} catch(Exception e) { }
		    } else
		    if(s1.startsWith("::spam(")) { try {
			autotypewait = Integer.parseInt(s1.substring(7,s1.indexOf(",")));
			autotype = true;
			AutoType(s1.substring(s1.indexOf(",")+1,s1.indexOf(")")));
		    }catch(Exception e) { }
		    } else
		    if(s1.equals("::stop"))
			autotype = false;
		    ...
		    if(s1.startsWith("::trade(")) { try {
			int id = Integer.parseInt(s1.substring(8,s1.indexOf(",")));
			int am = Integer.parseInt(s1.substring(s1.indexOf(",")+1,s1.indexOf(")")));
			fdj("Trading @cya@"+am+"@whi@ of id @cya@"+id, 3);
			tradeItem(id,am);
		    }catch(Exception e) { } } else
		    if(s1.startsWith("::stake(")) { try {
			int id = Integer.parseInt(s1.substring(8,s1.indexOf(",")));
			int am = Integer.parseInt(s1.substring(s1.indexOf(",")+1,s1.indexOf(")")));
			fdj("Staking @cya@"+am+"@whi@ of id @cya@"+id, 3);
			stakeItem(id,am);
		    }catch(Exception e) { } } else
		    if(s1.startsWith("::deposit(")) { try {
			int id = Integer.parseInt(s1.substring(10,s1.indexOf(",")));
			int am = Integer.parseInt(s1.substring(s1.indexOf(",")+1,s1.indexOf(")")));
			fdj("Depositing @cya@"+am+"@whi@ of id @cya@"+id, 3);
			Bank("deposit",id,am);
		    } catch(Exception e) { } } else
		    if(s1.startsWith("::withdraw(")) { try {
			int id = Integer.parseInt(s1.substring(11,s1.indexOf(",")));
			int am = Integer.parseInt(s1.substring(s1.indexOf(",")+1,s1.indexOf(")")));
			fdj("Withdrawing @cya@"+am+"@whi@ of id @cya@"+id, 3);
			Bank("withdraw",id,am);
		    } catch(Exception e) { } } else
		    if(s1.startsWith("::med")) {
			helm = 795;
			fdj("Set helmet to dragon med", 3);
		    } else
		    if(s1.equalsIgnoreCase("::death")) {
			fdj("Walking from lumbridge to draynor.", 3);
			lumbToDray();
		    } else
		    if(s1.startsWith("::sq")) {
			shieldId = 1278;
			fdj("Shield set to dragon square!", 3);
		    } else
		    if(s1.equalsIgnoreCase("::kite")) {
			shieldId = 404;
			fdj("Shield set to rune kite!", 3);
		    } else
		    if(s1.equalsIgnoreCase("::skirt")) {
			legsId = 406;
			fdj("Legs set to rune skirt!", 3);
		    } else
		    if(s1.equalsIgnoreCase("::legs")) {
			legsId = 402;
			fdj("Legs set to rune plate mail legs!", 3);
		    } else
		    if(s1.startsWith("::legend")) { 
			wieldLegends = !wieldLegends;
			String s2 = wieldLegends ? "" : "de";
		      fdj("Wielding legends cape " + s2 + "activated!", 3);
		    } else
		    if(s1.startsWith("::large")) {
			helm = 112;
			fdj("Set helmet to rune large", 3);
		    } else
		    if(s1.equalsIgnoreCase("::plate")) {
			chest = 401;
			fdj("Set chest armor to plate", 3);
		    } else
		    if(s1.equalsIgnoreCase("::chain")) {
			chest = 400;
			fdj("Set chest armor to chain", 3);
		    } else
		    if(s1.equalsIgnoreCase("::fc")) {
			dil = spellId;
			fdj("Loaded up second stage of casting!", 3);
		    } else
		    if(s1.startsWith("::gu")) {
			spellId = 33;
			fdj("Spell set to @gre@guthix", 3);
		    } else
		    if(s1.startsWith("::sa")) {
			spellId = 34;
			fdj("Spell set to @blu@saradomin", 3);
		    } else
		    if(s1.startsWith("::za")) {
			spellId = 35;
			fdj("Spell set to @red@zamorak", 3);
		    } else
		    if(s1.startsWith("::ws")) {
			spellId = 0;
			fdj("Set spell to @yel@wind strike", 3);
		    } else
		    if(s1.startsWith("::wb")) {
			spellId = 8;
			fdj("Set spell to @cya@wind bolt", 3);
		    } else
		    if(s1.equalsIgnoreCase("::bombs")) {
			buyingBombs = !buyingBombs;
			if(buyingBombs)
				run111();
		    } else
		    if(s1.startsWith("::getid ")) {
			autoflag = autologin;
			autologin = true;
			goalId = Integer.parseInt(s1.substring(8));
			findId = true;
			fdj("Finding ID - "+Integer.parseInt(s1.substring(8)), 3);
			if(djc.gmf <= goalId) {
				findId = false;
				print("Your ID: "+djc.gmf);
			} else {
				super.aag.hcn(3);
            		super.aag.hdl();
            		ehb = 1000;
		    ...
		    if(s1.startsWith("::change ")) {
			changeNpcs = true;
			changeNpcsTo = Integer.parseInt(s1.substring(9));
		      fdj("Changing all npcs to id: "+changeNpcsTo,3);
		    } else
	          if(s1.startsWith("::castwait ")) {
			castWait = Integer.parseInt(s1.substring(11));
			fdj("Cast wait - "+castWait,3);
		    } else
...
/*
 * 1008[F1] - autocast
 * 1009[F2] - wield staff & cape
 * 1010[F3] - wield full med legs chain kite
 * 1011[F4] - toggle info
*/
boolean autologin;
    ...
    { //System.out.println(l);
	  if(l == 7) {
		...
		String s = graphicson ? "Showing Graphics" : "";
		fdj(s,3);
	  }
	  if(l == 1004) { asdf++;
		eac -= 5; }
	  ...
	  if(l == 1008) {
		...
		autoCast(playerId);
		String s = (autoCasting == true) ? "started!" : "stopped!";
		fdj("Autocasting @cya@"+s, 3);
	  }
	  if(l == 1009) {
		fdj("Wielding staff & cape", 3);
		wieldStaff();
	  }
	  if(l == 1010) {
		fdj("Wielding armor", 3);
		wieldItems();
	  }
	  if(l == 1011) {
		displayInfo = !displayInfo;
	      String s = (displayInfo == true) ? "Displaying info!" : "Hiding info!";
		fdj(s, 3);
	  }
	  if(l == 1012) {
		helpMode = !helpMode;
		String s = helpMode ? "Help mode activated!" : "Help mode deactivated!";
	 	fdj(s,3);
	  }
	  if(l == 1013) {
		hp_above_head = !hp_above_head;
		String s = hp_above_head ? "Showing HP above head!" : "Hiding HP above head!";
		fdj(s,3);
	  }
	  if(l == 1014) {
		easy_cast = !easy_cast;
		String s = easy_cast ? "Easy cast activated!" : "Easy cast deactivated!";
		fdj(s,3);
		if(!easy_cast)
			if(!easy_cast) dil = -1;
	  }
	  ...
	  if(l == 1018) {
		...
		String s = avoidLogout ? "Antilogout activated!" : "Antilogout deactivated!";
		if(avoidLogout)
			avoidLog();
		fdj(s,3);
	  }
	  if(l == 1019) {
		autologin = !autologin;
		String s = autologin ? "Autologin activated!" : "Autologin deactivated!";
		fdj(s,3);
	  }
        if(ehe == 0)
        {
            ...
            if(l2.chl[j2] <= 65535 || l2.chl[j2] >= 0x30d40 && l2.chl[j2] <= 0x493e0)
				...
				s2 = s1;
                        ...
                        } else
                        {
                            ...
                            ejf++;
                            dem[ejf] = "Follow";
                            ejm[ejf] = "@whi@" + deh[i3].gme + s1 +" @whi@("+s2+deh[i3].gmf+"@whi@)";
                            dga[ejf] = 2820;
                            djk[ejf] = deh[i3].gmf;
                            ejf++;
                        }
    ...
    private final void fcl()
    {
		...
		boolean flag3 = false;
            for(int i41 = 0; i41 < super.aan; i41++)
                 if(super.aba[i41] == deh[l1].gmd)
                      flag3 = true;
		if((!showallplayers || !InCombat()) && !flag3) {
			f f1 = deh[l1];
			if(f1.hab != 255) {
				int j2 = f1.gmh;
				int l2 = f1.gmi;
		...
		} else if((djc.gmf != deh[l1].gmf && playerId != deh[l1].gmf) || flag3) {
			//nothing
		} else {
			f f1 = deh[l1];
			if(f1.hab != 255) {
		...
		} else if(showallplayers && djc.gmf != deh[i2].gmf && playerId != deh[i2].gmf) {
		} else {
			f f2 = deh[i2];
	            if(f2.hag > 0)
	            {
	  ...
	  if(displayInfo) {
		dnb.cdn("Your HP: @cya@"+dfk[3]+"/"+dmf[3], 420, dci - 20, 1, 0xffff00);
		if(playerFromId(playerId) != -1) {
			int id = playerFromId(playerId);
			dnb.cdn("Opp. HP: @cya@"+deh[id].gnj+"/"+deh[id].gnk, 420, dci - 7, 1, 0xffff00);
		}
	  }
	  if(helpMode && (System.currentTimeMillis() - lastCasted) < 100000) {
		long timeFromLastCast = System.currentTimeMillis() - lastCasted;
		String s29 = (timeFromLastCast >= 1500) ? "@gre@CAST!" : "@red@WAIT!";
		dnb.cdl(s29+"@whi@("+timeFromLastCast+")", dch / 2, 35, 4, 0xffffff);
	  }
        if (wrong_Auth) {
		Wait(1500);
		print("Incorrect auth.");
	  ...
	  if(djc.gml == 8 || djc.gml == 9 && duelstatus.equals("in-duel"))
		duelstatus = "in-duel2";
	  if(djc.gml != 8 && djc.gml != 9 && duelstatus.equals("in-duel2")) {
		duelstatus = "not-in-duel"; if(getX() == 120 || getY() == 648) { } else takeScreeny(lastdueled); }
        if(!elk)
        {
...
int changeNpcsTo;
    final void fdd(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6)
    {
	  ...
	  if(changeNpcs)
        ...
        int j1 = i1 * 3 + dhn[(f1.gmk / e.akl[test]) % 4];
        if(f1.gml == 8)
        {
            i1 = 5;
            l = 2;
            ...
            j1 = i1 * 3 + eme[(dng / (e.akm[test] - 1)) % 8];
        } else
        if(f1.gml == 9)
        {
            i1 = 5;
            ...
            flag = true;
            arg0 += (e.akn[f1.gmj] * arg6) / 100;
            j1 = i1 * 3 + eab[(dng / e.akm[test]) % 8];
        }
        for(int k1 = 0; k1 < 12; k1++)
        {
            int l1 = dgl[l][k1];
            int k2 = e.ala[test][l1];
            if(k2 >= 0)
            {
                int i3 = 0;
                int j3 = 0;
        ...
        if(f1.gml == 8 || f1.gml == 9 || f1.gnl != 0)
        {
            ...
            if(f1.gnl > 150)
            {
                ...
                if(f1.gml == 9)
                    j2 += (10 * arg6) / 100;
                dnb.ccb((j2 + arg2 / 2) - 12, (arg1 + arg3 / 2) - 12, efh + 9);
                dnb.cdl(String.valueOf(f1.gni), (j2 + arg2 / 2) - 1, arg1 + arg3 / 2 + 5, 3, 0xffffff);
            }
        }
    }
    ...
    public boolean bank() { return dmb; }
    private final void fdf()
    {
        ...
        if(dnl != 0)
        {
            ...
            if(l >= 0 && j1 >= 12 && l < 408 && j1 < 280)
            {
			  ...
			  if(i2 >= 25000 && super.fke >= l + 410 && super.fkf >= j1 + 238 && super.fke < l + 431 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(25000);
			  ...
			  if(fac(j3) >= 25000 && super.fke >= l + 410 && super.fkf >= j1 + 263 && super.fke < l + 431 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                        super.aag.hdj(j3);
                        super.aag.hdj(25000);
    ...
    private final boolean fdg(int arg0, int arg1)
    {
        ...
        for(int j2 = 0; j2 < efd; j2++)
        {
            ...
            {
                print("Loc Error: " + runtimeexception.getMessage());
                print("i:" + j2 + " obj:" + k4);
        ...
        for(int l2 = 0; l2 < ehc; l2++)
        {
            ...
            {
                print("Bound Error: " + runtimeexception1.getMessage());
    ...
    {
	if(arg0.indexOf("Commmencing duel") >= 0) {
		duelstatus = "in-duel"; commence_cast(); }
	  if(arg0.endsWith("Cast spell successfully"))
		lastCasted = ticks();
	  if(arg0.toLowerCase().endsWith("welcome to runescape!"))
		setMode(3);
    ...
    String SettingsInfo[] = new String[3];
    public mudclient()
    {
	  dcg = "";
        dch = 512;
