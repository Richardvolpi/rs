...
public final class mudclient extends aaa
{
    ...
    private final void enf(boolean flag)
    {
        ...
        i1 += 15;
        if(ecg)
            dng.ceg("Camera angle mode - @gre@Auto", k, 66, 1, 0xffffff);
        else
        ...
        i1 += 15;
        dng.ceg("To change your contact details,", k, 111, 0, 0xffffff);
        i1 += 15;
        dng.ceg("password, recovery questions, etc..", k, 126, 0, 0xffffff);
        i1 += 15;
        dng.ceg("please select 'account management'", k, 141, 0, 0xffffff);
        i1 += 15;
        if(deb == 0)
            dng.ceg("from the runescape.com front page", k, 156, 0, 0xffffff);
        else
        if(deb == 1)
            dng.ceg("from the link below the gamewindow", k, 156, 0, 0xffffff);
        else
            dng.ceg("from the runescape front webpage", k, 156, 0, 0xffffff);
        i1 += 15;
        i1 += 5;
    ...
    private final void enh()
    {
        ...
        i += 26;
        dng.cee("If you go much further north you will enter the", 256, 123, 1, 0xffffff);
        i += 13;
        dng.cee("wilderness. This a very dangerous area where", 256, 136, 1, 0xffffff);
        i += 13;
        dng.cee("other players can attack you!", 256, 149, 1, 0xffffff);
        i += 22;
        dng.cee("The further north you go the more dangerous it", 256, 171, 1, 0xffffff);
        i += 13;
        dng.cee("becomes, but the more treasure you will find.", 256, 184, 1, 0xffffff);
        i += 22;
        dng.cee("In the wilderness an indicator at the bottom-right", 256, 206, 1, 0xffffff);
        i += 13;
        dng.cee("of the screen will show the current level of danger", 256, 219, 1, 0xffffff);
        i += 22;
        int j = 0xffffff;
...
label0:
    ...
    private final void faa()
    {
        ...
        for(int i = 0; i < dka; i++)
        {
            ...
            if(ecj > 0)
            {
                ecj--;
                if(ecj == 0)
                    enl("You have been granted another life. Be more careful this time!", 3);
                if(ecj == 0)
                    enl("You retain your skills. Your objects land where you died", 3);
            }
        }
    ...
    private final void fbf()
    {
        ...
        if(!ech)
        {
            dln.ggj(256, 240, "Click on an option", 5, true);
        ...
        {
            dln.ggj(256, 240, "Welcome to RuneScape", 4, true);
    ...
    private final void fbj()
    {
        ...
        j += 20;
        dng.cee("Welcome to RuneScape " + eld, 256, j, 4, 0xffff00);
        j += 30;
        String s;
        if(dik == 0)
            s = "earlier today";
                ...
                if(cgc1 == egn.bem)
                {
                    ...
                    if(l2 == 3)
                    {
                        ...
                        } else
                        {
                            ...
                            {
                                ehb[emc] = "Attack";
                                eii[emc] = "@yel@" + ahd.amn[dif[i2].haf] + s1;
                                if(l3 >= 0)
                                    dne[emc] = 715;
                                else
                                    dne[emc] = 2715;
    ...
    private final void fdh()
    {
        ...
        {
            dng.cbm();
            dng.cee("Oh dear! You are dead...", elh / 2, eli / 2, 7, 0xff0000);
            fcl();
            dng.cbc(dil, 0, 0);
            return;
        }
        ...
        if(ddj)
        {
            ...
            if(Math.random() < 0.14999999999999999D)
                dng.cee("ZZZ", (int)(Math.random() * 80D), (int)(Math.random() * 334D), 5, (int)(Math.random() * 16777215D));
            if(Math.random() < 0.14999999999999999D)
                dng.cee("ZZZ", 512 - (int)(Math.random() * 80D), (int)(Math.random() * 334D), 5, (int)(Math.random() * 16777215D));
    ...
    private final void fdl(boolean flag)
    {
        ...
        if(eji == 0)
        {
            ...
            i1 += 12;
            dng.ceg("Fatigue: @yel@" + (eja * 100) / 750 + "%", i + 5, i1 - 13, 1, 0xffffff);
            i1 += 8;
            ...
            } else
            {
                ...
                i1 += 12;
                dng.ceg("Combat level: " + ejk.hbi, i + 5, i1, 1, 0xffffff);
                i1 += 12;
            }
        }
        if(eji == 1)
    ...
    private final void fee()
    {
        ...
        dng.cee("Controlled (+1 of each)", 94, 51, 3, 0);
        dng.cee("Aggressive (+3 strength)", 94, 71, 3, 0);
        dng.cee("Accurate   (+3 attack)", 94, 91, 3, 0);
        dng.cee("Defensive  (+3 defense)", 94, 111, 3, 0);
    }
