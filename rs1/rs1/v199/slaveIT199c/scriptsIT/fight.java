...
public class fight extends Macro {
    
    public fight(slave rs) {
        super(rs);
    }
    ...
    public String[] getComments() {
        return new String[]{"Usage: /fight name fightmode healathp   Start at walkback location"};
    }
        public void kill() {
            while(loggedin()) {
            rs.autologin = false;
	...
	int hptoheal = Integer.parseInt(arg[2]);
			...
			try{
				...
				//System.err.println("'" + NpcName(g) + "' '" + nN + "'");
				if(curDist < Dist && NpcName(g).equalsIgnoreCase(nN) && !isNpcInCombat(g) && isNpcAttackable(g)) {
				    Dist = curDist;
				    fin = g;
				}
			    }
			...
			if(fin != -1) {
                ...
                if(!inCombat() && !isNpcNearByName(nN) && (myX() != startX || myY() != startY)) {
