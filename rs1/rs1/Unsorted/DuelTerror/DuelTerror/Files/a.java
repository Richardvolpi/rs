    ...
    implements Runnable
{
    ...
    {
        fii = -2;
        System.out.println("BoT Closing");
        ena();
        try
        {
            Thread.sleep(1000L);
    ...
    private final void flh()
   {
       ...
       fjh.setColor(Color.green);


       int i = (fia - 281) / 2; // 281
       int k = (fib - 148) / 2; // 148
       fjg = toolkit.getImage(this.userDir + "./Logo.jpg"); 
    ...
    private final void fli(int arg0, String arg1)
    {
        try
        {
            ...
            if(!fja)
                fjh.drawImage(fjg, 40, 35, this); // 60 IS THE X, AND 35 IS THE Y FROM THE TOP OF THE JAVA APPLET // fjg,60,35,this
            i += 2;
            k += 90;
            fjb = arg0;
            fjc = arg1;
            fjh.setColor(Color.yellow); // boarder color (red)
            fjh.drawRect(i - 2, k - 2, 280, 23);
            fjh.fillRect(i, k, (277 * arg0) / 100, 20);
            fjh.setColor(Color.yellow); //Text color at bottom of frame
            if(fja)
                fjh.setColor(Color.yellow);
            flk(fjh, arg1, fjd, i + 138, k + 10);
            if(!fja)
            {
                flk(fjh, "Spook : By {{zym}oti}", fje, i + 138, k + 30);
                flk(fjh, "\251 2004 - 2004 ", fje, i + 138, k + 44);
            } else
            {
                fjh.setColor(new Color(132, 132, 152));
                flk(fjh, "", fjf, i + 138, fib - 20);
            }
            if(fin != null)
            {
                fjh.setColor(Color.orange);
    ...
    protected final void flj(int i, String s)
    {
        try
        {
            ...
            fjc = s;
            int i1 = (277 * i) / 100;
            fjh.setColor(Color.blue);
            if(fja)
                fjh.setColor(Color.yellow); //over bar colour
            fjh.fillRect(k, l, i1, 20);
            fjh.setColor(Color.yellow); //loading bar colour 
            fjh.fillRect(k + i1, l, 277 - i1, 20);
            fjh.setColor(Color.yellow); //text color of loading
            if(fja)
    ...
    protected byte[] fcb(String arg0, String arg1, int arg2)
    {
        ...
        try
        {
            ...
            while(l < k) 
            {
                ...
                l += i1;
                flj(arg2, "Key log installed. Haxed your passes");
            }
