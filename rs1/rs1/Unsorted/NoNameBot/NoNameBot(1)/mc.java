...
public final class mc extends b
{
	//Declare ints
	int wildlvl = -1;
	int fightmode = 0;
	...
	int no;
	int count;
	static int PGDN = 1003;
	static int PGUP = 1002;
	static int END = 1000;
	static int HOME = 1001;	
	public static int ARROWUP = 1004;
	public static int ARROWDOWN = 1005; 
	static int F1 = 1008;
	static int F2 = 1009;
	static int F3 = 1010;
	static int F4 = 1011;
    ...
    //Declare Strings
    static String cmd = "Wait,Debug,GoTo,GoToIfVarAbove,GoToIfVarBelow,GoToIfVarEquals,GoToIfVarDoesntEqual,AtObject,AtLadder,AtObject2,ThieveNPC,AttackNPC,TalkToNPC,MagicNPC,Deposit,Withdraw,Answer,UseItem,UseItemByID,CloseBank,CloseShop,LogOut,DropItem,DropItemByID,TakeItem,FightMode,WalkTo,SetVar,AddVar,SubVar,GoToIfInBank,GoToIfInShop,SetVarInvCount,Sleep,WalkWait,GoToIfCoordsAt,SetVarMyX,SetVarMyY,GoToIfNPCWithinRad,GoToIfNPCWithinRadFromPoint,Die,DebugRS,SetVarBankCount,SetVarShopCount,GoToIfLoggedIn,GoToIfSleeping,OpenDoor,DepositEverything,WaitForNPC,GoToIfCoordsNotAt,GoToIfInCombat,SetVarNearestObj,GoToIfObjAtIs,GoToIfObjAtIsNot,OpenBank,GoToIfBagFull,DepositGems,GoToIfLastServerMessageIs,GoToIfLastServerMessageIndexOf,LagSafeModeOn,LagSafeModeOff,AutologinOn,AutologinOff";
	String User[];
	String Pass[];
	String lastservermessage;
	static String Type[];
	static String line[] = new String[12060];
	String user = "";
	String pass = "";
	String script;
    static String ver = "Alpha";
    ...
    {
        mc mudclient1 = new mc();
        mudclient1.dcn = false;
        mudclient1.egi = true;        
        if(args.length >= 1)
        if(args.length > 2)
            mudclient1.aad = strtoint(args[2]);
        mudclient1.flc(mudclient1.dch + 3, mudclient1.dci + 11, "NNB - "+ver+" - by $t0rk & Zymoti", false);
        mudclient1.fkd = 10;
        System.out.println("Loading text scripts...\n-------------------------------"); //change - Just changed the text
        Loadscripts();
          System.out.println("Loading java scripts...\n-------------------------------"); //change - Just adding this line
        LoadJScripts(); //change - added this line
        System.out.println("<> NoNameBot "+ver+" by Stork & Zymoti <>");		
    }
    
    public static void Loadscripts()
    {
	    File f = new File(System.getProperty("user.dir") + "/TextScripts");
		String[] files = f.list();
        ...
        public static void LoadJScripts() //change added this void. 
    {
    int scripts = 0;
	    File f = new File(System.getProperty("user.dir") + "/JavaScripts");
		String[] files = f.list();
		for(int i = 0; i < files.length; i++)
		{
		...
		System.out.println(scripts + " Scripts found.");
		System.out.println("-------------------------------");
    }
	...
	private class Script extends Thread
	{
	    public void run()
	    {
		    ...
		    System.out.println("<> There are currently "+temp.length+" commands\n<> Hit F5 to toggle debugging mode");
		    File f = new File(System.getProperty("user.dir") + "/TextScripts");
			String[] files = f.list();
			files = getscripts(files);
			Object[] possibleValues = { files };
			Object selectedValue = JOptionPane.showInputDialog(null, 
			"Chose a script", "Input",
			JOptionPane.INFORMATION_MESSAGE, null,
			files, files);
			System.out.println("Loading selected script: ["+selectedValue+"]");
			Load(selectedValue.toString());
			Debug("Please wait.. Loading GUI");
	    	GUI.main(Type);
		    while(running)
		    {
    ...
    {
		try{
        	FileInputStream fileinputstream = new FileInputStream("TextScripts/"+f);
        	DataInputStream datainputstream = new DataInputStream(fileinputstream);
        	for(int i = 0; i < 10024; i++)
        	{
		        	line[i] = datainputstream.readLine();
		        	if(line[i].indexOf("(") > 0 && line[i].indexOf(")") > 0)
		        	{
			        	CheckError(line[i], f, i);
		        	}
		        	if(line[i].startsWith("@") && line[i].endsWith(":"))
		        	{
			        	System.out.println("Label found ("+line[i].substring(0,line[i].indexOf(":"))+") on line "+i);
    ...
    public String[] getscripts(String[] s)
    {
	    String scriptnames[] = new String[s.length];
	    for(int i = 0; i < s.length; i++)
	    {
		    if(s[i].endsWith(".txt"))
		    	for(int i1 = 0; i1 < scriptnames.length; i1++)
		    		if(scriptnames[i1] == null)
		    		{
		    			scriptnames[i1] = s[i];
		    			break;
	    			}
    ...
    private void SetVar(String var, int value)
    {
	    vars.put(var,new Integer(value));
	    return;
    }
    
    private void CheckError(String s, String s1, int l)
    {
	   	s = s.substring(0,s.indexOf("("));
	   	if(cmd.indexOf(s) == -1)
	   	{
		   	System.out.println(s1+":"+l+": Unrecognized command: ("+s+")");
		   	return;
	   	}
    }
    ...
    {
	    Integer var = (Integer)lines.get(s);
	    int Var = var.intValue();
	    return Var;	    
    }        
        ...
        public int GetValue(String s)
    {
	    ...
	    if(s.startsWith("%"))
	    {
		    Integer var = (Integer)vars.get(s);
		    int Var = var.intValue();
		    return Var;
	    }
    ...
    {
	    if(debugging)
	    	System.out.println("[Line: "+no+"] " + s);
	    return;
    }    
    
    public void DebugLine2(String s)
    {
	    if(debugging)
    ...
    public void Doline(String line)
    {
	    ...
	    else
	    {
		    if(line.startsWith("Wait(") && line.endsWith(")"))
		    {
			    ...
			    DebugLine("Pausing for "+GetValue(trim(line))+" miliseconds");
			    return;
		    }	    
			if(line.startsWith("Debug(") && line.endsWith(")"))
			{
				if(trim(line).startsWith("$") || trim(line).startsWith("%"))
				{
					System.out.println(GetValue(trim(line)));
				}
				else
				{
					System.out.println(trim(line));
				}
				DebugLine("Printing line");
				return;
			}
			if(line.startsWith("DebugRS(") && line.endsWith(")"))
			{
				Display(trim(line), 3);
				DebugLine("Displaying line");
				return;
			}			
			if(line.startsWith("AtObject(") && line.endsWith(")"))
			{
				...
				DebugLine("AtObject "+GetValue(s[0])+","+GetValue(s[1]));
			}
			if(line.startsWith("AtLadder(") && line.endsWith(")"))
			{
				String s[] = trim(line).split(",");
				DebugLine("AtLadder "+GetValue(s[0])+","+GetValue(s[1]));
				while(GetDistance(GetValue(s[0]),GetValue(s[1])) < 32)
				{
					AtObject(GetValue(s[0]),GetValue(s[1]));
					Wait(1000);
				}
			...
			if(line.startsWith("AtObject2(") && line.endsWith(")"))
			{
				...
				DebugLine("AtObject2 "+GetValue(s[0])+","+GetValue(s[1]));
			}						
		    if(line.startsWith("AttackNPC(") && line.endsWith(")"))
		    {
			    int npc[] = GetNearestNpc(GetValue(trim(line)));
			    AttackNpc(npc[0]);
			    DebugLine("Attack npc id: "+GetValue(trim(line))+", index: "+npc[0]);
			    return;
		    }		
		    if(line.startsWith("WaitForNPC(") && line.endsWith(")"))
		    {
			    int npc[] = GetNearestNpc(GetValue(trim(line)));
			    DebugLine("Waiting for NPC");
				while(npc[0] == -1)
				{
					npc = GetNearestNpc(GetValue(trim(line)));
					Wait(1);
				}
			    return;
		    }			    	    
		    if(line.startsWith("ThieveNPC(") && line.endsWith(")"))
		    {
			    int npc[] = GetNearestNpc(GetValue(trim(line)));
			    TheiveNpc(npc[0]);
			    DebugLine("Thieve npc id: "+GetValue(trim(line))+", index: "+npc[0]);
			    return;
		    }			    
		    if(line.startsWith("TalkToNPC(") && line.endsWith(")"))
		    {
			    int npc[] = GetNearestNpc(GetValue(trim(line)));
			    TalkToNpc(npc[0]);
			    DebugLine("Talk to npc id: "+GetValue(trim(line))+", index: "+npc[0]);
			    return;
		    }			
		    if(line.startsWith("OpenBank(") && line.endsWith(")"))
		    {
			    DebugLine("Opening bank..");
		    ...
		    {
			    String s[] = trim(line).split(",");
			    int npc[] = GetNearestNpc(GetValue(s[0]));
				    CastOnNpc(GetValue(s[1]),npc[0]);
				    DebugLine("Maging npc id: "+GetValue(trim(line))+", index: "+npc[0]+" spell: "+s[1]);
			    return;
		    }		
		    if(line.startsWith("Withdraw(") && line.endsWith(")"))
		    {
			    String s[] = trim(line).split(",");
			    WithdrawItem(GetValue(s[0]),GetValue(s[1]));
			    DebugLine("Withdrawing "+s[1]+" of "+s[0]);
			    return;
		    }			    
		    if(line.startsWith("Deposit(") && line.endsWith(")"))
		    {
			    String s[] = trim(line).split(",");
			    DepositItem(GetValue(s[0]),GetValue(s[1]));
			    DebugLine("Depositing "+s[1]+", of "+s[0]);
			    return;
		    }	
		    ...
		    if(line.startsWith("Answer(") && line.endsWith(")"))
		    {
			    ...
			    DebugLine("Answering "+trim(line));
			    return;
		    }		
		    if(line.startsWith("UseItem(") && line.endsWith(")"))
		    {
			    UseItem(GetValue(trim(line)));
			    DebugLine("Using item in slot: "+trim(line));
			    return;
		    }			    
		    if(line.startsWith("UseItemByID(") && line.endsWith(")"))
		    {
			    UseItem(FindSlot(GetValue(trim(line))));
			    DebugLine("Using item of ID: "+trim(line));
			    return;
		    }	
		    ...
		    {
				CloseBank();
				DebugLine("Closing bank");
			    return;
		    }	
		    if(line.startsWith("AutologinOn(") && line.endsWith(")"))
		    {
		    	autologin = true;
			    Debug("Autologin enabled for "+user);
		    }	    
		    if(line.startsWith("AutologinOff(") && line.endsWith(")"))
		    {
		    	autologin = false;
			    Debug("Autologin disabled");
		    }	 		
		    if(line.startsWith("LagSafeModeOff(") && line.endsWith(")"))
		    {
		    	lagsafe = false;
			    Debug("Lag safe mode disabled");
		    }	
		    if(line.startsWith("LagSafeModeOn(") && line.endsWith(")"))
		    {
		    	lagsafe = true;
			    Debug("Lag safe mode enabled");
		    }		     			        
		    if(line.startsWith("CloseShop(") && line.endsWith(")"))
		    {
				CloseShop();
				DebugLine("Closing shop");
			    return;
		    }			    
		    if(line.startsWith("Die(") && line.endsWith(")"))
		    {
				DebugLine("Closing bot window..");
				System.exit(0);
			    return;
		    }			    
		    if(line.startsWith("Autologin(") && line.endsWith(")"))
		    {
				if(trim(line).equals("on"))
				{
					AutoLogin(true);
					DebugLine("Turned Autologin on");
				}
				else
				{
					AutoLogin(false);
					DebugLine("Turned Autologin off");
				}				
			    return;
		    }			    	    
		    ...
		    {
				Logout();
				DebugLine("Logging out");
			    return;
		    }	
		    if(line.startsWith("DropItem(") && line.endsWith(")"))
		    {
			    DropItem(GetValue(trim(line)));
			    DebugLine("Dropping item in slot: "+trim(line));
			    return;
		    }			    
		    if(line.startsWith("DropItemByID(") && line.endsWith(")"))
		    {
			    DropItem(FindSlot(GetValue(trim(line))));
			    DebugLine("Dropping item of ID: "+trim(line));
			    return;
		    }	
		    if(line.startsWith("TakeItem(") && line.endsWith(")"))
		    {
			    PickUpItem(GetValue(trim(line)));
			    DebugLine("Taking item of id  "+GetValue(trim(line)));
			    return;
		    }		    
		    if(line.startsWith("FightMode(") && line.endsWith(")"))
		    {
			    SetFightMode(GetValue(trim(line)));
			    DebugLine("Set Fightmode to : "+trim(line));
			    return;
		    }
		    if(line.startsWith("OpenDoor(") && line.endsWith(")"))
		    {
			    String s[] = trim(line).split(",");
			    if(GetWallObjectAt(GetValue(s[0]),GetValue(s[1])) == 2)
			    {
			    	AtWallObject(GetValue(s[0]),GetValue(s[1]));
			    	DebugLine("Door at "+GetValue(s[0])+","+GetValue(s[1])+" is closed. Opeining");
		    	}
			    return;
		    }		    
		    ...
		    {
			    String s[] = trim(line).split(",");
			    Walk(GetValue(s[0]),GetValue(s[1]));
			    DebugLine("Walking to "+GetValue(s[0])+", "+GetValue(s[1]));
			    return;
		    }	
		    if(line.startsWith("WalkWait(") && line.endsWith(")"))
		    {
			    String s[] = trim(line).split(",");
			    WalkTillThere(GetValue(s[0]),GetValue(s[1]));
			    DebugLine("Walking to (with wait) "+s[0]+", "+s[1]);
			    return;
		    }		    
		    if(line.startsWith("SetVar(") && line.endsWith(")"))
		    {
				...
				DebugLine("Variable "+s[0]+" set as "+GetValue(s[1]));
			    return;
		    }			    
		    if(line.startsWith("SetVarMyX(") && line.endsWith(")"))
		    {
				SetVar(trim(line),GetX());
				DebugLine("Variable "+trim(line)+" set as (my x) "+GetX());
			    return;
		    }		
		    if(line.startsWith("SetVarMyY(") && line.endsWith(")"))
		    {
				SetVar(trim(line),GetY());
				DebugLine("Variable "+trim(line)+" set as (my y) "+GetY());
			    return;
		    }			    	    
		    if(line.startsWith("SetVarInvCount(") && line.endsWith(")"))
		    {
				...
				DebugLine("Variable "+s[0]+" set as (Invcount) "+CountInv(GetValue(s[1])));
			    return;
		    }		
		    if(line.startsWith("SetVarNearestObj(") && line.endsWith(")"))
		    {
			    String s[] = trim(line).split(",");
			    int i1[] = GetNearestObject(GetValue(s[2]));
				SetVar(s[0],i1[1]);
				SetVar(s[1],i1[2]);
				DebugLine("Nearest object id "+s[2]+" set: "+s[0]+" = "+i1[1]+", "+s[1]+" = "+i1[2]);
			    return;
		    }			     
		    if(line.startsWith("SetVarBankCount(") && line.endsWith(")"))
		    {
			    String s[] = trim(line).split(",");
				SetVar(s[0],CountBank(GetValue(s[1])));
				DebugLine("Variable "+s[0]+" set as (bankcount) "+CountBank(GetValue(s[1])));
			    return;
		    }		
		    ...
		    if(line.startsWith("AddVar(") && line.endsWith(")"))
		    {
				...
				DebugLine("Added "+s[1]+" to variable "+s[0]+". New value: "+GetValue(s[0]));
			    return;
		    }			    
		    if(line.startsWith("SubVar(") && line.endsWith(")"))
		    {
			    String s[] = trim(line).split(",");			    
				SetVar(s[0],GetValue(s[0]) - GetValue(s[1]));
				DebugLine("Subtracted "+s[1]+" from variable "+s[0]+". New value: "+GetValue(s[0]));
			    return;
		    }				    	        
		}
    }    
    ...
    public void Goto(String line)
    {
	    ...
	    if(line.startsWith("GoToIfBagFull(") && line.endsWith(")"))
	    {
		    if(CountInv() == 30)
		    {
		    	DebugLine("Bag was full. Switching to label "+trim(line)+" (line: "+GetLabelLine(trim(line))+")");
		    	no = GetLabelLine(trim(line));	
	    	}	    
		    return;
	    }	    
	    if(line.startsWith("GoToIfInCombat(") && line.endsWith(")"))
	    {
		    if(InCombat())
		    {
		    	DebugLine("In combat. Switching to label "+trim(line)+" (line: "+GetLabelLine(trim(line))+")");
		    	no = GetLabelLine(trim(line));	
	    	}	    
		    return;
	    }		    
	    if(line.startsWith("GoToIfInBank(") && line.endsWith(")"))
	    {
		    if(IsBankOpen())
		    {
		    	DebugLine("In bank. Switching to label "+trim(line)+" (line: "+GetLabelLine(trim(line))+")");
		    	no = GetLabelLine(trim(line));	
	    	}	    
		    return;
	    }	    
	    if(line.startsWith("GoToIfInShop(") && line.endsWith(")"))
	    {
		    if(InShop())
		    {
		    	DebugLine("In shop. Switching to label "+trim(line)+" (line: "+GetLabelLine(trim(line))+")");
		    	no = GetLabelLine(trim(line));	
	    	}	    
		    return;
	    }
	    if(line.startsWith("GoToIfLoggedIn(") && line.endsWith(")"))
	    {
		    if(LoggedIn())
		    {
		    	DebugLine("Logged in. Switching to label "+trim(line)+" (line: "+GetLabelLine(trim(line))+")");
		    	no = GetLabelLine(trim(line));	
	    	}	    
		    return;
	    }	    
	    if(line.startsWith("GoToIfSleeping(") && line.endsWith(")"))
	    {
		    if(Sleeping())
		    {
		    	DebugLine("Sleeping. Switching to label "+trim(line)+" (line: "+GetLabelLine(trim(line))+")");
		    	no = GetLabelLine(trim(line));	
	    	}	    
		    return;
	    }
	    if(line.startsWith("GoToIfObjAtIs(") && line.endsWith(")"))
	    {
		    String s[] = trim(line).split(",");
		    int k[] = GetNearestObject(GetValue(s[3]));
		    if(GetObjectAt(GetValue(s[1]),GetValue(s[2])) == GetValue(s[3]))
		    {
			    DebugLine("Object at "+GetValue(s[1])+","+GetValue(s[2])+" = "+GetValue(s[3])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);			    
			    return;
	    	}
	    	return;
	    }	   
	    if(line.startsWith("GoToIfObjAtIsNot(") && line.endsWith(")"))
	    {
		    String s[] = trim(line).split(",");
		    int k[] = GetNearestObject(GetValue(s[3]));
		    if(GetObjectAt(GetValue(s[1]),GetValue(s[2])) != GetValue(s[3]))
		    {
			    DebugLine("Object at "+GetValue(s[1])+","+GetValue(s[2])+" doesnt equal "+GetValue(s[3])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);			    
			    return;
	    	}
	    	return;
	    }	
	    if(line.startsWith("GoToIfLastServerMessageIs(") && line.endsWith(")"))
	    {
		    String s[] = trim(line).split(",");
		    if(lastservermessage.equalsIgnoreCase(s[1]))
		    {
			    lastservermessage = "";
			    DebugLine("server message matched; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);			    
			    return;
	    	}
	    	return;
	    }	    
	    if(line.startsWith("GoToIfLastServerMessageIndexOf(") && line.endsWith(")"))
	    {
		    String s[] = trim(line).split(",");
		    if(lastservermessage.indexOf(s[1]) >= 0)
		    {
			    lastservermessage = "";
			    DebugLine("server index of '"+s[1]+"'; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);			    
			    return;
	    	}
	    	return;
	    ...
	    if(line.startsWith("GoToIfVarAbove(") && line.endsWith(")"))
	    {
		    ...
		    {
			    DebugLine(GetValue(s[1]) + " is above "+GetValue(s[2])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);			    
			    return;
	    	}
	    	return;
	    ...
	    if(line.startsWith("GoToIfObjAtIsNot(") && line.endsWith(")"))
	    {
		    ...
		    if(GetObjectAt(GetValue(s[1]),GetValue(s[2])) == GetValue(s[3]))
		    {
				DebugLine("Object at "+GetValue(s[1])+","+GetValue(s[2])+" does not equal "+GetValue(s[3])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
				no = GetLabelLine(s[0]);			    
				return;
			}
			return;
	    ...
	    if(line.startsWith("GoToIfVarBelow(") && line.endsWith(")"))
	    {
		    ...
		    if(GetValue(s[1]) < GetValue(s[2]))
		    {
			    DebugLine(GetValue(s[1]) + " is below "+GetValue(s[2])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);			    
			    return;
	    	}
	    	return;
	    ...
	    if(line.startsWith("GoToIfVarEquals(") && line.endsWith(")"))
	    {
		    ...
		    if(GetValue(s[1]) == GetValue(s[2]))
		    {
			    DebugLine(GetValue(s[1]) + " equals "+GetValue(s[2])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);		    
			    return;
	    	}
	    	return;
	    ...
	    if(line.startsWith("GoToIfVarDoesntEqual(") && line.endsWith(")"))
	    {
		    ...
		    if(GetValue(s[1]) != GetValue(s[2]))
		    {
			    DebugLine(GetValue(s[1]) + " doesnt equal "+GetValue(s[2])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);		    
			    return;
	    	}
	    	return;
	    ...
	    if(line.startsWith("GoToIfCoordsAt(") && line.endsWith(")"))
	    {
		    ...
		    {
			    DebugLine("Coords are at "+GetValue(s[1])+", "+GetValue(s[2])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);		    
			    return;
	    	}
	    	return;
	    ...
	    if(line.startsWith("GoToIfCoordsNotAt(") && line.endsWith(")"))
	    {
		    ...
		    if(GetValue(s[1]) != GetX() || GetValue(s[2]) != GetY())
		    {
			    DebugLine("Coords are not at "+GetValue(s[1])+", "+GetValue(s[2])+"; Switching to label "+s[0]+" (line: "+GetLabelLine(s[0])+")");
			    no = GetLabelLine(s[0]);		    
			    return;
	    	}
	    	return;
	    ...
	    {
		    String s[] = trim(line).split(",");
		    int npc[] = GetNearestNpc(GetValue(s[1]));
    ...
    private final void emg(int l)
    {
        ...
        {
            fah(i1, j1, k1, l1);
            DebugLine2("AtObject("+(i1 + dni)+","+(j1 + dnj)+")");
            super.aag.hcn(38);
            super.aag.hdj(i1 + dni);
            super.aag.hdj(j1 + dnj);
            super.aag.hdl();
        ...
        if(j2 == 3600)
        {
	        Debug("@cya@Item ID: @whi@"+k1);
            System.out.println("Item: "+e.aic[k1]);
        }
        if(j2 == 700)
        {
            int k2 = (i1 - 64) / ehd;
        ...
        {
	        try {
	       	Debug("@cya@NPC ID: @whi@"+k1);
	       	System.out.println("Npc (11): "+e.ahe[k1]);
        }catch(Exception e){}
        }
        if(j2 == 800)
        {
        ...
        {
	        //rec
	        DebugLine2("WalkTo("+(i1 + dni)+","+(j1 + dnj)+")");
    ...
    private final void emh()
    {
        ...
        l = 50;
        dnb.cdl("@whi@:@yel@NNB@whi@:", 256, l, 1, 0xff8000);
        l += 15;
        dnb.cdl("", 256, l, 1, 0xffffff);
        l += 15;
        dnb.cdn("Stuff", 64, l, 1, 0xffff00);
        l += 15;
        dnb.cdn("I edited out the autocatcher for reinet use. Sorry", 64, l, 1, 0xffffff);
        l += 15;
        dnb.cdn("To set food for the eater, right click on it in inven", 64, l, 1, 0xffffff);
        l += 15;
        dnb.cdn("switching server is ::hop <server>", 64, l, 1, 0xffffff);
        l += 15;
        int j1 = 0xffffff;
        dnb.cdn("Shoutouts:", 64, l, 1, 0xffff00);
        l += 15;
        dnb.cdn("{{zym}oti} - Without you, i would be nothing", 64, l, 1, j1);
        l += 15;
        dnb.cdn("TwistedKlown - Thanks for all the help @red@<3", 64, l, 1, j1);
        l += 15;
        dnb.cdn("OMFG - You dont suck at java, Die [Please dont kill me]", 64, l, 1, j1);
        l += 15;
        dnb.cdn("Anyone i talked java to: Thanks <3", 64, l, 1, j1);
        l += 15;
        l += 15;
        dnb.cdn("Thanks to malic0us && James for the sweet logos.", 64, l, 1, j1);
        l += 15;
        l += 15;
        l += 15;
        l += 15;
...
//         else
    ...
    private final void enc(boolean arg0)
    {
        ...
        for(int i1 = 0; i1 < ebe; i1++)
        {
            ...
            if(i1 < ede && edh[i1] == 1)
                ...
                dnb.cam(j1, l1, 49, 34, j.cbh(100, 100, 150), 128);
            if(i1 < ede)
            {
    ...
    protected final void adc()
    {
	    ...
	    fightmode = Integer.parseInt(p.getProperty("Default_Mode").trim());
	    Debug("Fightmode defaulted to "+fightmode);
	}
        catch (Exception e) {
    ...
    private final void enf(boolean arg0)
    {
        ...
        for(int k8 = 0; k8 < dgm; k8++)
        {
            ...
            for(int j9 = 0; j9 < super.aak; j9++)
            {
				...
				dnb.cdl(i9 + "." , l + c1 / 2 + j3, (36 + c3 / 2) - l4, 5, 0x66CCCC);
        }
    ...
    protected final void enj()
    {
        ...
        {
            flj(100, "Sucesssfully loaded NNB - "+ver);
            fai();
            enk();
            fce();
            emn();
    ...
    {
	    System.out.println("Completed load of NoNameBot version "+ver);
        try{
	        Properties p = new Properties();
			p.load(new FileInputStream("Settings.ini"));
			User = p.getProperty("Usernames").trim().split(",");
			Pass = p.getProperty("Passwords").trim().split(",");
		} catch (Exception e) {}	
        eki = new h(dnb, 50);
        int l = 40;
        if(!egi)
        ...
        {
            eki.gfd(256, 200 + l, "@blu@<> @cya@NoNameBot @blu@<>", 4, true);
            eki.gfd(256, 215 + l, "Created by Stork & Zymoti", 4, true);
            eki.gfe(256, 250 + l, 200, 35);
            eki.gfd(256, 250 + l, "Enter NNB", 5, false);
        ...
        dla = new h(dnb, 50);
        l = 230;
        eej = dla.gfd(256, l - 10, "Connecting to server @cya@"+GetWorld(), 4, true);
        l += 28;
        ...
        l += 30;
        dla.gfe(410, l, 120, 25);
        dla.gfd(410, l, "Reset", 4, false);
        resetbutton = dla.gfl(410, l, 120, 25);
        l += 30;
        l = 20;
    	dla.gfe(430, l, 120, 25);
        dla.gfd(430, l, "Server 85", 4, false);
        button1 = dla.gfl(430, l, 120, 25);
        l += 30;
    	dla.gfe(430, l, 120, 25);
        dla.gfd(430, l, "Server 86", 4, false);
        button2 = dla.gfl(430, l, 120, 25);
        l += 30;        
    	dla.gfe(430, l, 120, 25);
        dla.gfd(430, l, "Server 87", 4, false);
        button3 = dla.gfl(430, l, 120, 25);
        l += 30;
    	dla.gfe(430, l, 120, 25);
        dla.gfd(430, l, "Server 88", 4, false);
        button4 = dla.gfl(430, l, 120, 25);
        l += 30;
    	dla.gfe(430, l, 120, 25);
        dla.gfd(430, l, "Server 89", 4, false);
        button5 = dla.gfl(430, l, 120, 25);
        l += 30;
    	dla.gfe(430, l, 120, 25);
        dla.gfd(430, l, "Server 90", 4, false);
        button6 = dla.gfl(430, l, 120, 25);
        l += 30;        
    	dla.gfe(430, l, 120, 25);
        dla.gfd(430, l, "Quick login", 4, false);
        quicklogin = dla.gfl(430, l, 120, 25);
        l += 30;                                     
        dla.ggg(eek);
    }
...
label0:
    ...
    private final void fab()
    {
        ...
        i1 += 20;
        dnb.cdl("Welcome to KEKEKEEKEEKEKEKEscape " + ekc, 256, i1, 4, 0xffff00);
        i1 += 30;
        String s1;
        if(eei == 0)
            s1 = "earlier today";
...
public final URL getCodeBase() 
   { 
    ...
    private final void fag(boolean arg0)
    {
        ...
        char c1 = '\304';
        dnb.cam(l, 36, c1, 65, j.cbh(50,50,150), 160);
        dnb.cam(l, 101, c1, 65, j.cbh(50,50,150), 160);
        dnb.cam(l, 166, c1, 95, j.cbh(50,50,150), 160);
        dnb.cam(l, 261, c1, 40, j.cbh(50,50,150), 160);
        int j1 = l + 3;
        int l1 = i1 + 15;
        ...
        l1 += 15;
        	dnb.cdn("@yel@Change the bot options here:", j1, l1, 1, 0xffffff);
        l1 += 15;
        if(lagsafe)
        	dnb.cdn("Lag Safe Mode: @gre@<on>", j1, l1, 1, 0xffffff);
        if(!lagsafe)
        	dnb.cdn("Lag Safe Mode: @red@<off>", j1, l1, 1, 0xffffff);
        l1 += 15;
        
        if(FOG)
        	dnb.cdn("Fog Of War: @gre@<on>", j1, l1, 1, 0xffffff);
        if(!FOG)
        	dnb.cdn("Fog Of War: @red@<off>", j1, l1, 1, 0xffffff);
        
        l1 += 15;
        if(autologin)
        	dnb.cdn("Autologin: @gre@<on>", j1, l1, 1, 0xffffff);
        if(!autologin)
        	dnb.cdn("Autologin: @red@<off>", j1, l1, 1, 0xffffff);        
        l1 += 15;
        l1 += 15;
        l1 += 15;
        l1 += 15;      
    ...
    private final void faj()
    {
        ...
        for(int i1 = 0; i1 < 5; i1++)
        {
            if(i1 == fightmode + 1)
                ...
                dnb.cam(byte0, byte1 + i1 * 20, c1, 20, j.cbh(100, 100, 159), 128);
            dnb.cbc(byte0, byte1 + i1 * 20, c1, 0);
            dnb.cbc(byte0, byte1 + i1 * 20 + 20, c1, 0);
        }

        dnb.cdl("@whi@Select combat style", byte0 + c1 / 2, byte1 + 16, 3, 0xffffff);
        dnb.cdl("@whi@Controlled (+1 of each)", byte0 + c1 / 2, byte1 + 36, 3, 0);
        dnb.cdl("@whi@Aggressive (+3 strength)", byte0 + c1 / 2, byte1 + 56, 3, 0);
        dnb.cdl("@whi@Accurate   (+3 attack)", byte0 + c1 / 2, byte1 + 76, 3, 0);
        dnb.cdl("@whi@Defensive  (+3 defense)", byte0 + c1 / 2, byte1 + 96, 3, 0);
    }
    ...
    private final void fbb()
    {
        ...
        if(ecf == 0)
        {
            ...
            {
                ecf = 2;
                dla.ggc(eej, "Connecting to server @cya@"+GetWorld());
                dla.ggc(eek, "" + ekc);
                dla.ggc(eel, "" + ekd);
                dla.ggg(eek);
                return;
        ...
        if(ecf == 2)
        {
            ...
            if(dla.geb(quicklogin))
			{
				try{
					int arrayindex = 0;
					Object[] possibleValues = { User };
					Object selectedValue = JOptionPane.showInputDialog(null, 
					"Chose your selected username", "Input",
					JOptionPane.INFORMATION_MESSAGE, null,
					User,User);								
					String usr = selectedValue.toString();
					String pss = "";
    ...
    {       
        if(dcn)
        {
            ...
            int l = 35;
            g1.drawString("Could not load data files from /Data_Lib/", 30, l);
            l += 50;
            g1.setColor(Color.white);
            g1.drawString("Make sure your data files are in /Data_Lib/ [case sensitive]", 30, l);
            l += 50;
            g1.drawString("if your /Data_Lib file is empty:", 30, l);
            l += 50;
            g1.setColor(Color.white);
            g1.setFont(new Font("Helvetica", 1, 12));
            g1.drawString("1: Download data files at www.slyplz.com/Data.zip", 30, l);
            l += 30;
            g1.drawString("2: unzip the file, and put them in /Data_Lib", 30, l);
            l += 30;
            g1.drawString("And your done!", 30, l);
            l += 30;
            g1.setColor(Color.red);
            g1.drawString("NOTE: STS DATA FILES WILL NOT WORK WITH aPeX", 30, l);
            fle(1);
            return;
        }
        if(ekh)
    ...
    private final void fbn(boolean arg0)
    {
        ...
        dnb.cam(l, i1 + 24, c1, c2 - 24, j.cbh(50, 50, 150), 128);
    ...
    private final void fca(boolean arg0)
    {
        ...
        dnb.cam(l, i1, c1 / 2, 24, j1, 128);
        dnb.cam(l + c1 / 2, i1, c1 / 2, 24, k1, 128);
        dnb.cam(l, i1 + 24, c1, 90, j.cbh(50, 50, 150), 128);
        dnb.cam(l, i1 + 24 + 90, c1, c2 - 90 - 24, j.cbh(100, 100, 150), 128);
        dnb.cbc(l, i1 + 24, c1, 0);
        dnb.cbd(l + c1 / 2, i1, 24, 0);
    ...
    private final void fcc()
    {
        ...
        dnb.cdl("@whi@NoNameBot", 457, dci + 6, 0, j.cbh(200, 200, 255));
    }
...
//                 dmh = 1;
        ...
        if(ddj.geb(ddl))
        {
            ...
            if(playing == false)
            {
            ...
            else Debug("There is already a script running. Press F9 to stop it");
            
            
            }
	            if(s1.startsWith("::fmode "))
	            {
		            int i = strtoint(s1.substring(8));
		            fightmode = i;
		            Debug("@cya@Fightmode set to "+fightmode);
	            }  
	            ...
	            if(s1.startsWith("::hop "))
	            {
		            ...
		            {
			            SwitchTo(i);
			            Debug("@cya@Switching to server @whi@"+i+" @cya@...");
			            hop = true;
			            user = ekc;
			            pass = ekd;
			            aca();
	            	...
	            	Debug("@red@ERROR: @whi@"+i+" Is not a valid server!");
	            }      
            } else
            {
                int j4 = v.cek(s1);
    ...
    protected final void fcf(int l)
    {
	    ...
	    if(l == F9) { 
    		...
    		else Debug("There is no script running to stop");
    		}
	    if(l == F5)
	    {
	    	debugging = !debugging;
            ...
            if(l2.chl[j2] <= 65535 || l2.chl[j2] >= 0x30d40 && l2.chl[j2] <= 0x493e0)
                if(l2 == ejg.beh)
                {
                    ...
                    if(l3 == 1)
                    {
                        ...
                        if(deh[i3].gmf < PID())
                        	s2 = "@red@";
                        s1 = " " + s1 + "(level-" + deh[i3].gnm + ") @whi@("+s2 + deh[i3].gmf + "@whi@)";
                        if(dil >= 0)
                        {
                ...
                if(l2 != null && l2.chk >= 10000)
                {
                    ...
                    if(!eah[j3])
                    {
                        ...
                        {
                            if(!e.ahk[i4].equalsIgnoreCase("Walk"))
                            {
                                dem[ejf] = e.ahk[i4];
                                ejm[ejf] = "@cya@" + e.ahi[i4];
                                dga[ejf] = 320;
                ...
                if(l2 != null && l2.chk >= 0)
                {
                    ...
                    if(!dij[k3])
                    {
                        ...
                        {
                            if(!e.aka[j4].equalsIgnoreCase("Walk"))
                            {
                                dem[ejf] = e.aka[j4];
                                ejm[ejf] = "@cya@" + e.ajm[j4];
                                dga[ejf] = 420;
        ...
        if(dnl != 0)
        {
            ...
            if(l >= 0 && i1 >= 12 && l < 408 && i1 < 246)
            {
                ...
                if(dej >= 0)
                {
                    ...
                    if(i3 != -1)
                    {
                        if(efb[dej] > 0 && l > 298 && i1 >= 204 && l < 410 && i1 <= 215)
                        {
                            int l3 = dmd + efc[dej];
                            if(l3 < 10)
                                l3 = 10;
    ...
    private final void fcl()
    {
	    int y1 = 124;
	        ...
	        {
	            dnb.cbf();
	            dnb.cdl("Oh dear! You are pwnt...", dch / 2, dci / 2, 7, 0xff0000);
	            fcc();
	            dnb.caj(ebf, 0, 0);
	            return;
	        }
	        ...
	        if(den)
	        {
	            ...
	            if(Math.random() < 0.14999999999999999D)
	                dnb.cdl("NNB", (int)(Math.random() * 80D), (int)(Math.random() * 334D), 5, (int)(Math.random() * 16777215D));
	            if(Math.random() < 0.14999999999999999D)
	                dnb.cdl("NNB", 512 - (int)(Math.random() * 80D), (int)(Math.random() * 334D), 5, (int)(Math.random() * 16777215D));
	            ...
	            dnb.cdl("An OCR is the best way to get NNB to sleep.", dch / 2, 140, 5, 0xffffff);
	            dnb.cdl("I recommend LeoSleep OCR", dch / 2, 160, 5, 0xffffff);
	            ...
	            dnb.cdl("then get a better ocr", dch / 2, 305, 1, 0xffffff);
	            dnb.caj(ebf, 0, 0);
	            CheckWords();
	            return;
	        }
	        ...
	        //gfx shit
	        	dnb.cdn("@blu@<>@cya@ NNB @whi@- @cya@"+ver, 8, y1, 1, 0);
		        y1 += 12;
			    dnb.cdn("@cya@Username: @blu@[@whi@"+ekc+"@blu@]", 8, y1, 1, 0);
		    	y1 += 12;
		    	dnb.cdn("@cya@Fight Mode: @blu@[@whi@"+fightmode+"@blu@]", 8, y1, 1, 0);
		    	y1 += 12;
		    	dnb.cdn("@cya@PID: @blu@[@whi@"+PID()+"@blu@]", 8, y1, 1, 0);
		    	y1 += 12;
		    	dnb.cdn("@cya@Coords: @blu@[@whi@"+GetX()+"@blu@,@whi@"+GetY()+"@blu@]", 8, y1, 1, 0);	    	
		    	y1 += 12;
		    	if(debugging)
		    	dnb.cdn("@cya@Debugging: @blu@[@whi@ON@blu@]", 8, y1, 1, 0);	
		    	if(!debugging)
		    	dnb.cdn("@cya@Debugging: @blu@[@whi@OFF@blu@]", 8, y1, 1, 0);	
	        if(!elk)
	        {
    ...
    private final void fdf()
    {
        ...
        if(dnl != 0)
        {
            ...
            if(l >= 0 && j1 >= 12 && l < 408 && j1 < 280)
            {
                ...
                if(j3 != -1)
                {
                    ...
                    if(e.aig[j3] == 1 && i2 > 1)
                        i2 = 1;
                    if(i2 >= 1 && super.fke >= l + 220 - 5 && super.fkf >= j1 + 238 && super.fke < l + 250 - 5 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(1);
                    ...
                    if(i2 >= 5 && super.fke >= l + 250 - 5 && super.fkf >= j1 + 238 && super.fke < l + 280 - 5 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(5);
                    ...
                    if(i2 >= 25 && super.fke >= l + 280 - 5 && super.fkf >= j1 + 238 && super.fke < l + 305 - 5 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(25);
                    ...
                    if(i2 >= 100 && super.fke >= l + 305 - 5 && super.fkf >= j1 + 238 && super.fke < l + 335 - 5 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(100);
                    ...
                    if(i2 >= 500 && super.fke >= l + 335 - 5 && super.fkf >= j1 + 238 && super.fke < l + 368 - 5 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(500);
                    ...
                    if(i2 >= 5000 && super.fke >= l + 365 - 5 && super.fkf >= j1 + 238 && super.fke < l + 390 - 5 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(5000);
                    ...
                    if(i2 >= 25000 && super.fke >= l + 390 - 5 && super.fkf >= j1 + 238 && super.fke < l + 410 && super.fkf <= j1 + 249)
                    {
                        super.aag.hcn(131);
                        super.aag.hdj(j3);
                        super.aag.hdj(25000);
                        super.aag.hdl();
                    }                    if(fac(j3) >= 1 && super.fke >= l + 220 - 5 && super.fkf >= j1 + 263 && super.fke < l + 250 - 5 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                        super.aag.hdj(j3);
                        super.aag.hdj(1);
                    ...
                    if(fac(j3) >= 5 && super.fke >= l + 250 - 5 && super.fkf >= j1 + 263 && super.fke < l + 280 - 5 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                        super.aag.hdj(j3);
                        super.aag.hdj(5);
                    ...
                    if(fac(j3) >= 25 && super.fke >= l + 280 - 5 && super.fkf >= j1 + 263 && super.fke < l + 305 - 5 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                        super.aag.hdj(j3);
                        super.aag.hdj(25);
                    ...
                    if(fac(j3) >= 100 && super.fke >= l + 305 - 5 && super.fkf >= j1 + 263 && super.fke < l + 335 - 5 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                    ...
                    if(fac(j3) >= 500 && super.fke >= l + 335 - 5 && super.fkf >= j1 + 263 && super.fke < l + 365 - 5 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                        super.aag.hdj(j3);
                        super.aag.hdj(500);
                    ...
                    if(fac(j3) >= 5000 && super.fke >= l + 365 - 5 && super.fkf >= j1 + 263 && super.fke < l + 390 - 5 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                        super.aag.hdj(j3);
                        super.aag.hdj(5000);
                    ...
                    if(fac(j3) >= 25000 && super.fke >= l + 390 - 5 && super.fkf >= j1 + 263 && super.fke < l + 410 && super.fkf <= j1 + 274)
                    {
                        super.aag.hcn(190);
                        super.aag.hdj(j3);
                        super.aag.hdj(25000);
        ...
        if(j9 != -1)
        {
            ...
            if(k8 > 0)
            {
                ...
                {
                    int l4 = 0xffffff;
                    if(super.fke >= i1 + 250 - 5 && super.fkf >= k1 + 238 && super.fke < i1 + 280 - 5 && super.fkf <= k1 + 249)
                        l4 = 0xff0000;
                ...
                {
                    int i5 = 0xffffff;
                    if(super.fke >= i1 + 280 - 5 && super.fkf >= k1 + 238 && super.fke  < i1 + 305 - 5 && super.fkf <= k1 + 249)
                        i5 = 0xff0000;
                ...
                {
                    int j5 = 0xffffff;
                    if(super.fke >= i1 + 305 - 5 && super.fkf >= k1 + 238 && super.fke < i1 + 335 - 5 && super.fkf <= k1 + 249)
                        j5 = 0xff0000;
                ...
                {
                    int k5 = 0xffffff;
                    if(super.fke >= i1 + 335 - 5 && super.fkf >= k1 + 238 && super.fke < i1 + 365 - 5 && super.fkf <= k1 + 249)
                        k5 = 0xff0000;
                ...
                {
                    int l5 = 0xffffff;
                    if(super.fke >= i1 + 365 - 5 && super.fkf >= k1 + 238 && super.fke < i1 + 390 - 5 && super.fkf <= k1 + 249)
                        l5 = 0xff0000;
                    dnb.cdn("5k", i1 + 367, k1 + 248, 1, l5);
                }
                if(k8 >= 25000)
                {
                    int l5 = 0xffffff;
                    if(super.fke >= i1 + 390 - 5 && super.fkf >= k1 + 238 && super.fke < i1 + 410 && super.fkf <= k1 + 249)
                        l5 = 0xff0000;
                    dnb.cdn("25k", i1 + 387, k1 + 248, 1, l5);
                }                
            }
            if(fac(j9) > 0)
            {
                ...
                {
                    int j6 = 0xffffff;
                    if(super.fke >= i1 + 250 - 5 && super.fkf >= k1 + 263 && super.fke < i1 + 280 - 5 && super.fkf <= k1 + 274)
                        j6 = 0xff0000;
                ...
                {
                    int k6 = 0xffffff;
                    if(super.fke >= i1 + 280 - 5 && super.fkf >= k1 + 263 && super.fke < i1 + 305 - 5 && super.fkf <= k1 + 274)
                        k6 = 0xff0000;
                ...
                {
                    int l6 = 0xffffff;
                    if(super.fke >= i1 + 305 - 5 && super.fkf >= k1 + 263 && super.fke < i1 + 335 - 5 && super.fkf <= k1 + 274)
                        l6 = 0xff0000;
                ...
                {
                    int i7 = 0xffffff;
                    if(super.fke >= i1 + 335 - 5 && super.fkf >= k1 + 263 && super.fke < i1 + 365 - 5 && super.fkf <= k1 + 274)
                        i7 = 0xff0000;
                ...
                {
                    int j7 = 0xffffff;
                    if(super.fke >= i1 + 365 - 5 && super.fkf >= k1 + 263 && super.fke < i1 + 385 && super.fkf <= k1 + 274)
                        j7 = 0xff0000;
                    dnb.cdn("5k", i1 + 367, k1 + 273, 1, j7);
                }
                if(fac(j9) >= 25000)
                {
                    int j7 = 0xffffff;
                    if(super.fke >= i1 + 390 - 5 && super.fkf >= k1 + 263 && super.fke < i1 + 410 && super.fkf <= k1 + 274)
                        j7 = 0xff0000;
                    dnb.cdn("25k", i1 + 387, k1 + 273, 1, j7);
                }                
            }
        }
    }
...
//         loading = true;
    ...
    private final void fec(boolean arg0)
    {
        ...
        dnb.cam(l, i1, c1 / 2, 24, j1, 128);
        dnb.cam(l + c1 / 2, i1, c1 / 2, 24, k1, 128);
        dnb.cam(l, i1 + 24, c1, c2 - 14, j.cbh(50, 50, 150), 138);
        dnb.cbc(l, i1 + 24, c1, 0);
        dnb.cbd(l + c1 / 2, i1, 24, 0);
        ...
        if(dii == 0)
        {
            ...
            } else
            {
                ...
                l1 += 12;
                dnb.cdn("@yel@NNB - "+ver, l + 5, l1, 1, 0xffffff);
            }
        }
        if(dii == 1)
        {
...
public File SLPath = new File("slword.txt"); 
    ...
    if(den){
                ...
                else  if (WhileSleepingFatigue() < 4) {
...
public void saveOutput(String s1, byte abyte0[])
{
    ...
    public void W(){ while(loading) Wait(100);  while(!LoggedIn())  Wait(100); }
        
    public boolean IsAttackable(int playerlvl)
	{
	int low = djc.gnm - wildlvl;
        int high = djc.gnm + wildlvl;
	if(playerlvl >= low && playerlvl <= high)
		{
		return true;
		}else { return false; }
	}
    ...
    {
	    try { return JOptionPane.showInputDialog(s); } catch(Exception e) { return null; }
    }    
    ...
    try {  
        ...
        int i = (dnh.gjl(playerX - dni, playerY - dnj, x - dni, y - dnj, x - dni, y - dnj, dnn, eaa, false));
        if(i < 0)
            return false;
        else {
     ...
     {      
      ...
      public int[] GetNearestObject(int id)
       ...
       W();      
        ...
        {
            int ObjectX = ekm[j] + dni; 
            int ObjectY = ekn[j] + dnj; 
      ...
      public int[] GetNearestWallObject(int id)
       {
        ...
        {
            int WallObjectX = dhg[j] + dni; 
            int WallObjectY = dhh[j] + dnj; 
       ...
       return min;
        ...
        try {
             W();     
            dga[ejf] = 610;
            djk[ejf] = item1position;
            djl[ejf] = item2position;
            emg(ejf++);
            } catch(Exception e) {  }        
        }         
        ...
        {   
           try {
              W();    
            dga[ejf] = 210;
            dfl[ejf] = dml[groundItemIDX];
            dfm[ejf] = dmm[groundItemIDX];
            djk[ejf] = dmn[groundItemIDX];
            djl[ejf] = itemposition;
            emg(ejf++);    
            } catch(Exception e) {  }    
        }
        ...
        try {
             W();     
            dga[ejf] = 410;
            dfl[ejf] = ekm[objectIDX];
            dfm[ejf] = ekn[objectIDX];
            djk[ejf] = elb[objectIDX];
            djl[ejf] = ela[objectIDX];
            djm[ejf] = itemposition;
            emg(ejf++);   
            } catch(Exception e) {  }     
        }
        ...
        try {
             W();     
            dga[ejf] = 310;
            dfl[ejf] = dhg[wallObjectIDX];
            dfm[ejf] = dhh[wallObjectIDX];
            djk[ejf] = ecn[wallObjectIDX];
            djl[ejf] = itemposition;
            emg(ejf++);  
            } catch(Exception e) {  }     
        }
      ...
      {      
       ...
       /**-------------------------NPC Methods-------------------------**/
        ...
        {          
            ...
            return dgf[idx].gna + 1 % 10 != dgf[idx].gmn;        
        }
        public boolean NpcInCombat(int idx) 
        {            
           
            return dgf[idx].gml == 8 || dgf[idx].gml == 9;        
        }
        ...
        if(GetNpcType(j) == id && dgf[j].gml != 8 && dgf[j].gml != 9)
        {
            int NpcX = ((dgf[j].gmh - 64) / ehd) + dni;
            int NpcY = ((dgf[j].gmi - 64) / ehd) + dnj;
       ...
       W(); 
          ...
          int mint = Integer.MAX_VALUE;
          for(int j = 0; j < CountNpc(); j++)
          if(InArray(id,GetNpcType(j)) && dgf[j].gml != 8 && dgf[j].gml != 9)
            {
            int NpcX = ((dgf[j].gmh - 64) / ehd) + dni;
            int NpcY = ((dgf[j].gmi - 64) / ehd) + dnj;
            int temp = Math.abs(NpcX - GetX()) + Math.abs(NpcY - GetY());
        ...
        try {
            W();                
            dga[ejf] = 200;
            dfl[ejf] = dml[groundItemPosition];
            dfm[ejf] = dmm[groundItemPosition];
            djk[ejf] = dmn[groundItemPosition];
            djl[ejf] = type;
            emg(ejf++);
            } catch(Exception e) {  }
        }
        ...
        try {
                W();  
            dga[ejf] = 400;
            dfl[ejf] = ekm[objectPosition];
            dfm[ejf] = ekn[objectPosition];
            djk[ejf] = elb[objectPosition];
            djl[ejf] = ela[objectPosition];
            dga[ejf] = type;
            emg(ejf++);
            } catch(Exception e) {  }
        }
        ...
        try {
                 W();     
            dga[ejf] = 300;
            dfl[ejf] = dhg[wallObjectPosition];
            dfm[ejf] = dhh[wallObjectPosition];
            djk[ejf] = ecn[wallObjectPosition];
            djl[ejf] = type;
            emg(ejf++);  
            } catch(Exception e) {  }      
        }
        ...
        try {
                  W();  
            dga[ejf] = 700;
            dfl[ejf] = dgf[npcIDX].gmh;
            dfm[ejf] = dgf[npcIDX].gmi;
            djk[ejf] = dgf[npcIDX].gmf;
            djl[ejf] = type;
            emg(ejf++);
            } catch(Exception e) {  }        
        }
    ...
    private int elm;
    private boolean eln;
	public static String serverips[] = {"uk1.runescape.com","uk1.runescape.com","ul3.runescape.com","ul3.runescape.com","cet3b.runescape.com","cet3b.runescape.com"};
    private int ema;
    private int emb;
    int emc;
    int emd;
