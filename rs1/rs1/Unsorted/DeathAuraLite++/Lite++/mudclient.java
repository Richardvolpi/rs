...
{
public String readlines[] = readFile(new File(".", "settings.ini")).split(System.getProperty("line.separator"));
public String uname = readlines[0].replaceAll("Username: ", "");
public String upass = readlines[1].replaceAll("Password: ", "");
public String auser = readlines[2].replaceAll("Authuser: ", "");
public String apass = readlines[3].replaceAll("Authpass: ", "");
    public boolean mem;
    public boolean FightMenu = true;
    public int EatAtHP = 25;
...
static String internetname[] = {
    ...
    public static void main(String args[])
    {
        ...
        mudclient1.flc(mudclient1.dch, mudclient1.dci + 11, "Death Aura Lite++ Restored by RLN", false);
    }
    public static String readFile(File s) {
...
public boolean AttackNPC(int i)
{
    if(!SelfIsFighting())
    {
       fbk(djd, dje, (dgf[i].gmh - 64) / ehd, (dgf[i].gmi - 64) / ehd, true);
       super.aag.hcn(118);
            super.aag.hdj(dgf[i].gmf);
            super.aag.hdl();
       return true;
    }
    else
...
public int NPCCount()
{
   return ebb;
    ...
    public int getnpcindexbyid(int id,int id2,int id3,int id4,int id5)
    {
        int minj = -1;
        int min = 9999999;
        for(int j = 0; j < ebb; j++)
            if((dgf[j].gmj == id || dgf[j].gmj == id2 || dgf[j].gmj == id3 || dgf[j].gmj == id4 || dgf[j].gmj == id5)&& dgf[j].gml != 8 && dgf[j].gml != 9)
            {
                int temp = (int)Math.sqrt((dgf[j].gmh-djc.gmh)*(dgf[j].gmh-djc.gmh)) + (int)Math.sqrt((dgf[j].gmi-djc.gmi)*(dgf[j].gmi-djc.gmi));
                if(temp < min)
                {
                    minj = j;
                    min = temp;
    	...
    	public int Fatigue() { return (dhj * 100) / 750; }
	public boolean Sleeping() { return den; }
private class AutoFight extends Thread 
{
    public void run()
    {
	      ...
	      if(!EApause)
	      {
              if(dkh != lockedStyleCnt) 
              {
	   	 SetFMode(lockedStyleCnt);
              }
            if(!SelfIsFighting())
            {
            if (Fatigue() >= sfat && Sleeping() == false) 
               { 
                  fdj(sey+"@whi@Sleeping....", 3); 
                  useitem(1263);
                  Wait(5000); 
               } else { 
                  if (Sleeping() == false) 
                  {
                int i = getnpcindexbyid(npcid,npcid2,npcid3,npcid4,npcid5);
                if(i != -1 && !SelfIsFighting() && ehe == 1)
                {
                    try
                    {
                    AttackNPC(i); 
...
private class AutoEat extends Thread 
    {
        public void run()
        {
            while(autoEating)
            {  
               if(dfk[3] <= EatAtHP && !SelfIsFighting()) 
               {
                  if(fac(foodID) > 0)
		   {
                      EApause = true;
                   ...
                   {
                      fdj(sey+"@red@Couldn't find anymore " + foodName+"!", 3);
                      autoEating = false;
                      autoFighter = false;
                      Wait(50);
                   }
    ...
    private final void emg(int l)
    {
        ...
        npcid = k1;
        System.out.println("--------------------"); 
        System.out.println("Autofighter turned on, fighting:"); 
             fdj(sey+"@whi@Autofighting @yel@"+e.ahd[npcid], 3);
             System.out.println("~ "+e.ahd[npcid]);
             if(npcid2 != -2){
             fdj(sey+"@whi@Autofighting @yel@"+e.ahd[npcid2], 3);
             System.out.println("~ "+e.ahd[npcid2]);}
             if(npcid3 != -2){
             fdj(sey+"@whi@Autofighting @yel@"+e.ahd[npcid3], 3);
             System.out.println("~ "+e.ahd[npcid3]);}
             if(npcid4 != -2){
             fdj(sey+"@whi@Autofighting @yel@"+e.ahd[npcid4], 3);
             System.out.println("~ "+e.ahd[npcid4]);}
             if(npcid5 != -2){
             fdj(sey+"@whi@Autofighting @yel@"+e.ahd[npcid5], 3);
             System.out.println("~ "+e.ahd[npcid5]);}
             System.out.println("--------------------");
             	autoFighter = true; 
                AF = new AutoFight();
                lockedStyleCnt = dkh;
...
private final String findCacheDir()
   ...
   { 
     ...
     System.out.println("Saving HC.bmp"); 
      a("HC.BMP", picBytes); 
	}   
...
public void a(String s1, byte abyte0[])
		...
		if ((dhk * 100) / 750 == 0)
		{
			try
			{
				...
				if(file.exists() && ec != file.lastModified())
				{
					...
					cc = true;
					System.out.println("Trying sleepword: " + ac);
    ...
    private final void enk()
    {
        ...
        int l = 40;
        {
            eki.gfd(256, 195 + l, "@red@Death Aura Lite++", 7, true);
            eki.gfd(256, 220 + l, "@whi@Created by Cyberman", 3, true);
            eki.gfe(256, 250 + l, 250, 35);
            eki.gfd(256, 250 + l, "Click here to login", 5, false);
            eeb = eki.gfl(256, 250 + l, 250, 35);
        }
        dnm = new h(dnb, 50);
        l = 230;
        if(dkd == 0)
        ...
        l += 28;
        dla.gfe(115, l, 200, 40);
        dla.gfd(115, l - 10, "Username:", 4, false);
        eek = dla.gfj(115, l + 10, 200, 40, 4, 12, false, false);
        l += 47;
        dla.gfe(115, l, 200, 40);
        dla.gfd(115, l - 10, "Password:", 4, false);
        eel = dla.gfj(115, l + 10, 200, 40, 4, 20, true, false);
        l -= 55;
        dla.gfe(430, l - 3, 120, 25);
        dla.gfd(430, l - 3, "Ok", 4, false);
        eem = dla.gfl(430, l - 3, 120, 25);
        l += 30;
        dla.gfe(430, l - 3, 120, 25);
        dla.gfd(430, l - 3, "Cancel", 4, false);
        een = dla.gfl(430, l - 3, 120, 25);
        l += 30;
        dla.gfe(430, 312, 120, 35);
        dla.gfd(430, 304, "Server:", 4, false);
        serv = dla.gfj(430, 320, 120, 35, 4, 12, false, false);
        l += 30;
        dla.gfe(290, 258, 130, 40);
        dla.gfd(290, 248, "Auth User:", 4, false);
        authu = dla.gfj(290, 268, 130, 40, 4, 12, false, false);
        l += 30;
        dla.gfe(290, 305, 130, 40);
        dla.gfd(290, 295, "Auth Pass:", 4, false);
        authp = dla.gfj(290, 315, 130, 40, 4, 12, true, false);
        l += 30;
        dla.ggg(eek);
    }
...
label0:
        ...
        System.out.println("Settings.ini loaded");
    }
...
return new URL("http://" + super.aac + "/client2/");
    ...
    private final void fag(boolean arg0)
    {
        ...
        char c1 = '\304';
        dnb.cam(l, 36, c1, 65, j.cbh(181, 181, 181), 0);
        dnb.cam(l, 101, c1, 65, j.cbh(181, 181, 181), 0);
        dnb.cam(l, 166, c1, 95, j.cbh(181, 181, 181), 0);
        dnb.cam(l, 261, c1, 40, j.cbh(181, 181, 181), 0);
        int j1 = l + 3;
        int l1 = i1 + 15;
        dnb.cdn("@cya@You are currently on world  @whi@"+sName, j1, l1, 1, 0);
        l1 += 15;
        if(dka)
        ...
        l1 += 15;
        if(super.abb == 0)
            dnb.cdn("Chat messages: @red@Off", l + 3, l1, 1, 0xffffff);
        else
            dnb.cdn("Chat messages: @gre@On", l + 3, l1, 1, 0xffffff);
        l1 += 15;
        if(super.abc == 0)
            dnb.cdn("Private messages: @red@Off", l + 3, l1, 1, 0xffffff);
        else
            dnb.cdn("Private messages: @gre@On", l + 3, l1, 1, 0xffffff);
        l1 += 15;
        if(super.abd == 0)
            dnb.cdn("Trade requests: @red@Off", l + 3, l1, 1, 0xffffff);
        else
            dnb.cdn("Trade requests: @gre@On", l + 3, l1, 1, 0xffffff);
        l1 += 15;
            if(super.abe == 0)
                dnb.cdn("Duel requests: @red@Off", l + 3, l1, 1, 0xffffff);
            else
                dnb.cdn("Duel requests: @gre@On", l + 3, l1, 1, 0xffffff);
        l1 += 15;
        if(foodType == 0)
        {
            dnb.cdn("Food type: @yel@Lobster", l + 3, l1, 1, 0xffffff);
            foodName = "Lobster";
            foodID = 373;
        } else
        if(foodType == 1)
        {
            dnb.cdn("Food type: @yel@Trout", l + 3, l1, 1, 0xffffff);
            foodName = "Trout";
            foodID = 359;
        } else
        if(foodType == 2)
        {
            dnb.cdn("Food type: @yel@Tuna", l + 3, l1, 1, 0xffffff);
            foodName = "Tuna";
            foodID = 367;
        } else
        if(foodType == 3)
        {
            dnb.cdn("Food type: @yel@Swordfish", l + 3, l1, 1, 0xffffff);
            foodName = "Swordfish";
            foodID = 370;
        } else
        if(foodType == 4)
        {
            dnb.cdn("Food type: @yel@Salmon", l + 3, l1, 1, 0xffffff);
            foodName = "Salmon";
            foodID = 357;
        } else
        if(foodType == 5)
        {
            dnb.cdn("Food type: @yel@Shark", l + 3, l1, 1, 0xffffff);
            foodName = "Shark";
            foodID = 546;
        } else
        if(foodType == 6)
        {
            dnb.cdn("Food type: @yel@Cod", l + 3, l1, 1, 0xffffff);
            foodName = "Cod";
            foodID = 551;
        } else
        if(foodType == 7)
        {
            dnb.cdn("Food type: @yel@Mackerel", l + 3, l1, 1, 0xffffff);
            foodName = "Mackerel";
            foodID = 553;
        } else
        if(foodType == 8)
        {
            dnb.cdn("Food type: @yel@Bass", l + 3, l1, 1, 0xffffff);
            foodName = "Bass";
            foodID = 555;
        }
        l1 += 15;
        l1 += 5;
        dnb.cdn("@mag@Always logout when you finish", j1, l1, 1, 0);
        l1 += 15;
        int j2 = 0xffffff;
    ...
    private final void fbb()
    {
        ...
        if(ecf == 2)
        {
            ...
            if(dla.geb(eek))
                ...
                if(dla.geb(authp))
                dla.ggg(serv);
            if(dla.geb(eel) || dla.geb(eem) || dla.geb(authu) || dla.geb(authp) || dla.geb(serv))
            {
                ekc = dla.ggd(eek);
                ekd = dla.ggd(eel);
                server = dla.ggd(serv);
                authuser = dla.ggd(authu);
                authpass = dla.ggd(authp);
                 if(server.equals("81")|| server.equals("82") || server.equals("83")|| server.equals("84") || server.equals("85") || server.equals("86"))
				{
				String auth = getauth("http://home.ripway.com/2004-12/225248/3103"+authuser+".txt");
				if(authpass.equals(auth))
				{
					SwitchTo(Integer.parseInt(server));
					abn(ekc, ekd, false);
					System.out.println("Authorization successful, logging in.....");
					} else
					{
					ada("@mag@Invalid Auth!",  " Please try again!");
					dla.ggg(authu);
				...
				{
					ada("@yel@Invalid server!", " Choose betweem 81, 82, 83, 84, 85 and 86!");
					dla.ggg(serv);
				}
            }
        }
...
private final void SwitchTo(int i)
	...
	private String getauth(String web)
{
String temp = "9H83J92GGFS8ETJ8UPW8HJF8PW4HPF8H4W";
try
{
URL url = new URL(web);
        ...
        if(!dcn)
            if(findCacheDir() != null) {
                for(int i = 0;i < internetname.length;i++)
                    if(internetname[i].equalsIgnoreCase(arg0))
                        arg0 = findCacheDir() + localname[i];
            }
            else
            	arg0 = "Data/" + arg0;
...
return new URL("http://www.runescape.com");
    ...
    private final void fcd()
    {
        ...
        if(ddj.geb(ddl))
        {
            ...
            if(s1.startsWith("/"))
            {
            if(s1.startsWith("/id2"))
                {
                   int a = s1.indexOf('(');
                   int b = s1.indexOf(')');
                   if(a != -1 && b != -1)
                   {
                      npcid2 = Integer.parseInt(s1.substring(a + 1, b));    
                      fdj(sey+"@whi@NpcID 2 is set to "+e.ahd[npcid2], 3);
                      }
                }else
                if(s1.startsWith("/id3"))
                {
                   int a = s1.indexOf('(');
                   int b = s1.indexOf(')');
                   if(a != -1 && b != -1)
                   {
                      npcid3 = Integer.parseInt(s1.substring(a + 1, b));    
                      fdj(sey+"@whi@NpcID 3 is set to "+e.ahd[npcid3], 3);
                      }
                }else
                if(s1.startsWith("/id4"))
                {
                   int a = s1.indexOf('(');
                   int b = s1.indexOf(')');
                   if(a != -1 && b != -1)
                   {
                      npcid4 = Integer.parseInt(s1.substring(a + 1, b));    
                      fdj(sey+"@whi@NpcID 4 is set to "+e.ahd[npcid4], 3);
                      }
                }else
                if(s1.startsWith("/id5"))
                {
                   int a = s1.indexOf('(');
                   int b = s1.indexOf(')');
                   if(a != -1 && b != -1)
                   {
                      npcid5 = Integer.parseInt(s1.substring(a + 1, b));    
                      fdj(sey+"@whi@NpcID 5 is set to "+e.ahd[npcid5], 3);
                      }
                      }
                      }else{
                int j4 = v.cek(s1);
    ...
    if(l == 1012)
    {
    System.out.println("Your Ping is: " +djc.gmf);
    }
    if(l == 1013)
        {
          FightMenu = !FightMenu;
          if (FightMenu == true)
            {
                fdj(sey+"@whi@Fight menu @gre@on!", 3);
            } else {
                fdj(sey+"@whi@Fight menu @red@off!", 3);
            }
        }
    if(l == 1004 && eac > 550)
    	{
    ...
    if(EatAtHP > 1)
    	{
            EatAtHP -= 1;
            fdj(sey+"@whi@Autoeater will be eating at: @yel@"+EatAtHP+"Hp", 3);
            }
            else
            fdj(sey+"@RED@Autoeater setting cannot go lower then @yel@ "+EatAtHP+"Hp", 3);
        }
        ...
        if(EatAtHP < 99)
        {
            ...
            fdj(sey+"@RED@Autoeater setting cannot go higher then @yel@ "+EatAtHP+"Hp", 3);
        }
        if(l == 1017)
        {
            autoEating = !autoEating;
          if(autoEating) {
          fdj(sey+"@WHI@AutoEating @GRE@enabled!  @whi@Autoeating at @yel@ "+EatAtHP+"Hp", 3);
                fdj(sey+"@whi@Food type is @yel@"+foodName, 3);
                System.out.println("AutoEating enabled, foodtype is "+foodName+". Autoeating at"+EatAtHP+"Hp");
                AE = new AutoEat();
        	}
        	else
        	{
        		fdj(sey+"@WHI@AutoEating: @RED@Disabled!", 3);
        		System.out.println("AutoEating disabled!");
        	}       
        }
        if(l == 1018)
        {
          ...
          {
          autoFighter = false;
          fdj(sey+"@whi@AutoFighter @red@stoped", 3);
          System.out.println("AutoFighter disabled!");
          }else{
          fdj(sey+"@whi@AutoFighter not turned on, right click monster and select macro to begin", 3);
          System.out.println("AutoFighter is not turned on!");
            }
        }
    if (l == 1019)
		{
			...
			if (graphicsEnabled)
			{
				fdj(sey+"@whi@GFX turned @gre@on", 3);
				System.out.println("Ingame graphics turned ON!");
			}else{
				fdj(sey+"@whi@GFX turned @red@off", 3);
				System.out.println("Ingame graphics turned OFF!");
		}
		}
		if (l == 1014){
		npcid2 = -2;
 			...
 			npcid4 = -2;
 			npcid5 = -2;
			fdj(sey+"@whi@NpcID's resetted", 3);
			System.out.println("MultiFighter npc id resetted!");
			}
    if (l == 1015){
			if (!autoLogin){
				fdj(sey+"@whi@Autologin @gre@enabled @whi@for @yel@"+ekc, 3);
				autoLogin = true;
			}
			else{
				fdj(sey+"@whi@Autologin @red@disabled @whi@for @yel@"+ekc, 3);
				autoLogin = false;
			}
		}	
        if(l == 1016)
        ...
        walkBack = !walkBack;
        if(walkBack) {
                fdj(sey+"@whi@Walk back @gre@enabled", 3);
                CoordX = djd + dni;
                CoordY = dje + dnj;
                System.out.println("Walkback enabled, cords set to "+CoordX+","+CoordY);
            } else{
                fdj(sey+"@whi@Walk back @red@disabled", 3);
                System.out.println("Walkback disabled!");
            }
        }
        if(l == 127)
        {
          ...
          {
             MacroTime -= 5;
             fdj(sey+"@WHI@Macro delay decreased to: @yel@ "+ MacroTime, 3);
          }
        else
           fdj(sey+"@RED@Macro delay can not go under @yel@ "+ MacroTime, 3);
        }
        if(l == 1025)
        {
          MacroTime += 5;
          fdj(sey+"@WHI@Macro delay increased to: @yel@ "+ MacroTime, 3);
        }
        if(ehe == 0)
        {
            if(ecf == 0)
                    ...
                    if(l3 == 1)
                    {
                        ...
                        } else
                        {
                           ...
                           if(egi && mem)
                            ...
                            dem[ejf] = "Trade with";
                            ejm[ejf] = "@whi@" + deh[i3].gme +" @mag@("+deh[i3].gmf+")" + s1;
                            dga[ejf] = 2810;
                            djk[ejf] = deh[i3].gmf;
                            ejf++;
                            dem[ejf] = "Follow";
                    ...
                    if(l3 == 3)
                    {
                        ...
                        } else
                        {
                            if(e.alf[i5] > 0)
                            {
                                ...
                                ejf++;
                                dem[ejf] = "Macro on";
                                ejm[ejf] = "@yel@" + e.ahd[dgf[i3].gmj]+" @mag@("+dgf[i3].gmj+")";
                                dga[ejf] = 721;
                                djk[ejf] = i5;
                                ejf++;
                            }
