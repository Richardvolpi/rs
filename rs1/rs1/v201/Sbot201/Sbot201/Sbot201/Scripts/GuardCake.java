...
public void ServerMessage(String message)
   {
      if (message.equals("@gam@You pick the guard's pocket"))
         thieved++;
      if (message.equals("@gam@I can't get close enough"))
    attack = true;
   }    
...
{ 
DisplayMessage("@gre@Bruncle: @whi@Invalid number of parameters. /RUN guardme [STYLE]@[Time till report]", 3); 
} else { 
...
{ 
  ...
  slept ++;
    ...
    if(!InCombat() && Running()&& GetCurrentStat(3) > (GetStat(3) - 5) && !attack)
    {
  SetFightMode(FightMode);
  int guard = GetNearestNPC(60);
  ThieveNPC(guard);
  Wait(700);
        }
        if (attack)
