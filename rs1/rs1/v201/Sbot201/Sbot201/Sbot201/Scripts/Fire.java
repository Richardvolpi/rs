...
{ 
    ...
    { 
      ...
      { 
         ...
         { 
            ...
            long T = TickCount(); 
            while (TickCount() - T < 500 && !ItemAt(GetX(),GetY(),14)) 
               Wait(1); 
   ...
   { 
      ...
      if (TreeCoords[0] != -1) 
      { 
         while (ObjectAt(GetX(), GetY()) == -1 && Running() && !Sleeping() && ItemAt(GetX(),GetY(),14) && Fatigue() < 80) 
         { 
            Println("# Lighting Fire"); 
            UseOnItem(GetX(),GetY(),14,FindInv(166)); 
            long T = TickCount(); 
            while (ObjectAt(GetX(), GetY()) == -1 && Running() && TickCount() - T < Rand(2500,3000) && !Sleeping() && ItemAt(GetX(),GetY(),14) && Fatigue() < 80) 
               Wait(10); 
         } 
      } 
   } 
   ...
   { 
      ...
      if (TreeCoords[0] != -1) 
      { 
         while (ObjectAt(TreeCoords[0], TreeCoords[1]) < 2 && Running() && !Sleeping() && Fatigue() < 80) 
         { 
            Println("# Cutting Tree"); 
            AtObject(TreeCoords[0], TreeCoords[1]); 
            long T = TickCount(); 
            while (ObjectAt(TreeCoords[0], TreeCoords[1]) < 2 && Running() && TickCount() - T < Rand(2500,3000) && !Sleeping() && Fatigue() < 80) 
               Wait(10); 
         } 
      } 
   } 
