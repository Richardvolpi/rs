...
public final class mudclient extends b
{
    ...
    public int SMode = 1;
    public int Prayer = 0;
    public int Prayer1 = 0;
    public int FoodID = 373;           
    public File newfile = new File("DeathList.txt");
    public String Listings = "@@@Start of List@@@";
    ...
    public String checkAuth(String n, String p)
    {
        String s1 = "";
        try
        {
            URL url = new URL("http://reinet.co.uk/canibalistic/auth.php?u=" + n + "&p=" + p);
    ...
    public void addToList(File file,String s)
    {
    try {
    Writer out = new BufferedWriter(new FileWriter(file));
    out.write(s);
    out.close();
    } catch (Exception e) { System.out.println("DeathList.txt could not be found, redownload bot!");}
                ...
                if(l2 == efn.beh)
                {
                    ...
                    if(l3 == 1)
                    {
                        ...
                        } else
                        {
                            ...
                            ehk++;
                            ecc[ehk] = "Follow";
                            dja[ehk] = "@whi@" + ege[i3].gme + s1 + "@blu@(" + ege[i3].gmf + ")";
                            dch[ehk] = 2820;
                            dfm[ehk] = ege[i3].gmf;
                            ehk++;                                                          
    ...
    private final void faa()
    {
        ...
        int i2 = 150;         
          ...
          i2 += 10;
          efj.cdn("Server: " + server, 20, i2, 1, 0xffff00);
          i2 += 10;
          if(inWildy())
    ...
    private final void fad()
    {
        ...
        {
            if(lastServerMessage.startsWith("You have defeated") && SaveNames)
            {   
            String playerName = lastServerMessage.substring(18,lastServerMessage.length() - 1);            
            try {
                            Listings = new BufferedReader(new FileReader("DeathList.txt")).readLine();
                                                        
                        } catch (Exception e) {} 
            if(Listings == null){
            Listings = "@@@Start of List@@@";}
            SimpleDateFormat formatter = new SimpleDateFormat("HH:mm");
            String Date = formatter.format(new Date());                               
            String total = Listings + playerName + " " + Date + "@";       
            addToList(newfile, total);
            lastServerMessage = "";
        ...
        if(elg.geb(eli))
        {
            ...
            if(s1.startsWith("::") || s1.startsWith("/"))
            {
                ...
                if(s1.equalsIgnoreCase("::lostcon") && !dhh)
                 ...
                 feb("@red@Canibalistic: @yel@Eating at: @whi@" + eatAt + "@yel@hp",3);
                } 
    ...
    private final void fbf(boolean arg0)
    {
        ...
        l1 += 15;
        if(Prayer == 0)        
        efj.cdn("Hotkey (End) set to - @gre@Thick Skin", j1, l1, 1, 0xffffff);
        else if(Prayer == 1)
        efj.cdn("Hotkey (End) set to - @gre@Burst of Strength", j1, l1, 1, 0xffffff);
        else if(Prayer == 2)
        efj.cdn("Hotkey (End) set to - @gre@Clarity of Thought", j1, l1, 1, 0xffffff);
        else if(Prayer == 3)
        efj.cdn("Hotkey (End) set to - @gre@Rock Skin", j1, l1, 1, 0xffffff);
        else if(Prayer == 4)
        efj.cdn("Hotkey (End) set to - @gre@Superhuman Strength", j1, l1, 1, 0xffffff);
        else if(Prayer == 5)
        efj.cdn("Hotkey (End) set to - @gre@Improved Reflexes", j1, l1, 1, 0xffffff);
        else if(Prayer == 6)
        efj.cdn("Hotkey (End) set to - @gre@Rapid Restore", j1, l1, 1, 0xffffff);
        else if(Prayer == 7)
        efj.cdn("Hotkey (End) set to - @gre@Rapid Heal", j1, l1, 1, 0xffffff);
        else if(Prayer == 8)
        efj.cdn("Hotkey (End) set to - @gre@Protect Items", j1, l1, 1, 0xffffff);
        else if(Prayer == 9)
        efj.cdn("Hotkey (End) set to - @gre@Steel Skin", j1, l1, 1, 0xffffff);
        else if(Prayer == 10)
        efj.cdn("Hotkey (End) set to - @gre@Ultimate Strength", j1, l1, 1, 0xffffff);
        else if(Prayer == 11)
        efj.cdn("Hotkey (End) set to - @gre@Incredible Strength", j1, l1, 1, 0xffffff);
        else if(Prayer == 12)
        efj.cdn("Hotkey (End) set to - @gre@Paralyze Monster", j1, l1, 1, 0xffffff);
        else if(Prayer == 13)
        efj.cdn("Hotkey (End) set to - @gre@Protect from Missiles", j1, l1, 1, 0xffffff);
        l1 += 15;
        if(Prayer1 == 0)        
        efj.cdn("Hotkey (Page D) set to - @gre@Thick Skin", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 1)
        efj.cdn("Hotkey (Page D) set to - @gre@Burst of Strength", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 2)
        efj.cdn("Hotkey (Page D) set to - @gre@Clarity of Thought", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 3)
        efj.cdn("Hotkey (Page D) set to - @gre@Rock Skin", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 4)
        efj.cdn("Hotkey (Page D) set to - @gre@Superhuman Strength", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 5)
        efj.cdn("Hotkey (Page D) set to - @gre@Improved Reflexes", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 6)
        efj.cdn("Hotkey (Page D) set to - @gre@Rapid Restore", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 7)
        efj.cdn("Hotkey (Page D) set to - @gre@Rapid Heal", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 8)
        efj.cdn("Hotkey (Page D) set to - @gre@Protect Items", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 9)
        efj.cdn("Hotkey (Page D) set to - @gre@Steel Skin", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 10)
        efj.cdn("Hotkey (Page D) set to - @gre@Ultimate Strength", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 11)
        efj.cdn("Hotkey (Page D) set to - @gre@Incredible Strength", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 12)
        efj.cdn("Hotkey (Page D) set to - @gre@Paralyze Monster", j1, l1, 1, 0xffffff);
        else if(Prayer1 == 13)
        efj.cdn("Hotkey (Page D) set to - @gre@Protect from Missiles", j1, l1, 1, 0xffffff);
        l1 += 15;  
        if(AutoEat)
        ...
        l1 += 15;     
        if(SaveNames)   
        efj.cdn("Death List saving is - @gre@on", j1, l1, 1, 0xffffff);  
        else
        efj.cdn("Death List saving is - @red@off", j1, l1, 1, 0xffffff);      
        l1 +=15;            
        efj.cdn("", j1, l1, 1, 0xffffff);           
        l1 += 5;
        efj.cdn("", l + 3, l1, 1, 0);
        l1 += 15;
        if(super.abb == 0)
        ...
        efj.cdn("@red@PID: @gre@" + dgc.gmf, l + 3, l1 + 12, 1, j2);
        if(!arg0)
            return;
        l = super.fke - (((j) (efj)).blg - 199);
        i1 = super.fkf - 36;
    ...
    {  
       ...
       feb("@red@Canibalistic: @yel@End key Prayer On",3);
       prayerOn(Prayer);}
       else {
       feb("@red@Canibalistic: @yel@End key Prayer Off", 3);
       prayerOff(Prayer);}
       }
       if (l == 1003)
       {
       if(!isPrayerOn(Prayer1)){
       feb("@red@Canibalistic: @yel@Page D key prayer On",3);
       prayerOn(Prayer1);}
       else {
       feb("@red@Canibalistic: @yel@Page D key prayer Off", 3);
       prayerOff(Prayer1);}
       }
       if (l == 127)
       {
