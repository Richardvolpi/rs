...
{
	$code = pun_trim($_GET['code']);
	$id = intval($_GET['uid']);
	
	if (($pun_config['o_invite_allow'] != '1' || $pun_user['id'] != $id) && $pun_user['g_id'] != PUN_ADMIN)
		message($lang_common['Bad request']);
	
	if ($pun_user['is_guest'])
		message($lang_common['No permission']);
	
	$db->query('DELETE FROM '.$db->prefix.'invites WHERE code=\''.$db->escape($code).'\' AND (owner='.intval($id).' OR '.$pun_user['g_id'].'='.PUN_ADMIN.')') or error('Unable to remove invite', __FILE__, __LINE__, $db->error());
	if(!$db->affected_rows())
		message('No such invite');
	
	if($id != PUN_ADMIN)
		$db->query('UPDATE '.$db->prefix.'users SET invites=invites+1 WHERE id='.intval($pun_user['id'])) or error('Unable to update users invite count', __FILE__, __LINE__, $db->error());
	
	redirect('profile.php?section=invite&id='.$id, 'Invitation cancelled');
}
else if ($action == 'invite')
{
	if ($pun_config['o_invite_allow'] != '1' && $pun_user['g_id'] != PUN_ADMIN)
		message($lang_common['Bad request']);
	
	if ($pun_user['is_guest'])
		message($lang_common['No permission']);
	
	if($pun_user['invites'] < 1 && $pun_user['g_id'] != PUN_ADMIN)
		message('Sorry you have no invites to give out just now');
	
	include PUN_ROOT.'include/email.php';
	
	$email = pun_trim($_POST['email']);
	$message = pun_trim($_POST['message']);
	
	if(!is_valid_email($email))
		message('Invalid email address');
	else if(is_banned_email($email))
		message('That email address is banned!');
	else if ($message == '')
		message($lang_misc['No e-mail message']);
	else if (strlen($message) > 65535)
		message($lang_misc['Too long e-mail message']);
	
	$result = $db->query('SELECT 1 FROM '.$db->prefix.'users WHERE UPPER(email) LIKE UPPER(\''.$db->escape($email).'\')') or error('Unable to check for existing user', __FILE__, __LINE__, $db->error());
	if($db->num_rows($result))
		message('That user already has an account!');
	
	$code = strtoupper(md5(time()));
	
	$db->query('INSERT INTO '.$db->prefix.'invites(code, email, owner, time) VALUES(\''.$code.'\', \''.$db->escape($email).'\', \''.intval($pun_user['id']).'\', '.time().')') or error('Unable to insert invite', __FILE__, __LINE__, $db->error());
	if($pun_user['g_id'] != PUN_ADMIN)
		$db->query('UPDATE '.$db->prefix.'users SET invites=invites-1 WHERE id='.intval($pun_user['id'])) or error('Unable to update users invite count', __FILE__, __LINE__, $db->error());
	
	// Load the "invite" template
      	$mail_tpl = trim(file_get_contents(PUN_ROOT.'lang/'.$pun_user['language'].'/mail_templates/invite.tpl'));
      	...
      	$mail_message = str_replace('<invite_url>', $pun_config['o_base_url'].'/register.php?email='.urlencode($email).'&invite='.urlencode($code), $mail_message);
      	...
      	redirect('profile.php?section=invite&id='.$pun_user['id'], 'Invitation sent');
}
