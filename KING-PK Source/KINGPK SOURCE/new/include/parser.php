...
$smiley_text = array(':D', ':)', ':(', ';)', ':@', ':P', '8)', ':O', ':roll:');
$smiley_img = array('grin.gif', 'smile.gif', 'sad.gif', 'wink.gif', 'angry.gif', 'tounge.gif', 'cool.gif', 'shock.gif', 'roll.gif');
...
function preparse_bbcode($text, &$errors, $is_signature = false)
{
	...
	else
	{
		...
		if (preg_match('#\[quote=(&quot;|"|\'|)(.*)\\1\]|\[quote\]|\[/quote\]|\[code\]|\[/code\]|\[youtube\]|\[/youtube\]#i', $text))
...
function handle_url_tag($url, $link = '')
{
	...
	else if (!preg_match('#^([a-z0-9]+)://#', $url, $bah)) 	// Else if it doesn't start with abcdef://, we add http://
		$full_url = 'http://'.$full_url;
...
function do_bbcode($text)
{
	...
	$pattern = array('#\[b\](.*?)\[/b\]#s',
					 ...
					 '#\[youtube\]http://(.+\.|)youtube\.com\/watch\?v=(.*?)\[/youtube\]#se',);
...
function handle_youtube_tag($text) {
	$text = pun_htmlspecialchars($text);
	return '<object width="425" height="350"><param name="movie" value="http://www.youtube.com/v/'.$text.'"></param><embed src="http://www.youtube.com/v/'.$text.'" type="application/x-shockwave-flash" width="425" height="350"></embed></object>';
}
...
function do_smilies($text)
{
	...
	for ($i = 0; $i < $num_smilies; ++$i)
		$text = preg_replace("#(?<=.\W|\W.|^\W)".preg_quote($smiley_text[$i], '#')."(?=.\W|\W.|\W$)#mi", '$1<img src="img/smilies/'.$smiley_img[$i].'" width="15" height="15" alt="'.substr($smiley_img[$i], 0, strrpos($smiley_img[$i], '.')).'" />$2', $text);
