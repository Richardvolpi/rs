...
{
	$tpl_temp = '<div id="brdwelcome" class="inbox">'."\n\t\t\t".'<ul class="conl">'."\n\t\t\t\t".'<li>'.$lang_common['Logged in as'].' <strong>'.pun_htmlspecialchars($pun_user['username']).'</strong>'.($pun_user['group_id'] == $pun_config['s_new_gid'] && $pun_config['s_length'] > 0 ? ' (Subscription expires '.format_time($pun_user['sub_expires'], true).' - <a href="subscription.php">Extend</a>)' : '').'</li>'."\n\t\t\t\t".'<li>'.$lang_common['Last visit'].': '.format_time($pun_user['last_visit']).'</li>';
	...
	if ($pun_user['g_id'] < PUN_GUEST)
	{
		...
		$result_header = $db->query('SELECT COUNT(id) FROM '.$db->prefix.'rscd_reports WHERE zapped IS NULL') or error('Unable to fetch reports info', __FILE__, __LINE__, $db->error());

		if ($db->result($result_header))
			$tpl_temp .= "\n\t\t\t\t".'<li class="reportlink"><strong><a href="admin_rscd_reports.php">There are new RSCD reports</a></strong></li>';
...
if ($pun_config['o_announcement'] == '1' && $pun_user['g_read_board'] != '0')
{
	ob_start();
