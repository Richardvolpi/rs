...
if (isset($_POST['form_sent']))
{
	...
	if ($form['rscd_rules_message'] != '')
		$form['rscd_rules_message'] = pun_linebreaks($form['rscd_rules_message']);
	else
	{
		$form['rscd_rules_message'] = 'Enter your rules here.';

		if ($form['rscd_rules'] == '1')
			$form['rscd_rules'] = '0';
	}
					...
					<fieldset>
						...
						<div class="infldset">
							<table class="aligntop" cellspacing="0">
								...
								<tr>
									<th scope="row">Enable Invites</th>
									<td>
										<input type="radio" name="form[invite_allow]" value="1"<?php if ($pun_config['o_invite_allow'] == '1') echo ' checked="checked"' ?> />&nbsp;<strong>Yes</strong>&nbsp;&nbsp;&nbsp;<input type="radio" name="form[invite_allow]" value="0"<?php if ($pun_config['o_invite_allow'] == '0') echo ' checked="checked"' ?> />&nbsp;<strong>No</strong>
										<span>Turn on/off the invite system.</span>
									</td>
								</tr>
								<tr>
								...
								<tr>
									<th scope="row">Use RSCD rules</th>
									<td>
										<input type="radio" name="form[rscd_rules]" value="1"<?php if ($pun_config['o_rscd_rules'] == '1') echo ' checked="checked"' ?> />&nbsp;<strong>Yes</strong>&nbsp;&nbsp;&nbsp;<input type="radio" name="form[rscd_rules]" value="0"<?php if ($pun_config['o_rscd_rules'] == '0') echo ' checked="checked"' ?> />&nbsp;<strong>No</strong>
										<span>When enabled, users must agree to a set of rules when registering for RSCD (enter text below). The rules will always be available through a link in the navigation table at the top of every page.</span>
									</td>
								</tr>
								<tr>
									<th scope="row">RSCD Rules</th>
									<td>
										<textarea name="form[rscd_rules_message]" rows="10" cols="55"><?php echo pun_htmlspecialchars($pun_config['o_rscd_rules_message']) ?></textarea>
										<span>Here you can enter any rules or other information that the user must review and accept when registering for RSCD. If you enabled rules above you have to enter something here, otherwise it will be disabled. This text will not be parsed like regular posts and thus may contain HTML.</span>
									</td>
								</tr>
							</table>
						</div>
