...
$result = $db->query('SELECT u.email, u.title, u.url, u.location, u.use_avatar, u.signature, u.email_setting, u.num_posts, u.registered, u.admin_note, p.id, p.poster AS username, p.poster_id, p.poster_ip, p.poster_email, p.message, p.hide_smilies, p.posted, p.edited, p.edited_by, g.g_id, g.g_title, g.g_user_title, b.id AS b_id, o.user_id AS is_online FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'users AS u ON u.id=p.poster_id INNER JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id LEFT JOIN '.$db->prefix.'bans AS b ON b.username=u.username LEFT JOIN '.$db->prefix.'online AS o ON (o.user_id=u.id AND o.user_id!=1 AND o.idle=0) WHERE p.topic_id='.$id.' ORDER BY p.id LIMIT '.$start_from.','.$pun_user['disp_posts'], true) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
while ($cur_post = $db->fetch_assoc($result))
{
	...
	if ($cur_post['poster_id'] > 1)
	{
		...
		if ($cur_post['g_id'] == PUN_ADMIN)
			$rank = '<img src="img/skin/admin.gif" alt="Admin" />&nbsp;';
		else if ($cur_post['g_id'] == PUN_MOD)
			$rank = '<img src="img/skin/mod.gif" alt="Moderator" />&nbsp;';
		else if ($cur_post['g_id'] == RSCD_PMOD)
			$rank = '<img src="img/skin/pmod.gif" alt="Player Mod" />&nbsp;';
		...
		if ($pun_config['o_show_user_info'] == '1')
		{
			...
			$user_info[] = '<dd>Group: '.(isset($cur_post['b_id']) ? 'Banned' : $cur_post['g_title']);
...
<div id="p<?php echo $cur_post['id'] ?>" class="blockpost<?php echo $vtbg ?><?php if (($post_count + $start_from) == 1) echo ' firstpost'; ?>">
	...
	<div class="box">
		<div class="inbox">
			<div class="postleft">
				<dl>
					<dt><?php echo $rank; ?><strong><?php echo $username ?></strong></dt>
