	...
	private boolean busy(Player player) {
		...
		if(player.getLocation().inModRoom()) {
			...
			return;
                ...
                if(world.getDuel() <= 0) {
			player.getActionSender().sendMessage("Duelings for pussy's get in the Wild");
			unsetOptions(player);
			unsetOptions(affectedPlayer);
			return;
		}
			...
			case 222: // Sending duel request
				...
				if(affectedPlayer == null || !player.withinRange(affectedPlayer, 8) || player.isDueling() || player.tradeDuelThrottling()) {
					...
					return;
                               ...
                               if(player.isDueling() || affectedPlayer.isDueling()) {
					player.getActionSender().sendMessage("You can not send duel requests while in a duel");
					return;
				}
