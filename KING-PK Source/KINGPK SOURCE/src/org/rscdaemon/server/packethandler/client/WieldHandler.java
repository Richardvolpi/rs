	...
	private void wieldItem(Player player, InvItem item) {
		...
		for(Entry<Integer, Integer> e : item.getWieldableDef().getStatsRequired()) {
			if(player.getMaxStat(e.getKey()) < e.getValue()) {
                ...
                if(item.isWielded() == item.getID() > 1) {
			System.out.println("Warning: " + player.getUsername() + " tried to double wield.");
			unWieldItem(player, item, true);
			item.setWield(false);
	 	}
		if(!youNeed.equals("")) {
		...
		if(EntityHandler.getItemWieldableDef(item.getID()).p2pOnly() && !player.getLocation().inp2pWild() && !player.getLocation().inp2pWild2() && !player.getLocation().inp2pWild3()) {
