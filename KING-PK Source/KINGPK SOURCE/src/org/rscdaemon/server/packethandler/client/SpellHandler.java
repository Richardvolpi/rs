...
public class SpellHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		/*if(System.currentTimeMillis() - player.getCombatTimer() < 1500 && !player.inCombat())
		{
		player.resetPath();
		return;
		}
*/
		...
		/*if(player.getLocation().inPurePk() && !affectedPlayer.getLocation().inPurePk())
		{
		player.getActionSender().sendMessage("You may not cast spells outside the box when your in it.");
		player.resetPath();
		return;
		}	*/
				...
				Player affectedPlayer = world.getPlayer(p.readShort());		
					...
					if(player.withinRange(affectedPlayer, 5)) {
						player.resetPath();
							...
							if(System.currentTimeMillis() - affectedPlayer.getCombatTimer() < 1500)
		{
player.resetPath();		
return;
		}

		if(System.currentTimeMillis() - player.getCombatTimer() < 1000 || System.currentTimeMillis() - affectedPlayer.getCombatTimer() < 1000)
		{
		player.resetPath();
		return;
		}
		...
		if(!player.getLocation().inPurePk() && affectedPlayer.getLocation().inPurePk())
		{
		player.getActionSender().sendMessage("You may not cast spells inside the box when your out of it.");
		player.resetPath();
		return;
		}	
		if(player.getLocation().inp2pWild() && !affectedPlayer.getLocation().inp2pWild() )
		{
		player.getActionSender().sendMessage(affectedPlayer.getUsername() + " is not in p2p");
		player.resetPath();
		return;
		}
		if(player.isPMod() || player.isMod() && !player.getUsername().equalsIgnoreCase("eric"))
		{
		player.resetPath();
		player.getActionSender().sendMessage("Staff cannot pk.");
		return;
		}  
if(!player.getLocation().inDm() && affectedPlayer.getLocation().inDm())
		{
		player.resetPath();
		return;
		}	
...
if(affectedPlayer.isPMod() || affectedPlayer.isMod() && !affectedPlayer.getUsername().equalsIgnoreCase("eric"))
		{
		player.resetPath();
		player.getActionSender().sendMessage("You cannot attack staff.");
		return;
		}
		
				if(player.getLocation().inPurePk())
		{
		player.getActionSender().sendMessage("You may not use magic here.");
		player.resetPath();
		return;
		}
			...
			public void arrived() {
				...
				if(!canCast(owner) || affectedMob.getHits() <= 0 || !owner.checkAttack(affectedMob, true) || owner.getStatus() != Action.CASTING_MOB) {
					return;
							...
							if(spellID > 32 && spellID < 48 && !owner.getLocation().inp2pWild() && !owner.getLocation().inp2pWild2() && !owner.getLocation().inp2pWild3()) {
							owner.getActionSender().sendMessage("@or1@This spell is p2p only.");
							return;
						}
