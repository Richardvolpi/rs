...
public class World {
	...
	public void registerPlayer(long user, String ip) {
		...
		ResultSet result;
		try {
			result = Server.db.getQuery("SELECT owner, block_private FROM `rscd_players` WHERE `user`=" + user);
			if(!result.next()) {
				return;
			}
			...
			Server.db.updateQuery("UPDATE `rscd_players` SET loggedin=" + 1 + ", world='" + id + "' WHERE user=" + user);
