		...
		switch(((LSPacket)p).getID()) {
			...
			case 11: // Add friend
				try {
					...
					if(Server.db.getQuery("SELECT 1 FROM `rscd_players` AS p LEFT JOIN `rscd_friends` AS f ON f.user=p.user WHERE (p.block_private=0 OR f.friend=" + user + ") AND p.user=" + friend).next()) {
						w = server.findWorld(friend);
						if(w != null) {
							world.getActionSender().friendLogin(user, friend, w.getID());
						}
					}
					if(Server.db.getQuery("SELECT 1 FROM `rscd_players` AS p LEFT JOIN `rscd_friends` AS f ON f.friend=p.user WHERE p.block_private=1 AND f.user=" + friend + " AND p.user=" + user).next()) {
						w = server.findWorld(friend);
						if(w != null) {
			...
			case 12: // Remove friend
				try {
					...
					if(Server.db.getQuery("SELECT 1 FROM `rscd_players` WHERE block_private=1 AND user=" + user).next()) {
						w = server.findWorld(friend);
						if(w != null) {
