...
public class PrivacySettingHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		switch(idx) {
			case 0: // Chat block
				try { Server.db.updateQuery("UPDATE `rscd_players` SET block_chat=" + (on ? 1 : 0) + " WHERE user=" + user); } catch(SQLException e) { Server.error(e.getMessage()); }
				break;
			case 1: // Private block
				try {
					Server.db.updateQuery("UPDATE `rscd_players` SET block_private=" + (on ? 1 : 0) + " WHERE user=" + user);
					ResultSet result = Server.db.getQuery("SELECT user FROM `rscd_friends` WHERE friend=" + user + " AND user NOT IN (SELECT friend FROM `rscd_friends` WHERE user=" + user + ")");
					while(result.next()) {
				...
				break;
			case 2: // Trade block
				try { Server.db.updateQuery("UPDATE `rscd_players` SET block_trade=" + (on ? 1 : 0) + " WHERE user=" + user); } catch(SQLException e) { Server.error(e.getMessage()); }
				break;
			case 3: // Duel block
				try { Server.db.updateQuery("UPDATE `rscd_players` SET block_duel=" + (on ? 1 : 0) + " WHERE user=" + user); } catch(SQLException e) { Server.error(e.getMessage()); }
				break;
		}
