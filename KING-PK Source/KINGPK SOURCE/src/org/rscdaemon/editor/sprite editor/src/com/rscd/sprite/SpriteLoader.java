...
import com.rscd.sprite.jagex.DataOperations;
import com.rscd.sprite.jagex.DataFileDecrypter;
import com.rscd.sprite.entityhandling.EntityHandler;
...
public class SpriteLoader {
	...
	private final void loadTextures() {
		...
		byte[] textures = loadFile("textures.jag");
		...
		for(int tileIndex = 0;tileIndex < 55;tileIndex++) {
			...
			int j = textureMap.get(TEXTURE_TEMP).getSomething1();
	...
	private final void loadEntities() {
		...
		byte[] entity = loadFile("entity.jag");
		byte[] entityIndex = DataOperations.loadDataFile("index.dat", 0, entity);
		byte[] entityMembers = loadFile("entity.mem");
