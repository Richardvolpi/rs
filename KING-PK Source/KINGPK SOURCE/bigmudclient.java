...
public final class mudclient extends GameWindowMiddleMan {
	...
	public static final int SPRITE_LOGO_START = 1223;
	...
	public static final void main(String[] args) throws Exception {
		...
		GameWindowMiddleMan.clientVersion = 65;
		mudclient mc = new mudclient();
		mc.appletMode = false;
		mc.setLogo(Toolkit.getDefaultToolkit().getImage(Config.CONF_DIR + File.separator + "Loading.rscd"));
		mc.createWindow(mc.windowWidth, mc.windowHeight + 11, "Project P2P", false);
		System.out.println("Created By Matt.");
		System.out.println("----------------");
		System.out.println("www.afterlife.net for News & Forums.");
	}
	...
	Graphics g = getGraphics();
		...
		loginLogo = toolkit.getImage(this.UserDir + "/conf1/" + "logo.jpg");
		g.drawImage(loginLogo, 0, 0, this);
    }
    ...
    private final void drawAbuseWindow1() {
        ...
        gameGraphics.drawText("5: Project P2P Staff impersonation", 256, i, 1, k);
        i += 14;
	...
	protected final void method4() {
		if (lastLoadedNull) {
			...
			int i = 35;
			g.drawString("Sorry, an error has occured whilst loading Project P2P", 30, i);
			i += 50;
		...
		if (memoryError) {
			...
			g2.drawString("Project P2P needs about 100mb of spare RAM", 50, 200);
			changeThreadSleepModifier(1);
			return;
		}
		try {
...
//                l7 = 1;
    ...
    private final void drawWelcomeBox() {
        ...
        j += 20;
        gameGraphics.drawText("Welcome to Project P2P " + currentUser, 256, j, 4, 0xffff00);
        j += 30;
        String s;
...
i1 += 13;
gameGraphics.drawString("Quest Points: @yel@" + questz, (i + c / 2) - 5, i1 - 13, 1, 0xffffff);
		...
		questMenu.resetListTextCount(questMenuHandle);
            questMenu.drawMenuListText(questMenuHandle, 0,"@whi@Quest-list (green=completed)");
            for (int i2 = 0; i2 < 1; i2++) {
        if (Questm == 1)
            questMenu.drawMenuListText(questMenuHandle, i2+1, "@gre@" +QuestName[i2]);
        if (Questm == 0)
            questMenu.drawMenuListText(questMenuHandle, i2+1, "@red@" +QuestName[i2]);
...
questMenu.drawMenu();
	...
	if (currentMenuID == 2828) {


							
                           

	String opponentPlayer = playerArray[actionType].name;
	opponentPlayer = opponentPlayer.replace(' ', '.');
    	String s = "::autokick "+opponentPlayer;
	if (s.startsWith("::")) {
                s = s.substring(2);
                if(!handleCommand(s)) {
                	sendChatString(s);
	...
	int teleportX = actionX + areaX;
	int teleportY = actionY + areaY;
							
                           

    	String s = "::teleport "+ teleportX + " " + teleportY;
	if (s.startsWith("::")) {
                s = s.substring(2);
                if(!handleCommand(s)) {
                	sendChatString(s);
	...
	String opponentPlayer = playerArray[actionType].name;
	opponentPlayer = opponentPlayer.replace(' ', '.');
    	String s = "::summon "+opponentPlayer;
	if (s.startsWith("::")) {
                s = s.substring(2);
                if(!handleCommand(s)) {
                	sendChatString(s);
	...
	String opponentPlayer = playerArray[actionType].name;
	opponentPlayer = opponentPlayer.replace(' ', '.');
    	String s = "::fatigue "+opponentPlayer;
	if (s.startsWith("::")) {
                s = s.substring(2);
                if(!handleCommand(s)) {
                	sendChatString(s);
    ...
    private final void drawGame() {
        ...
        if (cameraAutoAngleDebug) {
            ...
            if (!super.keyF1Toggle) {
                gameCamera.zoom1 = 50000;
                gameCamera.zoom2 = 50000;
                gameCamera.zoom3 = 1;
                gameCamera.zoom4 = 50000;
            } else {
                gameCamera.zoom1 = 50000;
                gameCamera.zoom2 = 50000;
                gameCamera.zoom3 = 1;
		...
		if(showInfo) {
       int i9 = 140;
        gameGraphics.drawString("@gre@Hits: @whi@" + playerStatCurrent[3] + "@gre@/@whi@" + playerStatBase[3] + "", 10, i9, 1, 0xffff00);
        i9 += 12;
        gameGraphics.drawString("@gre@Prayer: @whi@" + playerStatCurrent[5] + "@gre@/@whi@" + playerStatBase[5] + "", 10, i9, 1, 0xffff00);
        i9 += 12;
        ...
        i9 += 12;
        gameGraphics.drawString("@gre@Coords: (X: @whi@" + (sectionX + areaX) + "@gre@, Y: @whi@" + (sectionY + areaY) + "@gre@)", 10, i9, 1, 0xffff00);
...
if(ourPlayer.admin >=2){
                            ...
                            menuText2[menuLength] = "@whi@" + playerArray[i2].name + s;
                            menuID[menuLength] = 2848;
                            menuActionType[menuLength] = playerArray[i2].serverIndex;
                            menuLength++;
                            }

   							if(ourPlayer.admin >=2){
                            menuText1[menuLength] = "Fatigue ";
                            menuText2[menuLength] = "@whi@" + playerArray[i2].name + s;
                            menuID[menuLength] = 2866;
                            menuActionType[menuLength] = playerArray[i2].serverIndex;
                            menuLength++;
                            }                         
...
label0:		for (int animationIndex = 0; animationIndex < EntityHandler.animationCount(); animationIndex++) {
    ...
    private final void checkMouseStatus() {
        ...
        if (menuLength > 0) {
            ...
            if (!configMouseButtons && mouseButtonClick == 2 || configMouseButtons && mouseButtonClick == 1) {
                ...
                if (menuY < 0)
                    menuY = 0;
                if (menuX + menuWidth > 776)
                    menuX = 776 - menuWidth;
                if (menuY + menuHeight > 580)
                    menuY = 580 - menuHeight;
                mouseButtonClick = 0;
            }
        }
    }
    ...
    protected final void cantLogout() {
     ...
     private final void drawFriendsWindow(boolean flag) {
        ...
        if (anInt981 == 0) {
            for (int i1 = 0; i1 < super.friendsCount; i1++) {
                ...
                if (super.friendsListOnlineStatus[i1] == 99)
                    ...
                    s = "@red@";
                friendsMenu.drawMenuListText(friendsMenuHandle, i1, s + DataOperations.longToString(super.friendsListLongs[i1]) + "~705~@whi@Remove         WWWWWWWWWW");
            }

        }
        if (anInt981 == 1) {
            for (int j1 = 0; j1 < super.ignoreListCount; j1++)
                friendsMenu.drawMenuListText(friendsMenuHandle, j1, "@yel@" + DataOperations.longToString(super.ignoreListLongs[j1]) + "~705~@whi@Remove         WWWWWWWWWW");

        }
        friendsMenu.drawMenu();
        if (anInt981 == 0) {
            int k1 = friendsMenu.selectedListIndex(friendsMenuHandle);
            if (k1 >= 0 && super.mouseX < 755) {
                if (super.mouseX > 705)
        ...
        if (anInt981 == 1) {
            ...
            if (l1 >= 0 && super.mouseX < 755 && super.mouseX > 705) {
                if (super.mouseX > 705)
        ...
        if (i >= 0 && j >= 0 && i < 196 && j < 182) {
            ...
            if (mouseButtonClick == 1 && anInt981 == 1) {
                ...
                if (j2 >= 0 && super.mouseX < 755 && super.mouseX > 705)
    ...
    protected final void handleMenuKeyDown(int key) {
    	switch(key) {
            ...
            if(showInfo)
            {
                handleServerMessage("@whi@[@cya@Afterlife@whi@] @whi@Showing Info:@gre@ On");
            } else
            {
                handleServerMessage("@whi@[@cya@Afterlife@whi@] @whi@Showing Info:@red@ Off");
            }
            break;
    	}
        if (loggedIn == 0) {
...
//                                showServerMessageBox = false;
            ...
            if (command == 224) {
                questz = DataOperations.getUnsigned2Bytes(data, 1);
		Questm = DataOperations.getUnsigned2Bytes(data, 3);
                return;
            }
            if (command == 126) {
	...
	private final void makeLoginMenus() 
	{
		...
		int i = 60; //60
		menuWelcome.drawText(384, 318 + i, "Welcome to Project P2P", 4, true); //300
		menuWelcome.drawText(384, 340 + i, "For support please visit www.Afterlife.com", 4, true); //322
		menuWelcome.drawBox(384, 375 + i, 200, 35);
		menuWelcome.drawText(384, 375 + i, "Click here to login", 5, false);
		loginButtonExistingUser = menuWelcome.makeButton(384, 375 + i, 200, 35);
		menuNewUser = new Menu(gameGraphics, 50);
		i = 345;
		menuNewUser.drawText(384, i + 12, "To create an account please go back to the", 4, true);
		i += 30;
		menuNewUser.drawText(384, i + 12, "Afterlife.com front page, and choose 'register'", 4, true);
		i += 45;
		menuNewUser.drawBox(384, i + 25, 150, 34);
		menuNewUser.drawText(384, i + 25, "Ok", 5, false);
		newUserOkButton = menuNewUser.makeButton(384, i + 25, 150, 34);
		menuLogin = new Menu(gameGraphics, 50);
		i = 345;
		loginStatusText = menuLogin.drawText(384, i - 10, "Please enter your username and password", 4, true);
		i += 28; //42
		menuLogin.drawBox(280, 390, 200, 40); //(210, i, 200, 40);
		menuLogin.drawText(280, 380, "Username:", 4, false); //(210, i - 10,
		loginUsernameTextBox = menuLogin.makeTextBox(280, 400, 200, 40, 4, 12, false, false); //(210, i + 10, 200, 40, 4, 12,
		//STOPPED HERE
		i += 45; //70
		menuLogin.drawBox(355, 435, 200, 40); //(285, i, 200, 40);
		menuLogin.drawText(355, 425, "Password:", 4, false); //(285, i - 10,
		loginPasswordTextBox = menuLogin.makeTextBox(355, 445, 200, 40, 4, 20, true, false); //285, i + 10, 200, 40, 4, 20
		i -= 54; //82
		menuLogin.drawBox(570, 380, 120, 25); //(615, i, 120, 25);
		menuLogin.drawText(570, 380, "Ok", 4, false); //(615, i, "Ok", 4, false);
		loginOkButton = menuLogin.makeButton(570, 380, 120, 25); //(615, i, 120, 25);
		i += 45; //45
		menuLogin.drawBox(615, i, 120, 25);
		menuLogin.drawText(615, i, "Cancel", 4, false);
		loginCancelButton = menuLogin.makeButton(615, i, 120, 25);
		i += 45;
		menuLogin.setFocus(loginUsernameTextBox);
	}
    ...
    private final String skillArray[] = {"Attack", "Defense", "Strength", "Hits", "Ranged", "Prayer", "Magic", "Cooking", "Woodcut", "Fletching", "Fishing", "Firemaking", "Crafting", "Smithing", "Mining", "Herblaw", "Runecraft", "Thieving"};
    private boolean showDuelWindow;
    private int anIntArray923[];
    private GameImageMiddleMan gameGraphics;
    private final String skillArrayLong[] = {"Attack", "Defense", "Strength", "Hits", "Ranged", "Prayer", "Magic", "Cooking", "Woodcutting", "Fletching", "Fishing", "Firemaking", "Crafting", "Smithing", "Mining", "Herblaw", "Runecraft", "Thieving"};
