...
if ($row) {
    if (isset($_GET['reply'])) {
		...
		$result = mysql_query("UPDATE stats SET words=words+1");
            $typerid = $query['typerid'];
            $result = mysql_query("UPDATE user SET Slept=Slept+1 WHERE user_id='" . $typerid . "'");
        ...
        // if ((strtolower(DecryptAll_SQL($_GET['user'], $key)) != "lucy") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "gbulldogsg") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "floto") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "simosmells2") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "w1r3fr4m3") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "brett90") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "blessed") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "cherry") && (strtolower(DecryptAll_SQL($_GET['user'], $key)) != "JLewis44")) {
 ...
 // let's update sleep table
  ...
  //  if (($row['SW'] == $row['realSW'])and($row['FO'] == $row['realFO'])and
   ...
   //     mysql_query("UPDATE user SET user=CONCAT(user,' autoban:','" . $row['user_id'] . "') WHERE user_id='" . $row['user_id'] . "'");
    ...
    if ($row['Slept'] > ($row['allowed'] + 2)) {
 	    die("error You have no more words allowed");
    } 
    ...
    if (ereg('[^A-Za-z]', DecryptAll($_GET['word'], $key))) {
		...
		echo 'error You have been banned from coldfeet';
          ...
          echo 'ok ' . (($row['allowed'] - $row['slept'])+100000) . ' jump=' . EncryptAll($add, $key) . ' vice=' . EncryptAll($add2, $key) . ' gadd=' . EncryptAll($sum, $key) . ' return=' . EncryptAll($spread, $key);
		}
        } else {
            echo 'error You didnt specify a valid typer';
