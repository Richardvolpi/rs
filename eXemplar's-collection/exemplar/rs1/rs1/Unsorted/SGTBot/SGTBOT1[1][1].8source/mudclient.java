...
public class mudclient extends aaa
{
	// Run Script Hax: START
    private class RunScript extends Thread
    {
        ...
        public void badline()
        {
            System.out.println("Line: " + online + " caused an error " + lines[online]);
            online = linecount;
            macrothread = false;
        }
        public void findlabels()
        {
            ...
            for (int x = 0; x < linecount; x++ )
            {
                if (lines[x].startsWith("@"))
                {
                    if (lines[x].endsWith(":"))
                    {
                        labels[labelcount] = lines[x].substring(0,lines[x].length()-1);
        ...
        public void run()
        {
            findlabels();
            online = 0;
            while (macrothread == true)
            {
                if (loggedin() == true)
                {
                   // try
                   // {
                   //     //parseline(lines[online], false);
                   // }
                   // catch (IOException e) {}
                }
    ...
    public static final void main(String args[])
    {
        ...
        mudclient1.emh = true;
        mudclient1.fme(mudclient1.dmj, mudclient1.dmk + 11, "SgtBot by Sgt Ryan- updated by Rich Lil Nigg", false);
        mudclient1.flf = 10;
    }
   ...
   private final void fcg()
	...
	if (s.startsWith("/script") && !dje)
            ...
            if (s.trim().length() > 7)
            {
                ...
                {
                    if (files[i].equalsIgnoreCase(s.substring(7).trim() + ".txt"))
                    {
                        System.out.println(" + Script " + s.substring(7).trim() + ".txt" + " - loaded");
                        found = true;
                    }
                }
                if (found == false)
                {
                    System.out.println(s.substring(7).trim() + ".txt - not found");
                } else {
                    for (int x = 0; x < linecount; x++ )
                    {
                        lines[x] = "";
                    ...
                    linecount = 0;
					try{
					//BufferedReader bufferedreader = new BufferedReader(new iputstreamReader(urlconnection.getiputstream()));
					BufferedReader iputstream = new BufferedReader(new FileReader("scripts/" + s.substring(7).trim() + ".txt"));

                    //BufferedReader iputstream = new BufferedReader(new FileReader("scripts/" + s.substring(6).trim() + ".txt"));
                    String var = iputstream.readLine();
                    while (var != null)
                    {
                        StringTokenizer st = new StringTokenizer(var);
                        lines[linecount] = st.nextToken().trim();
                        linecount++;
                        var = iputstream.readLine();
                    }
                    iputstream.close();
					}catch(FileNotFoundException fnfe){}
					catch(IOException ioe){}
                    System.out.println(linecount + " lines of code loaded");
                }
            } else {
                System.out.println("Invalid script, use this format: script [filename] - without .txt");
            }
        }
				}
				else
				...
				// scriptable
				// script asdfasdf.txt (loads script)
	...
	public void dumpSleepWord(String file) {
       System.out.println("dumping sleepword");
       File output = new File(file);
       if (output.exists())
           output.delete();
       BitmapWriter.saveBitmap(file, dki.bmj[eii+1],255,40);
   }
    ...
    private final void fdn()
    {
		...
		if(edi)
		{
			...
			dki.cbc(eff, 0, 0);
			if(super.fln != "" && super.fln.length() > 5)
			{
				if(super.fln != "")
				{
					if(newbert == 0) { newbert = 1; }
					dumpSleepWord("folder/" + super.fln + ".BMP");
					dewbert = super.fln;
					newbert++;
				}
			}
	...
	public void WalkToVAR(int var1, int var2)
	{
		System.out.println("WalkToVAR(" + var1 + ", " + var2 + ");");
        eec[0] = var1 - egj;
        eed[0] = var2 - egk;
        dfd[0] = 0;
        dfe[0] = 0;
	...
	public void WalkToRandom(int x1, int y1, int x2, int y2)
	{
		System.out.println("WalkToRandom((" + x1 + ", " + x2 + "), (" + y1 + ", " + y2 + "));");
        eec[0] = (getrandom(x1, x2) - egj);
        eed[0] = (getrandom(y1, y2) - egk);
        dfd[0] = 0;
        dfe[0] = 0;
        dff[0] = 0;
        eka[0] = 920;
    ...
    public void AttackNPCVAR(int i)
    {
		System.out.println("AttackNPCVAR(" + i + ");");
        eka[0] = 715;
        eec[0] = eai[i].had;
        eed[0] = eai[i].hae;
        dfd[0] = eai[i].hab;
	...
	public void AtObjectCoordsWithItem(int coordx, int coordy, int itemid)
	{
		// System.out.println("AtObjectCoordsWithItem(" + coordx + ", " + coordy + ", " + itemid + ");");
		// int objid = 0;
        // for (int x = 0; x < 1500; x++ )
        // {
		// 	if (af[x] + ng == coordx && bf[x] + og == coordy)
		// 	{
		// 		objid = lg[x];
		// 	}
        // }
	...
	public void AtObjectCoordsWithItemByID(int coordx, int coordy, int itemid)
	{
		System.out.println("AtObjectCoordsWithItemByID(" + coordx + ", " + coordy + ", " + itemid + ");");
		super.aah.hfj(127);
        super.aah.hen(coordx);
        super.aah.hen(coordy);
        super.aah.hen(findinv(itemid));
        super.aah.hff();
	}
    ...
    {
		System.out.println("TalkToNPC(" + npcid + ");");
		Talk2NPC(GetNearestNPCbyID(npcid));
    }

	public void TalkToNPCVAR(int npcid)
    {
		System.out.println("TalkToNPCVAR(" + npcid + ");");
		Talk2NPC(GetNearestNPCbyID(npcid));
    }
	...
	public void ThieveNPCVAR(int i)
    {
		System.out.println("ThieveNPCVAR(" + i + ");");
        eka[0] = 725;
        eec[0] = eai[i].had;
        eed[0] = eai[i].hae;
        dfd[0] = eai[i].hab;
    ...
    {
		System.out.println("UseItemID(" + i + ");");
    ...
    public void ffe(int i)
    {
        ...
        if(i == 1010)
        {
			...
			if(AutoMenuIs == 6)
			{
            if (macrothread == false)
            {
                if (linecount > 0)
                {
                    rs = new RunScript();
                    macrothread = true;
					adh(" +++++ SCRIPT TURNED ON!");
                    System.out.println("Started running script.");
                } else {
					adh(" +++++ SCRIPT NOT LOADED!");
                    System.out.println("Script not loaded because of one of the below:");
					System.out.println(" - Not a valid file type (in terms of extension)");
					System.out.println(" - The file contains 0 lines.");
                }
            } else {
				adh(" +++++ SCRIPT IS ALREADY RUNNING!");
                System.out.println("Script is already running...");
            }
			}
        }
        if(i == 1011)
...
adh("@gre@Sgt Bot:  @whi@Guildminer coming soon!");
	...
	// scriptable
    public String lines[] = new String[10000];
    public String dump[] = new String[10000];
    public int dumpcount = 0;
    public int linecount = 0;
    public boolean macrothread = false;
    ...
    public boolean AutoLogging = false;
    public int combatmode = 1;
    public int scriptcount = 0;
    public int y = 0;
    public int sleepat = 95;
    public boolean setsleeping = false;
    public boolean drawing = true;

	private RunScript rs = null;
	// scriptable
