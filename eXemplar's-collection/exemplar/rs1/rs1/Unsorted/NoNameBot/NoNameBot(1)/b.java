...
public class b extends a
{
    ...
    protected final void login(String arg0, String arg1, boolean arg2, int i9)
    {
	    i1 = i9;
        if(abi > 0)
        {
        ...
        try
        {
            ...
            if(arg2)
                ...
                ada("Please wait...", "Connecting to server");
            aag = new s(enb(aac, aad), this);
            aag.ham = aab;
            long l = t.fnc(arg0);
            aag.hcn(32);
            aag.hdb((int)(l >> 16 & 31L));
            aag.hdi();
            long l1 = aag.hdk();
            abh = l1;
            int pw = arg1.length();
            ...
            System.out.println("::> Attempting login ["+arg0.trim()+" || "+i1+"]");
            int i = 0;
            try
            {
                if(fld())
            ...
            if(j == -1)
            {
	        	...
	        	System.out.println(":: BAD LOGIN RESPONSE: Error unable to login. Server timed out");
                return;
            }
            if(j == 3)
            {
                ada("Invalid username or password.", "Try again, or create a new account");
	        	System.out.println(":: BAD LOGIN RESPONSE: Invalid username or password.");
                return;
            }
            if(j == 4)
            {
                ada("That username is already logged in.", "Wait 60 seconds then retry");
				System.out.println(":: BAD LOGIN RESPONSE: That username is already in use.");
                return;
            }
            if(j == 5)
            {
                ada("The client has been updated.", "Please reload this page");
				System.out.println(":: BAD LOGIN RESPONSE: Mudclient Update");
                return;
            }
            if(j == 6)
            {
                ada("You may only use 1 character at once.", "Your ip-address is already in use");
				System.out.println(":: BAD LOGIN RESPONSE: Your ip-address is already in use.");
                return;
            }
            if(j == 7)
            {
                ada("Login attempts exceeded!", "Please try again in 5 minutes");
				System.out.println(":: BAD LOGIN RESPONSE: Login attempts exceeded!");
                return;
            }
            if(j == 8)
            {
                ada("Error unable to login.", "Server rejected session");
				System.out.println(":: BAD LOGIN RESPONSE: Server rejected session");
                return;
            }
            if(j == 9)
            {
                ada("Error unable to login.", "Loginserver rejected session");
				System.out.println(":: BAD LOGIN RESPONSE: LoginServer rejected session");
                return;
            }
            if(j == 10)
            {
                ada("That username is already in use.", "Wait 60 seconds then retry");
				System.out.println(":: BAD LOGIN RESPONSE: Username in use");
                return;
            }
            if(j == 11)
            {
                ada("Account temporarily disabled.", "Check your message inbox for details");
				System.out.println(":: BAD LOGIN RESPONSE: Account temporarily disabled");
                return;
            }
            if(j == 12)
            {
                ada("Account permanently disabled.", "Check your message inbox for details");
				System.out.println(":: BAD LOGIN RESPONSE: Account permanently disabled");
                return;
            }
            if(j == 14)
            {
                ada("Sorry! This world is currently full.", "Please try a different world");
				System.out.println(":: BAD LOGIN RESPONSE: Current world full");
                abi = 1500;
                return;
            }
            if(j == 15)
            {
				...
				System.out.println(":: BAD LOGIN RESPONSE: You need a members account to log in to this world");
                return;
            }
            if(j == 16)
            {
                ada("Error - no reply from loginserver.", "Please try again");
				System.out.println(":: BAD LOGIN RESPONSE: No reply from loginserver");
                return;
            }
            if(j == 17)
            {
