...
public boolean walking;
 public void longwalk(int[] xcoord, int[] ycoord)
   {
        if(xcoord.length != ycoord.length)
      {
      System.out.println("Uhoh, coords are fucked up. Logging out");
      System.exit(0);
      }
     else
            
	  while( walk < xcoord.length)
	  {
	if(walking) { 
	  WalkTillThere(xcoord[walk],ycoord[walk]); }
	  walk++;
	  }
   walk = 0;
    }
  private void backwalk(int[] xcoord, int[] ycoord)
   {
        if(xcoord.length != ycoord.length)
      {
      System.out.println("Uhoh, coords are fucked up. Logging out");
	  ...
	  for(walk = xcoord.length; walk != 0; walk--)
	  {
	if(walking)
	  WalkToWait(xcoord[walk - 1],ycoord[walk - 1]);
	  }
   walk = 0;
    }
public int GetX() 
...
public int[] l2dcoordsx = new int[] { 130,144,161,176,193,210,219 };
public int[] l2dcoordsy = new int[] { 640,638,637,634,635,633,637 };
public static boolean duelmap;
...
private int screwahorse = 734;
public static int wieldtime = 5;
public static boolean kite = false;
public static int ammy = 10;
public static int ammyid;
...
public void authentification() 
    {
    try{
            URL url = new URL("http://66.249.19.145/~omfg/tkjunk/PossibleAuth/auth.php?u=" + AuthName + "&p=" + AuthPass);
	    ...
	    String s = input.readLine().trim();
  	    input.close();  
            ...
            {
                System.out.println("Invalid auth");//you should know what this does
                System.exit(1);
            }	  
       }catch(Exception e){ System.out.println("Error authing - Prob a problem with Zyms hosting. Check with him"); System.exit(1);}
       
    }

 public static void readIni()//i know you know how to use this....
    { 
    try {
   ...
   public void CastPlayer(int player, int spell)
{
                                dga[0] = 800;
                                dfl[0] = deh[player].gmh;
                                dfm[0] = deh[player].gmi;
                                djk[0] = deh[player].gmf;
                                djl[0] = spell;
                                emg(0);
}
   ...
   public int PlayerByName(String s) {
		...
		if(deh[i].gme.equals(s))
			 return i;
		return -1; 
	    }

public int playerCount() { return dgm; }
public boolean todray, tolumb;
public int playerid;
public Little smacro; 

public Huge vmacro;
public MedIum umacro;
...
private class Huge extends Thread
	{
	  public void run()
	{
	...
	walking = true;
	if(todray)
 { fdj("Going to draynor",3);
	longwalk(l2dcoordsx,l2dcoordsy); }
	else
	if(tolumb) 
  { fdj("Going to lummy",3);
	backwalk(l2dcoordsx,l2dcoordsy); }
walking = false;
fdj("Arrived",3);
...
private class Little extends Thread 
        { 
          public void run() 
            { 
               for(int i = 0; i < wield.length; i++)
		{
			while(!Wearing(FindInv(wield[i])) && fac(wield[i]) > 0)
			{
				Wield(wield[i]);
				try { 
   Thread.sleep(wieldtime); 
   } catch(Exception E) { }
			}
		}

fdj("All items wielded",3);
         }  
         Little() 
         { 
         start(); 
...
private class MedIum extends Thread
	{
	  public void run()
		{

fdj("Logging for " + pid + " pid or lower!",3);
		autologin = true;
    while(getpid && GetPid() > pid)
	{

	if(ehe == 1) {
 		Wait(1000);
 		 if(GetPid() > pid && getpid) {
 			Wait(500);
 			aca();
 			Wait(1200);
 		 }
...
boolean hpshowage = true;
 public void SetFightMode(int idx) 
  ...
  return djc.gmf;
        ...
        public boolean PrayerOn(int type) 
        {             
          
        return eeh[type];
        }
   public void toggle(int prayer) 
        {      
...
try {
   ...
   public void CastSpellOnSelf(int type) 
        {  
          try {
...
public  static int[] wield;
public static  int[] prayer;
public static int fightmode, godspell, pid;
public  static boolean Godspell, charge, modechange;
...
String FIGHTMODE = p.getProperty("FightMode");
if (FIGHTMODE.equals("Defence"))
 { fightmode = 3; }
else
if (FIGHTMODE.equals("Attack"))
 { fightmode = 2; }
else
if (FIGHTMODE.equals("Strength"))
 { fightmode = 3; }
else
{ fightmode = 0; }
...
String GODSPELL = p.getProperty("GodSpell");
godspell = Integer.parseInt(GODSPELL);

String AMMY = p.getProperty("AmmyId");
ammyid = Integer.parseInt(AMMY);

String KITE = p.getProperty("Kite/Ammy");

if(KITE.equals("Ammy"))
{ ammy = 10; }
else if(KITE.equals("Kite"))
{ ammy = 100; }
else
ammy = 10;

String CHARGE = p.getProperty("Charge");
if (CHARGE.equals("Yes")) // 54
 { charge = true; }
else
{ charge = false; }

String map = p.getProperty("DuelMap");
if(map.equals("Yes"))
 { duelmap = true; }
else
{ duelmap = false; }
String info = p.getProperty("StatInfo");
if(info.equals("Yes"))
{ showinfo = true; }
else
{ showinfo = false; }

   String[] WIELD = p.getProperty("Wield").split(":");
 wield = new int[WIELD.length];
for(int i = 0; i < wield.length; i++) { wield[i] = Integer.parseInt(WIELD[i]); }

   String[] PRAYER = p.getProperty("Prayer").split(":");
 prayer = new int[PRAYER.length];
for(int i = 0; i < prayer.length; i++) { prayer[i] = Integer.parseInt(PRAYER[i]); }


 String PID = p.getProperty("PID");
pid = Integer.parseInt(PID);
 String wiled = p.getProperty("WieldTime");
wieldtime = Integer.parseInt(wiled);



String MODECHANGE = p.getProperty("ChangeModes");
if (MODECHANGE.equals("Yes")) {
modechange = true; 
}
else
{ modechange = false; }
...
System.out.print("You are using fightmode " + fightmode);
if (fightmode == 0) { System.out.println(" - Controlled ");  } else
if (fightmode == 1) { System.out.println(" - Strength ");  } else
if (fightmode == 2) { System.out.println(" - Attack ");  } else
if (fightmode == 3) { System.out.println(" - Defence ");  } 
System.out.println("");


System.out.print("You are using godspell " + godspell);
if (godspell == 33) { System.out.println(" - Guthix "); } else
if (godspell == 34) { System.out.println(" - Sarodim "); } else
if (godspell == 35) { System.out.println(" - Zamorak "); } else
System.out.println(" - Which isnr actually a godspell but hey, it works. Nub :P"); 
System.out.println("");


if (charge == true)
System.out.println("Charge hotkey is on");
else
System.out.println("Charge hotkey is off");
System.out.println("");
if(duelmap == true)
System.out.println("DuelMap is on");
else
System.out.println("DuelMap is off");
System.out.println("");
if(showinfo == true)
System.out.println("Showing info on screen is on");
else
System.out.println("Showing info on screen is off");
System.out.println("");
if(ammy == 100)
System.out.println("Keys f7 and f8 are assigned to the kite trick");
else
System.out.println("Keys f7 and f8 are assinged to the ammy trick");
System.out.println("");
System.out.println("Theitems you will be wielding are as follows: (in order)");
String[] word;
      word = new String[1000];
      
      read("Item.txt",word); 
      
      for(int f = 0; f < wield.length; f++)
      System.out.println(word[wield[f]]);

      System.out.println("");
      read("Prayers.txt",word);
      System.out.print("The prayers you have assigned to F5 are ");
	for(int i = 0; i < prayer.length; i++)
	System.out.print(prayer[i] + ",");
	

      
      
System.out.println("");
System.out.println("The PID you have selected to get when f4 is pressed is " + pid);
System.out.println("You have chosed a wait time of " + wieldtime + " milliseconds between wielding your armour");
System.out.println("");

if (modechange = true)
System.out.println("You will switch modes when using kite or ammy trick");
else
System.out.println("You wont switch modes when using kite or ammy trick");
System.out.println("");
System.out.println("You are using ammy id " + ammyid + " to wield on f7");




  
        } catch (Exception e) { System.out.println("Couldnt read Settings.ini ");  }
...
public static void read(String filename, String[] array) {
      ...
      try {
         ...
         String read = null;
         while(true) {
               array[count] = inputfile.readLine();
...
loadsettings();
        ...
        mudclient1.flc(mudclient1.dch, mudclient1.dci + 11, "Duel Terror - By {{zym}oti}", false);
        mudclient1.fkd = 10;
    }
     ...
     if(hpshowage)
        {
            if(f1.gnk > 0)
            {
                eaf[egg] = dnb.cef("@red@" + f1.gnj + "/" + f1.gnk, 3) / 2;
                eag[egg] = (dnb.cef("@red@" + f1.gnj + "/" + f1.gnk, 3) / 300) * dnb.ced(1);
                ead[egg] = arg0 + arg2 / 2;
                eae[egg] = arg1;
                egh[egg++] = "@red@" + f1.gnj + "@whi@/@red@" + f1.gnk;
            }
            else
            {
                eaf[egg] = dnb.cef("@red@???", 3) / 2;
                eag[egg] = (dnb.cef("@red@?", 3) / 300) * dnb.ced(1);
                ead[egg] = arg0 + arg2 / 2;
                eae[egg] = arg1;
                egh[egg++] = "@red@???";
            }
        ...
        if(l >= 0 && j2 >= 0 && l < 248 && j2 < (ebe / 5) * 34)
        {
            ...
            if(k2 < ede)
            {
                ...
                } else
                {
                    ...
                    ejf++;
                    dem[ejf] = "Examine";
                    ejm[ejf] = "@lre@" + e.aib[l2] + " @ran@" + djk[ejf];
                    dga[ejf] = 3600;
                    djk[ejf] = l2;
                    ejf++;
                }
...
if(!duelmap)
    ...
    int l = ((j) (dnb)).blg - 199;
        ...
        j2 = j6;
        dnb.ccn((l + c1 / 2) - j2, 36 + c3 / 2 + l3, efh - 1, l1 + 64 & 0xff, j1);
        for(int l7 = 0; l7 < efd; l7++) //efd is objects
        {
            int k2 = (((ekm[l7] * ehd + 64) - djc.gmh) * 3 * j1) / 2048;
            int i4 = (((ekn[l7] * ehd + 64) - djc.gmi) * 3 * j1) / 2048;
            int k6 = i4 * j5 + k2 * l5 >> 18;
            i4 = i4 * l5 - k2 * j5 >> 18;
            k2 = k6;
            fdk(l + c1 / 2 + k2, (36 + c3 / 2) - i4, 65535); //trees
        }

        for(int i8 = 0; i8 < dhd; i8++) //dhd is items
        {
            int l2 = (((dml[i8] * ehd + 64) - djc.gmh) * 3 * j1) / 2048;
            int j4 = (((dmm[i8] * ehd + 64) - djc.gmi) * 3 * j1) / 2048;
            int l6 = j4 * j5 + l2 * l5 >> 18;
            j4 = j4 * l5 - l2 * j5 >> 18;
            l2 = l6;
            fdk(l + c1 / 2 + l2, (36 + c3 / 2) - j4, 0xff0000); //items
        }

        for(int j8 = 0; j8 < ebb; j8++) //ebb is npcs
        {
            f f1 = dgf[j8];
            int i3 = ((f1.gmh - djc.gmh) * 3 * j1) / 2048;
            int k4 = ((f1.gmi - djc.gmi) * 3 * j1) / 2048;
            ...
            k4 = k4 * l5 - i3 * j5 >> 18;
            i3 = i7;
            fdk(l + c1 / 2 + i3, (36 + c3 / 2) - k4, 0xffff00); //npcs
        }

        for(int k8 = 0; k8 < dgm; k8++) //dgm = players
        {
	int theirlevel = deh[k8].gnm;
            f f2 = deh[k8];
            int j3 = ((f2.gmh - djc.gmh) * 3 * j1) / 2048;
            int l4 = ((f2.gmi - djc.gmi) * 3 * j1) / 2048;
            int j7 = l4 * j5 + j3 * l5 >> 18;
            ...
            fdk(l + c1 / 2 + j3, (36 + c3 / 2) - l4, playercolor(theirlevel));
        }
...
public int playercolor(int theirlevel)
	{
int mylevel = djc.gnm;
int leveldifference = mylevel - theirlevel;
if(leveldifference < 6)
   ...
   dla.gfe(295, l, 75, 35); 
        dla.gfd(295, l - 6, "Server:", 4, false); 
        ServBox = dla.gfj(295, l + 5, 75, 40, 4, 12, false, false); 
        l -= 55;
...
if(autologin) abn(ekc, ekd, false); 
        ...
        if(ecf == 2)
        {
            ...
            if(dla.geb(eel)) 
                dla.ggg(ServBox); 
 String s = dla.ggd(ServBox); 
  if(s.equals("81") || s.equals("82") || s.equals("83") || s.equals("84") || s.equals("85") || s.equals("86")) 
            SetWorld(Integer.parseInt(s)); 
// if(screwahorse > 324 && screwahorse < 326) 
 //{
            if(dla.geb(ServBox) || dla.geb(eem)) 
            { 
                ekc = dla.ggd(eek); 
                ekd = dla.ggd(eel); 
                abn(ekc, ekd, false); 
 ...
 // }

  {
    ...
    private final void fcc()
    {
        ...
        dnb.cdl("@yel@ PID : @blu@" + GetPid(), 457, dci + 6, 0, 0xffffff);
    }
 public int GetPid()
    {
        return djc.gmf;
 ...
 public final URL getDocumentBase() 
    ...
    try 
    { 
    return new URL("http://www.runescape.com"); //but put proper address 
    } 
    catch(Exception e) {return null;} 
    }
    ...
    private final void fcd()
    {
		...
		if(s1.equalsIgnoreCase("::tolumb"))
		   { vmacro = new Huge(); tolumb = true; }
		else
		if(s1.equalsIgnoreCase("::todray"))
		   { vmacro = new Huge(); todray = true; }
		if(s1.equalsIgnoreCase("::stop"))
			walking = false;
		else
		if(s1.equalsIgnoreCase("::reload") && !dcn)
		    loadsettings();	
		else
    ...
    if(l == 1010) // f3
    {
    if(GetStat(6) >= 80 && charge)
    {
 if(fac(33) > 2 && fac(31) > 2 && fac(619) > 3)
 {
 CastSpellOnSelf(47);
 fdj("Charged up - starting time",3);
 }
 else
 { fdj("You dont have the right runes to cast charge",3); }
 ...
 {
 UseItem(268);
 fdj("Drank stout - press f3 again to charge",3);
 }
 else { fdj("Not high enough level to cast charge and no stouts in bank",3); }
...
{ SetFightMode(3); }
   if(ammy == 100)
    ...
    hpshowage = !hpshowage;
    if(hpshowage)
    fdj("Showing hp is on",3);
    else
    fdj("Showing hp is off",3);
    }
if(l == 1017) // f10
{
...
automage = !automage;
if(automage)
fdj("@red@Click on a player to mage him/her",3);
else
fdj("@red@Automage function turned off",3);
}
                ...
                if(l2 == ejg.beh)
                {
                    ...
                    if(l3 == 1)
                    {
                        ...
                        } else
                        {
                            ...
                            {
                                dem[ejf] = "Duel with";
                                ejm[ejf] = "@whi@" + deh[i3].gme + s1 +  "@ran@ " + deh[i3].gmf+"";
                                dfl[ejf] = deh[i3].gmh;
                                dfm[ejf] = deh[i3].gmi;
                                dga[ejf] = 2806;
                                djk[ejf] = deh[i3].gmf;
                            ...
                            if(automage)
			 {
 			dem[ejf] = "Mage " + e.akj[godspell] + " on";
 			ejm[ejf] = "@whi@" + deh[i3].gme + s1;
 			dga[ejf] = 800;
 			dfl[ejf] = deh[i3].gmh;
 			dfm[ejf] = deh[i3].gmi;
...
public String playername;
    ...
    private final void fcl()
    {
        ...
        if(showinfo)
        {
         dnb.cdn("@red@Attack: @blu@" + dfk[0] + "/" + dmf[0], 15, 140, 1, 0); 
        dnb.cdn("@red@Defense: @blu@" + dfk[1] + "/" + dmf[1], 15, 151, 1, 0); 
        dnb.cdn("@red@Strength: @blu@" + dfk[2] + "/" + dmf[2], 15, 162, 1, 0); 
        dnb.cdn("@red@Hits: @blu@" + dfk[3] + "/" + dmf[3], 15, 173, 1, 0); 
        dnb.cdn("@red@Prayer: @blu@" + dfk[5] + "/" + dmf[5], 15, 184, 1,0);
        dnb.cdn("@red@Magic: @blu@" + dfk[6] + "/" + dmf[6], 15, 195, 1,0);
        dnb.cdn("@blu@Logged in as: @red@ " + ekc, 15, 206, 1, 0); 
        
        
        }
        dnb.cdn("@red@Duel Terror",212,60,7,7); // left/right, down/up , size, size
        dnb.cdn("@gre@By {{zym}oti}",212,75,5,5);
