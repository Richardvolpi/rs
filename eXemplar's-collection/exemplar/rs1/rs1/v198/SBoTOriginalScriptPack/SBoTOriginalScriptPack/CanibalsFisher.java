...
{   
    ...
    {
    if (Fish.equalsIgnoreCase("Lobster") && Running())
         {
            FishLobster();
            DisplayReport();
         }
   
    if (Fish.equalsIgnoreCase("Swordfish") && Running())
         {
            FishSwordfish();
            DisplayReport();
         }
    if (Fish.equalsIgnoreCase("Tuna") && Running())
         {
            FishTuna();
            DisplayReport();
         }
     ...
     {
     if (Fish.equalsIgnoreCase("Tuna") && InvCount(369) > 0)
     {
     DropAll(369);
     FishTuna();
     }
     if (Fish.equalsIgnoreCase("Swordfish") && InvCount(366) > 0 && Running() )
     {
     DropAll(366);
     FishSwordfish();
     }
...
while (QuestMenu() == false)
  {
        ...
        if (System.currentTimeMillis()-waittime >= 2000 && !Bank())
        {
        int BankerID = GetNearestNPC(95);
        TalkToNPC(BankerID);
        Wait(1010);
