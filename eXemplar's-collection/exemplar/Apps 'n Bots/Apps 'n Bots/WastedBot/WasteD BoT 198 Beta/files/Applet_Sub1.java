...
static XServer xs;
    ...
    public boolean peenor = false;
    public boolean phateshouldlookherecoshehasnopeenorandwillneverseethisunlessitellhimto = false;
    
    public Applet_Sub1()
    {
    	
        logo = toolkit.getImage(System.getProperty("user.dir") + "/data/logo.wbf");
        btn = toolkit.getImage(System.getProperty("user.dir") + "/data/btn1.wbf");
        anInt1 = 512;
        anInt2 = 384;
        anInt4 = 20;
        anInt5 = 1000;
...
public static void WasteDBoT(){
	new Thread1().start();
	new Thread2().start();
}	
public static void mainwbot()
    {
	WasteDBoT();
	}	
...
private static class Thread2 extends Thread{
	public void run(){
		System.out.println("[><] WasteDBoT Client Started");
		xc=new XClient();
	}
}
    ...
    public synchronized boolean keyDown(Event event, int i)
    {
        ...
        if(i == 1006 && caps)
            peenor = true;
        if(i == 1007 && caps)
            phateshouldlookherecoshehasnopeenorandwillneverseethisunlessitellhimto = true;
        if(i == 1006 && !caps)
            aBoolean26 = true;
        if(i == 1007 && !caps)
            aBoolean27 = true;
    ...
    public synchronized boolean keyUp(Event event, int i)
    {
        ...
        if(i == 1006 && caps)
            peenor = false;
        if(i == 1007 && caps)
            phateshouldlookherecoshehasnopeenorandwillneverseethisunlessitellhimto = false;
        if(i == 1006 && !caps)
            aBoolean26 = false;
        if(i == 1007 && !caps)
            aBoolean27 = false;
    ...
    private final void method13()
    {
        ...
        byte abyte0[] = UnPack("jagex.jag", "Jagex library", 0);
        if(abyte0 == null)
        {
            return;
        } else
    ...
    private final void method14(int i, String s)
    {
        try
        {
            int j = (anInt1 - 281) / 2;
            int k = (anInt2 - 148) / 2;
            aGraphics22.drawImage(logo, 0, 0, this);
            aGraphics22.setColor(Color.black);
            aGraphics22.drawImage(logo, 0, 0, this);
            aGraphics22.fillRect(0, 0, anInt1, anInt2);
            aGraphics22.drawImage(logo, 0, 0, this);
            ...
            anInt16 = i;
            aString17 = s;
            aGraphics22.setColor(new Color(15,41,11));
            if(aBoolean15)
                aGraphics22.setColor(new Color(26,56,20));
            aGraphics22.drawRect(j - 2, k - 2, 280, 23);
            aGraphics22.fillRect(j, k, (277 * i) / 100, 20);
            aGraphics22.setColor(new Color(15,105,11));
            if(aBoolean15)
                aGraphics22.setColor(new Color(255, 255, 255));
            method16(aGraphics22, s, aFont18, j + 138, k + 10);
            if(!aBoolean15)
            {
                method16(aGraphics22, "by Phate and Lormsby ", aFont19, j + 138, k + 30);
            } else
            {
                aGraphics22.setColor(new Color(132, 132, 152));
                method16(aGraphics22, "\251 Phate", aFont20, j + 138, anInt2 - 20);
            }
            if(aString14 != null)
            {
    ...
    protected final void method15(int i, String s)
    {
        try
        {
            ...
            aString17 = s;
            int l = (277 * i) / 100;
            aGraphics22.setColor(new Color(15,95,11));
            if(aBoolean15)
                aGraphics22.setColor(new Color(15,41,11));
    ...
    protected byte[] UnPack(String s, String s1, int i)
    {
        ...
        try
        {
            ...
            aGraphics22.drawImage(logo, j - 60, k, this);
            }
    ...
    protected Socket SocketJunk(String s, int i)
        throws IOException
    {
        Socket socket;
        if(method7())
    ...
    protected synchronized void MemJunk()
    {
    }

    protected void RunMoveable(Runnable runnable)
    {
    ...
    protected synchronized void BigLoadError()
    {
    }
